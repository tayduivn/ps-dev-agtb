...
function post_install()
{
    ...
    if($sugar_config['dbconfig']['db_type'] == 'mysql') {
	   $log->info('Running SQL file 610_ent_to_ult_mysql.sql');
	   $schemaFile = "$self_dir/610_ent_to_ult_mysql.sql";
    } else if ($sugar_config['dbconfig']['db_type'] == 'mssql') {
	   $schemaFile = "$self_dir/610_ent_to_ult_mssql.sql";
	   if(in_array(get_class($db),array('SqlsrvManager','FreeTDSManager')) && file_exists("$self_dir/610_ent_to_ult_mssql_freetds.sql")){
	       $schemaFile = "$self_dir/610_ent_to_ult_mssql_freetds.sql";
	   }	   
