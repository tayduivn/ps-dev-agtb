...
foreach ($accounts as $account_id => $account_months) {
	...
	foreach ($month_data as $row) {
	    ...
	    $data[] = array('Month' => $current_month,
			    'Account' => $account_id,
			    ...
			    'Rolling Active Subs' =>          $account_active_subs,
			    'Rolling New Subs' =>             $account_new_subs,
			    'Rolling Cancelled Subs' =>       $account_cancelled_subs,
			    
			    'Rolling Month Active Subs' =>    $months[$current_month]['Active Subs'],
			    'Rolling Month New Subs' =>       $months[$current_month]['New Subs'],
			    'Rolling Month Cancelled Subs' => $months[$current_month]['Cancelled Subs'],
			    );
	}
    }
    $data[] = array('Month' => $current_month,
		    ...
		    'Opportunity Name' => 'Account Total',
		    'Opp' => '',
		    'Subs' => 0,
...
foreach ($data as $row) {
    if ($first) {
	$first = false;
	echo join(',', array_map('csv_field', array_keys($row)));
	echo "\n";
    }
    echo join(',', array_map('csv_field', $row));
    echo "\n";
}
...
header("Pragma: cache");
header("Content-Disposition: inline; filename=Subscriber_List.csv");
