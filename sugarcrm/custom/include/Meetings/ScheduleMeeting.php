   ...
   function schedule(&$bean, $event, $arguments) {
      ...
      if ($bean->type == 'WebEx') {
         ...
         if ($bean->external_id == '') {
            ...
            $params = array(
               'subject' => $bean->name,
               'startTime' => date($this->date_format, strtotime($bean->date_start)),
               'password' => $bean->password
            );
            $meeting_keys = array(
               substr($bean->external_id, 0, strpos($bean->external_id, '-')),
               substr($bean->external_id, strpos($bean->external_id, '-') + 1)
            );
            $response =$this->meeting->editMeeting($meeting_keys, $params);
            echo $response;
            $this->meeting->logoff();
         }
