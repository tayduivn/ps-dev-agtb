...
*}
{literal}<script>
if(typeof GoogleAlertsDashlet == 'undefined') { // since the dashlet can be included multiple times a page, don't redefine these functions
	GoogleAlertsDashlet = function() {
		var _anim;
			...
			animate: function(speed){
					...
					_computedScale = _computedScale + _durationConst;
      				...
      				//if(!_anim){
   					...
   					//}
					_anim.animate(); 
					_anim.onComplete.subscribe(GoogleAlertsDashlet.complete);
			},
			stop: function(){		
					_anim.stop();
			},
...
//YAHOO.util.Event.onAvailable('rss_{/literal}{$id}{literal}', GoogleAlertsDashlet.animate);
</script>{/literal}