...
class OpportunitiesViewMasscloseopportunitiesfinance extends SugarView 
{	
    /**
     * Constructor
     */
 	...
 	public function display()
    {
		...
		$header_display = '';
		if (!in_array($GLOBALS['current_user']->user_name, array('sadek', 'jmullan'))
		    && !$GLOBALS['current_user']->check_role_membership('Finance')){
		//if(!$GLOBALS['current_user']->check_role_membership('Finance')){
			sugar_die('You do not have access to this page. Please contact Internal Systems if you think you should have access.');
		}
		else{
			// salesops user
			$assign_task_to = 'bc5bc161-c4f1-422f-d76d-45bab06c211b';
			$reject_sales_stage = 'Closed Won';
			$before_sales_stage = 'Sales Ops Closed';
			$after_sales_stage = 'Finance Closed';
			$header_display = 'Finance Opportunities (currently Sales Ops Closed)';
		}
...
EOQ;
		...
		** ITRequest #: 19969
		** Description: Update report post moof to point at new orders module, add partner assigned to and revenue type
		*/
		...
		" accounts.id 'account_id',
		  ...
		  opportunities_cstm.partner_assigned_to_c 'partner_assigned_to',
		  assigned_user.user_name 'assigned_user',
                  orders.id 'order_guid', orders.payment_method 'payment_method',
                  partnerAccount.name 'partner_name'
		";
		$query_from = 
		"  opportunities inner join opportunities_cstm on opportunities.id = opportunities_cstm.id_c
		   ...
		   inner join accounts on accounts_opportunities.account_id = accounts.id and accounts.deleted = 0
                   left join orders on opportunities_cstm.order_number = orders.order_id
                   left join accounts partnerAccount on opportunities_cstm.partner_assigned_to_c = partnerAccount.id
		";
		$query_where = 
		...
		$headers_details = array(
			...
			'opportunities_cstm.partner_assigned_to_c' => 'Partner',
		...
		$headers_details['reject_to_sales_rep'] = 'Reject (To Rep)';
		...
		while($row = $GLOBALS['db']->fetchByAssoc($res)){
			$details = array(
				...
				"<a href=index.php?module=Accounts&action=DetailView&record={$row['partner_assigned_to']} target=_blank>{$row['partner_name']}</a>",
				...
				"<center><a href='/index.php?module=Orders&action=DetailView&record={$row['order_guid']}' target=_blank>{$row['order_number']}</a></center>",
				"<center>{$row['payment_method']}</center>",
			...
			$details[] = "<center><input type='radio' name='oppclose{$row['id']}' value='reject_to_sales_rep'></center>";
