...
$quote[2]['VALUE'] = $rep->first_name.' '.$rep->last_name;
$quote[3]['VALUE'] = $focus->date_quote_expected_closed;
$options['showLines'] = 0;
$options['shaded'] = 0;
...
$pdf->ezTable($quote,'' ,$mod_strings['LBL_PDF_QUOTE_TITLE'] ,$options);
...
if(is_array($product_bundle_list)){
	...
	foreach ($ordered_bundle_list as $product_bundle) {
        ...
        /* BEGIN SUGARINTERNAL CUSTOMIZATION */
        if (isset($in_total_group_stages)
            && key_exists($product_bundle->bundle_stage, $in_total_group_stages)) {
            /* END SUGARINTERNAL CUSTOMIZATION */
			$count = 0;
			$item = array();
				...
				if (is_array($bundle_list)) {
					while (list($key, $line_item) = each ($bundle_list)) {
						...
						if ($line_item->object_name == "Product") {
							$item[$count][$mod_strings['LBL_PDF_ITEM_QUANTITY']] = $line_item->quantity;
                            ...
                            // jwhitcraft customization to remove list_price from a quote ITR:12426
		...
		// BEGIN SUGARINTERNAL CUSTOMIZATION: CLOSE PARENT FOR CUSTOMIZATION BLOCK ABOVE
		}
		// END SUGARINTERNAL CUSTOMIZATION: CLOSE PARENT FOR CUSTOMIZATION BLOCK ABOVE
		}
	}
}
...
// BEGIN SUGARINTERNAL CUSTOMIZATION - CHANGE FILENAME
$filename = preg_replace("#[^A-Z0-9\-_\.]#i", "_", $focus->billing_account_name);
if (!empty($focus->quote_num)) {
    $filename .= "_{$mod_strings['LBL_PROPOSAL']}_{$focus->quote_num}";
}
$filename = $mod_strings['LBL_PROPOSAL']."_{$filename}.pdf";
// END SUGARINTERNAL CUSTOMIZATION - CHANGE FILENAME
...
/* BEGIN SUGARINTERNAL CUSTOMIZATION */
if (defined('give_me_a_pdf')
    || (isset($_REQUEST['email_action']) && $_REQUEST['email_action']=="EmailLayout")) {
    /* END SUGARINTERNAL CUSTOMIZATION */
