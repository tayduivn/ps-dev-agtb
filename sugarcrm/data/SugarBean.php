	...
	function mark_deleted($id)
	{
		...
		else
		{
            ...
            if ( isset($this->field_defs['modified_user_id']) ) {
                ...
                //BEGIN SUGARCRM flav=pro ONLY
                if ($this->isFavoritesEnabled()) SugarFavorites::markRecordDeletedInFavorites($id, $date_modified, $this->modified_user_id);
                //END SUGARCRM flav=pro ONLY
            } else {
                $query = "UPDATE $this->table_name set deleted=1 , date_modified = '$date_modified' where id='$id'";
                //BEGIN SUGARCRM flav=pro ONLY
                if ($this->isFavoritesEnabled()) SugarFavorites::markRecordDeletedInFavorites($id, $date_modified);
                //END SUGARCRM flav=pro ONLY
            }
