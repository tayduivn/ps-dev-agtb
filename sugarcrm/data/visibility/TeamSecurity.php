...
class TeamSecurity extends SugarVisibility
{
    ...
    protected function addVisibility(&$query)
    {
        ...
        // Workflows are exempt from TeamSec
        if($this->bean->module_dir == 'WorkFlow' || $this->module_dir == 'WorkFlow') return;
        if($this->bean->disable_row_level_security) return;

        global $current_user;
        if (empty($current_user) || $current_user->isAdminForModule($this->module_dir)) {
            return;
        }
