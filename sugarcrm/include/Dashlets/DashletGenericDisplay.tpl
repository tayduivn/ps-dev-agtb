				...
				<td scope='row' align='{$params.align|default:'left'}' valign=top class='{$_rowColor}S1' bgcolor='{$_bgColor}'><span sugar="sugar{$colCounter}b">
					...
					{elseif $params.type == 'multienum'}
						{if !empty($rowData.$col)} 
							...
							{/foreach}	
						{/if}
					{elseif $col|lower == 'set_complete'}
					   {assign var='tmpModule' value=$params.module|default:$pageData.bean.moduleDir}
					   {if $tmpModule == 'Tasks'}
					       {assign var='closeStatus' value='Completed'}
					   {else}
					       {assign var='closeStatus' value='Held'}
					   {/if}
					<a onclick='YAHOO.SUGAR.closeActivityPanel.show("{$tmpModule}","{$rowData.ID}","{$closeStatus}","{$dashletId}");'><img border="0" src="{sugar_getimagepath file="close_inline.png"}"></a>
