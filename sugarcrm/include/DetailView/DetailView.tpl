...
{{else}}
    ...
    <h4>
      <a href="javascript:void(0)" class="collapseLink" onclick="setCollapseState('{{$module}}_d', '{{$smarty.foreach.section.iteration}}', true); collapsePanel({{$smarty.foreach.section.iteration}});">
      <img border="0" id="detailpanel_{{$smarty.foreach.section.iteration}}_img_hide" src="{sugar_getimagepath file="basic_search.gif"}"></a>
      <a href="javascript:void(0)" class="expandLink" onclick="setCollapseState('{{$module}}_d', '{{$smarty.foreach.section.iteration}}', false); expandPanel({{$smarty.foreach.section.iteration}});">
