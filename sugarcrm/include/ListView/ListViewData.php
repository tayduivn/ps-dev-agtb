...
class ListViewData {
    ...
    function getAdditionalDetails($fields, $adFunction, $editAccess)
    {
        ...
        if($editAccess) $extra .= (!empty($results['editLink']) ? "<a title=\'{$app_strings['LBL_EDIT_BUTTON']}\' href={$results['editLink']}>SugarThemeRegistry::current()->getImage('edit_inline', 'border=0', null, null, ".gif", $mod_strings['LBL_EDITINLINE'])</a>" : '');
        $extra .= (!empty($results['viewLink']) ? "<a title=\'{$app_strings['LBL_VIEW_BUTTON']}\' href={$results['viewLink']}>".SugarThemeRegistry::current()->getImage('view_inline', 'style=\'margin-left: 2px;\' border=0 ', null, null, ".gif", $mod_strings['LBL_VIEWINLINE'])."</a>" : '')
            . "', DELAY, 200, STICKY, MOUSEOFF, 1000, WIDTH, "
            . (empty($results['width']) ? '300' : $results['width'])
            . ", CLOSETEXT, SugarThemeRegistry::current()->getImage('close', 'border=0 style=\'margin-left:2px; margin-right: 2px;\' ', null, null, ".gif", $mod_strings['LBL_CLOSEINLINE']).'</div>', "
            ...
            . "onmouseout=\"return nd(1000);\">".SugarThemeRegistry::current()->getImage('info_inline', "style='padding: 0px 5px 0px 2px' border='0'", null, null, '.png', $mod_strings['LBL_INFOINLINE'])."</span>";
