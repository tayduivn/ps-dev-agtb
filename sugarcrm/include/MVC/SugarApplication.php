...
class SugarApplication
{
    ...
    function loadUser()
    {
        ...
        if (isset($_SESSION['authenticated_user_theme_color'])) {
            ...
            self::setCookie(
                'ck_login_theme_color_20', $_SESSION['authenticated_user_theme_color'], time() + 86400 * 90
            );
        }
        if (isset($_SESSION['authenticated_user_theme_font'])) {
            $GLOBALS['log']->debug(
                "setting cookie ck_login_theme_font_20 to " . $_SESSION['authenticated_user_theme_font']
            );
            self::setCookie('ck_login_theme_font_20', $_SESSION['authenticated_user_theme_font'], time() + 86400 * 90);
        }
        if (isset($_SESSION['authenticated_user_language'])) {
            $GLOBALS['log']->debug(
                "setting cookie ck_login_language_20 to " . $_SESSION['authenticated_user_language']
            );
    ...
    )
    {
        ...
        if (file_exists("custom/application/Ext/Language/en_us.lang.ext.php")) {
            $app_list_strings = self::_mergeCustomAppListStrings(
                "custom/application/Ext/Language/en_us.lang.ext.php", $app_list_strings
            );
        }
        ...
        if ($language_used != $default_language) {
            if (file_exists("custom/application/Ext/Language/$default_language.lang.ext.php")) {
                $app_list_strings = self::_mergeCustomAppListStrings(
                    "custom/application/Ext/Language/$default_language.lang.ext.php", $app_list_strings
                );
                $GLOBALS['log']->info("Found extended language file: $default_language.lang.ext.php");
            }
        ...
        if (file_exists("custom/application/Ext/Language/$language.lang.ext.php")) {
            $app_list_strings = self::_mergeCustomAppListStrings(
                "custom/application/Ext/Language/$language.lang.ext.php", $app_list_strings
            );
            $GLOBALS['log']->info("Found extended language file: $language.lang.ext.php");
        }
    ...
    )
    {
        ...
        foreach ($app_list_strings as $key => $value) {
            $exemptDropdowns = array(
                "moduleList", "parent_type_display", "record_type_display", "record_type_display_notes"
            );
