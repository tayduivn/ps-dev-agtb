...
class MetaDataManager {
    ...
    public function getSugarFields()
    {
        ...
        foreach ( $allSugarFields as $fieldName ) {
            ...
            foreach ( $backwardsPlatforms as $platform ) {
                $templateDirs[] = $baseFieldDirectory.$fieldName."/${platform}/";
            }
            $fieldData['templates'] = $this->getHbtTemplates($templateDirs);
    ...
    protected function getViewTemplates() {
        $templates = $this->getHbtTemplates('include/templates/'.$this->platforms[0]);
