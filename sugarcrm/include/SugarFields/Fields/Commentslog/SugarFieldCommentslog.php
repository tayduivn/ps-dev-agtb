...
class SugarFieldCommentslog extends SugarFieldBase
{
    /**
     * @inheritdoc
     */
    ...
    public function apiFormatField(array &$data, SugarBean $bean, array $args, $fieldName, $properties, array $fieldList = null, ServiceBase $service = null)
    {
        ...
        $msg_beans = $bean->commentslog_link->getBeans(array('orderby' => 'date_entered'));
        ...
        foreach ($msg_beans as $msg_bean) {
            if (!$msg_bean->created_by_name) {
                ...
                $msg_bean = BeanFactory::retrieveBean('Commentslog', $msg_bean->id, array('use_cache' => false));
            }

            // newly created commentslog tends to not like to have created_by_link,
    ...
    {
        if (!isset($params['commentslog']) || !is_string($params['commentslog'])) {
            return; // don't do anything it nothing is entered
        }

        $commentslog_bean = BeanFactory::newBean('Commentslog');
        $commentslog_bean->setEntry($params['commentslog']);
        $commentslog_bean->setModule($bean->getModuleName());
        $commentslog_bean->save();

        $bean->load_relationship('commentslog_link');
        if (!$bean->commentslog_link->add($commentslog_bean)) {
            LoggerManager::getLogger()->fatal("Failed to add commentslog");
        }
    }
}
