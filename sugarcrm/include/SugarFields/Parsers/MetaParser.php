...
/**
 * createReportFile
 * Accept the path to two files and creates a file to report the results
 * @param $customPanels Array of panels created from parsing the HTML file
 * @param $createdFile Path pointing to file that was created after parsing
 * @param $masterFile Path pointing to the master copy file
 * @param $reportPath Path pointing to location where to create report file
 * 
 */
function createReportFile($customPanels=array(), $createdFile, $masterFile, $reportPath, $view) {
   if(!is_array($customPanels)) {
   	  $GLOBALS['log']->error("expecting parameter1 to be an Array");
   	  return;
   }
   if(!file_exists($createdFile)) {
   	  $GLOBALS['log']->error("$createdFile could not be found");
   }
   if(!file_exists($masterFile)) {
   	  $GLOBALS['log']->error("$masterFile could not be found");
      return;
   }
   if(!file_exists($reportPath)) {
   	  $GLOBALS['log']->error("$reportPath could not be found");
   	  return;
   }
   
   $report = array();
   
   $elements0 = $this->getPanelsElements($customPanels, $view);
   
   require($createdFile);
   $panels = $this->getPanels($viewdefs);
   $elements1 = $this->getPanelsElements($panels, $view);

   require($masterFile);
   $panels = $this->getPanels($viewdefs);
   $elements2 = $this->getPanelsElements($panels, $view);
   
   $elementsRemovedFromCustom = array();
   foreach($elements0 as $name) {
   	  if(!isset($elements1[$name])) {
   	  	 $elementsRemovedFromCustom[] = $name;
   	  }
   }
   
   $report['elements_removed_from_original'] = $elementsRemovedFromCustom;
   
   $elementsMissingFromMasterCopy = array();
   foreach($elements2 as $name) {
   	  if(!isset($elements1[$name])) {
   	  	 $elementsMissingFromMasterCopy[] = $name;
   	  }
   }
   
   $report['elements_missing_from_master_copy'] = $elementsMissingFromMasterCopy;
   
   $elementsAdded = array();
   foreach($elements1 as $name) {
   	  if(!isset($elements2[$name])) {
   	  	 $elementsAdded[] = $name;
   	  }
   }
   
   $report['elements_added'] = $elementsAdded;
   $header  = "<?php\n\n";
   $header .= "\$report = " . var_export($report, true);	
   $header .= ";\n ?>";

   $evfp = fopen($reportPath . "/$view" . "_results.php",'w');
   fwrite($evfp, $header);
   fclose($evfp);   
}
 ...
 * 
 */
function getPanels($viewdefs) {
   $key0 = array_keys($viewdefs);
   $key1 = array_keys($viewdefs[$key0[0]]);
   $panels = $viewdefs[$key0[0]][$key1[0]]['panels'];
   return $panels;
}

/**
 * getPanelsElements
 * Given the $panels Array in a metadata file, return an single-dimension
 * Array of the field names sorted
 * @param $panels Array of $panels; 
 */
function getPanelsElements($panels = array(), $view) {
   ...
   //If it's not an editview panel, we have to simulate multiple panels
   //$panels = $view == 'editview' ? $panels : array($panels);
   ...
   if(!$this->hasMultiplePanels($panels)) {
	  $panels = array('default'=>$panels);
   }   
...
function test() {
   $errors = array();
   $html = "aaa<tr><td>abc</td><td><table><tr><td>123</td></tr><tr><td>123</td></tr></table></td></tr>bbb";
   $val = $this->getElementsByType('tr', $html);
   if(count($val) != 1) {
   	  $errors[] = "failed test 1";
   }  
   $html = "<tr>...</tr><tr>...</tr><tr><tr></tr></tr>";
   $val = $this->getElementsByType('tr', $html);
   if(count($val) != 3) {
   	  $errors[] = "failed test 2";
   }
