	...
	function addTeamSecurityClause() {
		...
		if(!is_admin($current_user)) {
            $dbResult = $this->db->query(
                'SELECT team_id FROM team_memberships WHERE user_id=' . $current_user->id . ' AND deleted=0'
            );
            $teamsIds = array();
            while ($team = $this->db->fetchByAssoc($dbResult)) {
                $teamsIds[] = $team['team_id'];
            }
            $teamsIds = array_unique($teamsIds);
            return ' AND emails.team_set_id IN ("' . implode('", "', $teamsIds) . '") ';
		}
	}
	//END SUGARCRM flav=pro ONLY
