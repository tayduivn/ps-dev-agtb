    ...
    public function connect(array $configOptions = null, $dieOnError = false)
    {
        ...
        if (!$this->connected) {
            $message = "Could not connect to DB server with options ".
                ...
                $this->connectOptions['db_port'].": ".mysqli_connect_error();

            $this->registerError('', $message, $dieOnError);
            return false;
        }
                
        if (!empty($this->connectOptions['db_name']) && !$this->selectDb($this->connectOptions['db_name'])) {
            $this->registerError('', "Unable to select database {$dbname}: ".mysqli_connect_error(), $dieOnError);
            return false;
        }
