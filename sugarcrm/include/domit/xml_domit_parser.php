...
/*File Changes :
 * 23/10/2007 - Benjamin : line 3218 add if() to prevent bug with one french RSS
 * 
 * 
 */
...
class DOMIT_Parser {
	...
	*/
	function parse (&$myXMLDoc, $xmlText, $preserveCDATA = true) {
		...
		$xmlText = preg_replace('/>\s</i' , '><', $xmlText);
		...
		if ((strlen($data) > 2)  && ($this->parseItem == '')){
			...
			switch ($pre) {
				...
				case '<![': //cdata section coming
					...
					break;	
				case '<!-': //comment
					if($this->commentElement($this, substr($data, 4, (strlen($data) - 7)))){
