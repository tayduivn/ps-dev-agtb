...
//<<<<----------Pick all the returned values into an array, so we can merge the non-primary mails into one field with delimeters. This merge is done only for ACCOUNTS module.
       $assocArray = array();
       $i = 0;
       while($val = $db->fetchByAssoc($result, false)) {
         if (isset($val['primary_email_address']) && strtolower($focus->module_dir) == "accounts") {
           if ($val['primary_email_address'] == 1) {
             $assocArray[$i] = $val;
             $i++;
           } else {
             $assocArray[$i - 1]['any_email'] .= $val['email_address'] . ";";
           }
         } else {
           $assocArray[] = $val;
         }
...
function get_field_order_mapping($name='',$reorderArr = '', $exclude = true){
    ...
    $fields_to_exclude['accounts'] = array('account_name', 'primary_email_address');//<<<----$fields_to_exclude['accounts'] = array('account_name');
