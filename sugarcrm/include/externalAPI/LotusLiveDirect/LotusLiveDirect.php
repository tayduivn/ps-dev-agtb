...
//        $GLOBALS['log']->debug("RESPONSE: ".var_export($rawResponse, true));
    ...
    private function loadDocCache($forceReload = false) {
        ...
        $sql = "
SELECT DISTINCT doc_id AS id, doc_url AS url, filename AS name, date_modified AS date_modified, doc_direct_url AS direct_url FROM notes WHERE doc_type = 'LotusLiveDirect'
UNION ALL
SELECT DISTINCT doc_id AS id, doc_url AS url, filename AS name, date_modified AS date_modified, doc_direct_url AS direct_url FROM document_revisions WHERE doc_type = 'LotusLiveDirect' ORDER BY date_modified DESC";
