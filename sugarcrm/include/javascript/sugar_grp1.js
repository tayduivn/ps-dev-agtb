...
function add_error_style(formname,input,txt){try{inputHandle=typeof input=="object"?input:document.forms[formname][input];style=get_current_bgcolor(inputHandle);if(txt.substring(txt.length-1)==':')
...
throw(expr+": Syntax Error");var func=expr.substring(0,open_paren_loc);var params=expr.substring(open_paren_loc+1,expr.length-1);var level=0;var length=params.length;var argument="";var args=new Array();var currChar=null;var lastCharRead=null;var justReadString=false;var isInQuotes=false;var isPrevCharBK=false;for(var i=0;i<=length;i++){lastCharRead=currChar;if(i==length){argument=YAHOO.lang.trim(argument);if(argument!="")
...
throw("Syntax Error: Unexpected ','");args[args.length]=this.tokenize(argument);argument="";continue;}
argument+=currChar;}
if(isInQuotes)throw("Syntax Error (Unterminated String Literal)");if(level!=0)throw("Syntax Error (Incorrectly Matched Parantheses)");return{type:"function",name:YAHOO.lang.trim(func),args:args}}
...
{return type;}}
return false;};SUGAR.expressions.ExpressionParser.prototype.evaluate=function(expr)
...
return fixed;if((/^[\w\-]+\(.*\)$/).exec(expr)==null){throw("Syntax Error (Expression Format Incorrect '"+expr+"' )");debugger;}
...
throw(func+": No such function defined");var params=expr.substring(open_paren_loc+1,expr.length-1);var level=0;var length=params.length;var argument="";var args=new Array();var currChar=null;var lastCharRead=null;var justReadString=false;var isInQuotes=false;var isPrevCharBK=false;for(var i=0;i<=length;i++){lastCharRead=currChar;if(i==length){args[args.length]=this.evaluate(argument);break;}
isPrevCharBK=(lastCharRead=='\\');currChar=params.charAt(i);if(isInQuotes&&currChar!='"'&&!isPrevCharBK){argument+=currChar;continue;}
if(currChar=='"'&&!isPrevCharBK&&level==0)
{if(isInQuotes){var end_reg=params.indexOf(",",i);if(end_reg<0)end_reg=params.length-1;var start_reg=(i<length-1?i+1:length-1);var temp=params.substring(start_reg,end_reg);if((/^\s*$/).exec(temp)==null)
throw(func+": Syntax Error (Improperly Terminated String '"+temp+"')"+(start_reg)+" "+end_reg);}
isInQuotes=!isInQuotes;}
if(currChar=='('){level++;}else if(currChar==')'){level--;}
else if(currChar==','&&level==0){args[args.length]=this.evaluate(argument);argument="";continue;}
argument+=currChar;}
...
return null;}
SUGAR.util.DateUtils={convert:function(date,newFormat,oldFormat){if(oldFormat==null||oldFormat==""){oldFormat=SUGAR.util.DateUtils.guessFormat(date);}
if(oldFormat==false){return false;}
var jsDate=new Date("Jan 1, 1970 00:00:00");var part="";var dateRemain=date;oldFormat=oldFormat+" ";for(var c in oldFormat){c=oldFormat[c];if(c==':'||c=='/'||c=='-'||c=='.'||c==" "||c=='a'||c=="A"){var i=dateRemain.indexOf(c);if(i==-1)i=dateRemain.length;var v=dateRemain.substring(0,i);dateRemain=dateRemain.substring(i+1);switch(part){case'm':jsDate.setMonth(v-1);break;case'd':jsDate.setDate(v);break;case'Y':jsDate.setYear(v);break;case'h':var timeformat=oldFormat.substring(oldFormat.length-5);if(timeformat.toLowerCase=="i a "||timeformat.toLowerCase==c+"ia "){if(dateRemain.substring(dateRemain.length-3).toLowerCase=='pm'){v=v*1;if(v<12){v+=12;}}}
case'H':jsDate.setHours(v);break;case'i':v=v.substring(0,2);jsDate.setMinutes(v);break;}
part="";}else{part=c;}}
return jsDate.toString();},guessFormat:function(date){if(typeof date!="string")
...
return dateFormat;}}// End of File include/Expressions/javascript/expressions.js
...
SUGAR.forms.AssignmentHandler.ANIMATE=true;SUGAR.forms.AssignmentHandler.VARIABLE_MAP={};SUGAR.forms.AssignmentHandler.LOCKS={};SUGAR.forms.AssignmentHandler.register=function(variable){if(variable instanceof Array){for(var i=0;i<variable.length;i++){SUGAR.forms.AssignmentHandler.VARIABLE_MAP[variable[i]]=document.getElementById(variable[i]);}}else{SUGAR.forms.AssignmentHandler.VARIABLE_MAP[variable]=document.getElementById(variable);}}
SUGAR.forms.AssignmentHandler.registerFields=function(flds){if(typeof(flds)!='object')return;var form=document.forms[flds.form];var names=flds.fields;if(typeof(form)=='undefined')return;for(var i=0;i<names.length;i++){var el=form[names[i]];if(el!=null)SUGAR.forms.AssignmentHandler.VARIABLE_MAP[el.id]=el;}}
SUGAR.forms.AssignmentHandler.registerForm=function(f){var form=document.forms[f];if(typeof(form)=='undefined')return;for(var i=0;i<form.length;i++){var el=form[i];if(el!=null&&el.value!=null&&el.id!=null&&el.id!="")
SUGAR.forms.AssignmentHandler.VARIABLE_MAP[el.id]=el;}}
SUGAR.forms.AssignmentHandler.registerView=function(view,startEl){var Dom=YAHOO.util.Dom;if(Dom.get(view)!=null&&Dom.get(view).tagName=="FORM"){return SUGAR.forms.AssignmentHandler.registerForm(view);}
var nodes=YAHOO.util.Selector.query("."+view+".view [id]",startEl);for(var i in nodes){if(nodes[i].id!="")
...
SUGAR.forms.AssignmentHandler.getValue=function(variable){var field=SUGAR.forms.AssignmentHandler.getElement(variable);if(field==null||field.tagName==null)return null;if(field.tagName.toLowerCase()=="select"){if(field.selectedIndex==-1){return null;}else{return field.options[field.selectedIndex].value;}}
...
return field.value;return field.innerHTML;}
SUGAR.forms.AssignmentHandler.getElement=function(variable){var field=SUGAR.forms.AssignmentHandler.VARIABLE_MAP[variable];if(field==null)
...
return null;if(SUGAR.forms.AssignmentHandler.LOCKS[variable]!=null){throw("Circular Reference Detected");}
...
SUGAR.forms.FlashField(field);SUGAR.forms.AssignmentHandler.LOCKS[variable]=true;var listeners=YAHOO.util.Event.getListeners(field,'change');if(listeners!=null){for(var i=0;i<listeners.length;i++){var l=listeners[i];l.fn(null,l.obj);}}
SUGAR.forms.AssignmentHandler.LOCKS[variable]=null;}
SUGAR.forms.AssignmentHandler.showError=function(variable,error)
{var field=SUGAR.forms.AssignmentHandler.getElement(variable);if(field==null)
return null;add_error_style(field.form.name,field,error);}
...
SUGAR.forms.DefaultExpressionParser=new SUGAR.expressions.ExpressionParser();SUGAR.forms.evalVariableExpression=function(expression,varmap)
{expression=SUGAR.forms._performRangeReplace(expression);this.replaceAll=function(haystack,needle,rpl){var str=haystack;while(str.indexOf(needle)>-1){str=str.replace(needle,rpl);}
return str;}
var handler=SUGAR.forms.AssignmentHandler;if(typeof(varmap)=='undefined')
{varmap=new Array();for(v in handler.VARIABLE_MAP){if(v!=""){varmap[varmap.length]=v;}}}
if(expression==SUGAR.expressions.Expression.TRUE||expression==SUGAR.expressions.Expression.FALSE)
return SUGAR.forms.DefaultExpressionParser.evaluate(expression);for(var i=0;i<varmap.length;i++){var v=varmap[i];var value=handler.getValue(v);if(value==null)
continue;value=value.replace(/\n/g,"");var regex=new RegExp("\\$"+v,"g");if((/^(\s*)$/).exec(value)!=null||value===""){expression=expression.replace(regex,'""');}
else if((/^(\-)?[0-9,]+(\.[0-9]+)?$/).exec(value)!=null){value=this.replaceAll(value,",","");expression=expression.replace(regex,value);}
else{expression=expression.replace(regex,'"'+value+'"');}}
...
{var trigger=new SUGAR.forms.Trigger(variables,condition);if(triggeronload){YAHOO.util.Event.addListener(window,"load",SUGAR.forms.Trigger.trigger,trigger);}
return trigger;}
SUGAR.forms.Dependency=function(trigger,actions,falseActions,testOnLoad)
{this.actions=actions;this.falseActions=falseActions;trigger.setDependency(this);this.trigger=trigger;if(testOnLoad){SUGAR.forms.Trigger.fire("",trigger);}}
SUGAR.forms.Dependency.prototype.fire=function(undo)
{try{var actions=this.actions;if(undo&&this.falseActions!=null)
actions=this.falseActions;if(actions instanceof SUGAR.forms.AbstractAction){actions.exec();}else{for(var i in actions){var action=actions[i];if(typeof action.exec=="function")
action.exec();}}}catch(e){if(!SUGAR.isIE&&console&&console.log){console.log('ERROR: '+e);}
...
SUGAR.forms.Trigger.prototype._attachListeners=function(){var handler=SUGAR.forms.AssignmentHandler;if(!(this.variables instanceof Array)){var el=handler.getElement(this.variables);if(!el)return;if(el.type&&el.type.toUpperCase()=="CHECKBOX")
{YAHOO.util.Event.addListener(el,"click",SUGAR.forms.Trigger.fire,this);}else{YAHOO.util.Event.addListener(el,"change",SUGAR.forms.Trigger.fire,this);}
return;}
...
{var eval;var val;try{eval=SUGAR.forms.evalVariableExpression(obj.condition);}catch(e){if(!SUGAR.isIE&&console&&console.log){console.log('ERROR:'+e+"; in Condition: "+obj.condition);}}
if(typeof(eval)!='undefined')
val=eval.evaluate();if(val==SUGAR.expressions.Expression.TRUE){if(obj.dependency instanceof SUGAR.forms.Dependency){obj.dependency.fire(false);return;}}else if(val==SUGAR.expressions.Expression.FALSE){if(obj.dependency instanceof SUGAR.forms.Dependency){obj.dependency.fire(true);return;}}}
