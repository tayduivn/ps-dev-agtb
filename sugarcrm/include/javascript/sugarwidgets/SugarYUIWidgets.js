	...
	_addTrEl : function (oRecord) {
		...
		if (!this.disableEmptyRows || (
		  ...
		  && oRecord.getData()[this.getColumnSet().keys[0].key] != "")
            ...
            //allow drag drop to be turned off for explicitly marked rows
            _rowDD.subscribe('b4MouseDownEvent', function(e) {
                if (e.currentTarget.className.indexOf('sw-no-drag-drop') != -1) {
                    return false;
                }
            });
		}
