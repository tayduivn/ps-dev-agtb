 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: ja.lang.php 51719 2009-10-22 17:18:00Z mitani $
 * Description:
...
$mod_strings = array (
   ...
   'ERR_DB_OCI8_CONNECT' => '入力されたデータベースのユーザ名もしくはパスワードが不正なため、データベースへの接続が確立できませんでした。正しいユーザ名とパスワードを入力してください。' ,
   'ERR_DB_OCI8_VERSION' => 'このOracleのバージョンはSugarCRMがサポートしていません。SugarCRMと互換性のあるバージョンをインストールしてください。' ,
   'LBL_DBCONFIG_MSG1' => 'データベース名を入力してください。これは、ユーザにアサインされるデフォルトのテーブルスペースとなります。' ,
   
   'LBL_Q' => '商談クエリ' ,
   'LBL_Q1_DESC' => 'タイプごとの商談' ,
   'LBL_Q2_DESC' => '取引先ごとの商談' ,
   'LBL_R1' => '6ヶ月の売上パイプラインレポート' ,
   'LBL_R1_DESC' => '月・タイプごとに整理された6ヶ月以上先の商談' ,
   'LBL_OPP' => '商談データセット' ,
   ...
   'ERR_CHECKSYS_MBSTRING' => 'マルチバイトストリングをサポートするエクステンション（mbstring）が見つかりません。エクステンションがインストールされていないか、PHPがコンパイルされる際にmbstringモジュ－ルが有効になっていない可能性があります。mbstringを有効にする方法についてはPHPのマニュアルを参照ください。' ,
   ...
   'LBL_MSSQL_FTS' => '全文検索' ,
   'LBL_MSSQL_FTS_INSTALLED' => 'インストール済み' ,
   'LBL_MSSQL_FTS_INSTALLED_ERR1' => '全文検索機能はインストールされていません。' ,
   'LBL_MSSQL_FTS_INSTALLED_ERR2' => 'このままインストールは続けられますが、全文検索機能を持つSQL Serverを再インストールしない限り、全文検索機能を使うことはできません。SQLサーバのインストールガイドを参照するか、管理者に連絡をしてください。' ,
   ...
   'LBL_INSTALL_TYPE_MSG3' => 'インスト－ルのためのその他のオプションを入力してください。ほとんどの設定はインストール後に管理メニューから指定することも可能です。インストール経験のあるユーザ向けです。' ,
   ...
   'LBL_PERFORM_TABLES' => 'SugarCRMアプリケーションのテ－ブル、監査テーブル、関連メタテーブルを作成しています。' ,
   ...
   //BEGIN SUGARCRM flav=pro ONLY
   'LBL_WELCOME_2' => 'インストールのドキュメントについては<a href="http://docs.sugarforum.jp/" target="_blank">SugarCRMドキュメント</a>を参照ください。<BR><BR> テクニカルサポートが必要な場合はケアブレインズまでお問い合わせください。' ,
   //END SUGARCRM flav=pro ONLY
...
システムチェックのエラーについてはインスト－ルガイドを参照ください。<br>' ,
