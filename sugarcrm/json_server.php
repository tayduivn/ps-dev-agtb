	...
	foreach($query_obj['conditions'] as $condition) {
		...
		if ($condition['name']=='email1' or $condition['name']=='email2') {
			...
			if($condition['op'] == 'contains') {
				$cond_arr[] = $GLOBALS['db']->getValidDBName($table.$condition['name'])." like '%".$GLOBALS['db']->quote($condition['value'])."%'";
			}
			if($condition['op'] == 'like_custom') {
				...
				$cond_arr[] = $GLOBALS['db']->getValidDBName($table.$condition['name'])." like '$like'";
			} else { // starts_with
				$cond_arr[] = $GLOBALS['db']->getValidDBName($table.$condition['name'])." like '".$GLOBALS['db']->quote($condition['value'])."%'";
			}
		}
