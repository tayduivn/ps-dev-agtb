...
foreach ($modules as $the_module => $fields) {
	...
	if ($db->dbType == 'oci8') {
		...
		$result = $db->query('select column_name as field, data_type as type, nullable, data_precision, data_scale, data_length from user_tab_columns where table_name='."'".strtoupper($mod->table_name)."_CSTM' ");
