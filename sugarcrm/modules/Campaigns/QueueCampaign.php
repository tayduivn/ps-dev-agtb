...
//BEGIN SUGARCRM flav=ent ONLY
		if ($test) {
			$send_date_time="TO_DATE('".gmdate($GLOBALS['timedate']->get_db_date_time_format(), mktime() - 60)."','YYYY-MM-DD HH24:MI:SS')";
		} else {
			$send_date_time= "TO_DATE('".$timedate->to_db_date($mergedvalue)  . ' ' . $timedate->to_db_time($mergedvalue)."','YYYY-MM-DD HH24:MI:SS')";
		}
//END SUGARCRM flav=ent ONLY
	} else {
		if ($test) {
