...
if(is_admin($current_user) && $_REQUEST['module'] != 'DynamicLayout' && !empty($_SESSION['editinplace'])){  
    ...
    $ss->assign("ADMIN_EDIT","<a href='index.php?action=index&module=DynamicLayout&from_action=".$_REQUEST['action'] ."&from_module=".$_REQUEST['module'] ."&record=".$record. "'>".SugarThemeRegistry::current()->getImage/*ALTFIXED*/("EditLayout","border='0' align='bottom'",null,null,'.gif',$mod_strings['LBL_EDIT_LAYOUT'])."</a>");    
...
//create the html to create tracker table
    foreach($trkr_lists as $trkr_id){
      ...
      if(isset($ct_focus->tracker_name) && !empty($ct_focus->tracker_name)){
            ...
            $trkr_html .= SugarThemeRegistry::current()->getImage/*ALTFIXED*/('delete_inline', "border='0' alt='rem' align='absmiddle'", 12, 12, ".gif", $mod_strings['LBL_DELETE_INLINE'])."</a></td></tr></table></div>";
 ...
 //this is not a newlsetter campaign, so fill in target list table
    ...
    if(count($prospect_lists)>0){
        ...
        //retrieve prospect list
             ...
             $trgt_html .= SugarThemeRegistry::current()->getImage/*ALTFIXED*/('delete_inline', "border='0' alt='rem' align='absmiddle'", 12, 12, ".gif", $mod_string['LBL_DELETE_INLINE'])."</a></td></tr></table></div>";
