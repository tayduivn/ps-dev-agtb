...
/**
 * The SugarBean for Each commentslog message, should be immutable.
 */
class Commentslog extends Basic
{
    public $module_dir = 'Commentslog';
    public $object_name = 'Commentslog';
    public $table_name = 'commentslog';
     ...
     * @var string
     */
    protected $joinTable = 'commentslog_rel';
     ...
     * @var string
     */
    protected $joinKey = 'commentslog_id';
    ...
    /**
     * Sets the entry of this commentslog message. Shall only be called while creating
     * new commentslog message, not for editing
     * @param string $entry The entry of this commentslog message
     * @modifies $this->entry
    ...
    /**
     * Sets the module of this commentslog message. Shall only be called while creating
     * new commentslog message, not for editing
     * @param string $module The module this commentslog is associated to,
    ...
    /**
     * Gets all the commentslog for every record id given
     *
     * @param $focus
    ...
    public function getRelatedModuleRecords($focus, $ids)
    {
        ...
        // Not use this in Commentslog module, use only for other modules
        if (empty($ids) || ($focus == null) || ($focus->table_name === 'commentslog')) {
            return array();
        }

        $query = new SugarQuery($this->db);
        $query->from($focus);
        $query->join('commentslog_link');
        $query->select()->fieldRaw('commentslog_id');
        $query->where()->in('record_id', $ids);
        $results = $query->execute();

        $returnArray = array();
        foreach ($results as $result) {
            $returnArray[] = $result['commentslog_id'];
        }
    ...
    /**
     * Retrieves the record id and module of the commentslog
