...
class Document extends SugarBean {
	...
	function fill_in_additional_detail_fields() {
		...
		if($this->ACLAccess('DetailView')){
			$file_url = "<a href='index.php?entryPoint=download&id=".basename(UploadFile :: get_url($this->filename, $this->document_revision_id))."&type=Documents' target='_blank'>".SugarThemeRegistry::current()->getImage/*ALTFIXED*/($img_name, 'border="0"', null,null,'.gif',$mod_strings['LBL_LIST_VIEW_DOCUMENT'])."</a>";


			if(!empty($this->doc_type) && $this->doc_type != 'Sugar' && !empty($this->doc_url))
                $file_url= "<a href='".$this->doc_url."' target='_blank'>".SugarThemeRegistry::current()->getImage/*ALTFIXED*/($this->doc_type.'_image_inline', 'border="0"',null,null,'.png',$mod_strings['LBL_LIST_VIEW_DOCUMENT'])."</a>";
