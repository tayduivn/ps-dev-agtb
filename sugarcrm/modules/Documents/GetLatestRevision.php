...
if (!empty($_REQUEST['record'])) {
	...
	if (!empty($document->document_revision_id) && !empty($_REQUEST['get_latest_for_id']))  {
		$query="update linked_documents set document_revision_id='{$document->document_revision_id}', date_modified='".TimeDate2::getInstance()->nowDb()."' where id ='{$_REQUEST['get_latest_for_id']}' ";
