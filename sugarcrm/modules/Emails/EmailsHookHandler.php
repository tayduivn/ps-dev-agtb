...
{
    /**
     * Anytime an attachment is unlinked from an Email, the attachment Note bean needs to be deleted and the
     * referenced file needs to be deleted from the uploads directory
     *
     * @param SugarBean $bean The email.
     * @param string $event
     * @param array $args
     */
    public function removeEmailAttachment(SugarBean $bean, $event, array $args)
    {
        ...
        if ($event !== 'after_relationship_delete') {
            $message = '%s is an invalid event. %s is only concerned with after_relationship_delete events.';
        ...
        if ($attachment) {
            $attachment->mark_deleted($attachment->id);
        } else {
            $message = 'Failed to delete the attachment Notes/%s for Emails/%s in %s.';
