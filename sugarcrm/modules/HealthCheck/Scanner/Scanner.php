...
class HealthCheckScanner
{
    ...
    public function scan()
    {
        ...
        if (version_compare($sugar_version, '7.9.0.0', '<')) {
            ...
            $this->scanCustomPhpFiles(array(
                function ($file, $contents) use (&$calls) {
                    $this->findPhp4ConstructorCalls($file, $contents, $calls);
                },
                array($this, 'findUnsupportedApiUsages'),
            ));
            file_put_contents('stock-parent-calls.json', json_encode($calls));
        }
    ...
    protected function checkEmailHelpCustomizationsForNewFilenameConflicts()
    {
        ...
        if (version_compare($version, '7.10.0.0', '<')) {
            $customFiles = array(
                ...
                'custom/modules/Emails/clients/base/api/help/emails_record_get_help.html',
                ...
                'custom/modules/Emails/clients/base/api/help/mail_recipients_lookup_post_help.html',
                'custom/modules/OutboundEmailConfiguration/clients/base/api/help/outbound_email_configuration_list_get_help.html',
    ...
    protected function checkSingleLanguageFile($file)
    {
        ...
        if (method_exists('Opportunity', 'getSettings')) {
    ...
    private function findPhp4ConstructorCalls($file, $contents, &$calls)
    {
        preg_match_all('/class\s+([^\s])+\s+extends\s+([^\s]+)/im', $contents, $classMatches);

        foreach ($classMatches[0] as $i => $_) {
            $class = $classMatches[1][$i];
            $parent = $classMatches[2][$i];
            if (preg_match(
                '/(parent|' . preg_quote($parent) . ')\s*::\s*' . preg_quote($parent) . '\s*\(/im',
                $contents,
                $callMatches
            )) {
                if ($this->isRemovedStockPhp4Constructor($parent)
                    // autogenerated module builder class
                    || preg_match('/_sugar$/', $parent)
                    // autogenerated module builder class for developers
                    || strcasecmp($parent, $class . '_sugar') == 0
                ) {
                    $this->updateStatus('stockPhp4ConstructorCall', $file, $callMatches[1], $parent);
                    $calls[$file][] = array($callMatches[1], $parent);
                } else {
                    $this->updateStatus('customPhp4ConstructorCall', $file, $callMatches[1], $parent);
                }
            }
        }
    }
...
ENDP;
    ...
    /**
     * Checks if the class is a stock SugarCRM class
     *
     * @param string $class Class name
     * @return bool
     */
    protected function isRemovedStockPhp4Constructor($class)
    {
        static $classes;

        if ($classes === null) {
            $classes = array_flip(require __DIR__ . '/removed-php4-constructors.php');
        }
