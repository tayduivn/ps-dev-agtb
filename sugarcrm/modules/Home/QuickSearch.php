...
class quicksearchQuery
{
     ...
     * @return	string
     */
    protected function constructWhere($query_obj, $focus)
    {
    ...
    public function get_contact_array($args)
    {
        ...
        for($i = 0; $i < count($list_return); $i++) {
            ...
            foreach($args['field_list'] as $field) {
                ...
                if(preg_match('/reports_to_id$/s',$field) || preg_match('/contact_id$/s',$field)) {
                    // We are overriding the reports_to_id param to change to 'id'
