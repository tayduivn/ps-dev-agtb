 	...
 	public function display()
 	{
        ...
        if(function_exists($adFunction)) { // does the additional details function exist
		    ...
		    if(!empty($results['editLink']))
		    	$retArray['caption'] .= "<a title='".$GLOBALS['app_strings']['LBL_EDIT_BUTTON']."' href='".$results['editLink']."'><img border=0 src='".SugarThemeRegistry::current()->getImageURL('edit_inline.gif',false)."'></a>";
		    if(!empty($results['viewLink']))
		    	$retArray['caption'] .= "<a title='".$GLOBALS['app_strings']['LBL_VIEW_BUTTON']."' href='".$results['viewLink']."'><img border=0 src='".SugarThemeRegistry::current()->getImageURL('view_inline.gif',false)."' style='margin-left:2px;'></a>";
            }
            $retArray['caption'] .= ""; 
