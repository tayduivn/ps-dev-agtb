       ...
       //check if status was draft or in review
	    ...
	    if($KBDocument->status_id == 'Published'){
	    	...
	    	if(empty($KBDocument->active_date) || $KBDocument->active_date==null){	    		
	            $KBDocument->active_date = $timedate->to_display_date(gmdate($GLOBALS['timedate']->dbDayFormat), false);
	    	}
	...
	$KBDocumentKBTag->db->query($q); 
	  ...
	  if($KBDocument != null){	
		...
		if($KBDocument->status_id == 'Published'){
	    	...
	    	if(empty($KBDocument->active_date) || $KBDocument->active_date==null){	    		
	            $KBDocument->active_date = $timedate->to_display_date(date($GLOBALS['timedate']->dbDayFormat), false);
