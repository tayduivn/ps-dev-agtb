 ...
 */
/*********************************************************************************
 * $Id: KBOLDDocumentRevision.php 20511 2007-02-28 00:18:41 +0000 (Wed, 28 Feb 2007) vineet $
...
class KBOLDDocumentRevision extends SugarBean {
	...
	var $table_name = "kbolddocument_revisions";	
	var $object_name = "KBOLDDocumentRevision";
	var $module_dir = 'KBOLDDocumentRevisions';
	var $new_schema = true;
...
*/
     ...
     * @deprecated
     */
    public function KBOLDDocumentRevision()
    {
        self::__construct();
    }
	...
	public function __construct() {
		parent::__construct();
		$this->setupCustomFields('KBOLDDocumentRevisions');  //parameter is module name
	...
	{/*
		...
		$query = "SELECT kbolddocument_name, revision, kbolddocument_revision_id FROM kbolddocuments, document_revisions where kbolddocuments.id = '$this->document_id' AND document_revisions.id = documents.document_revision_id";
	...
	function getDocumentRevisionNameForDisplay($revId='') {
		...
		$localLabels = return_module_language($current_language, 'KBOLDDocumentRevisions');
	...
	function fill_document_name_revision($doc_id) {
		...
		//find the document name and current version.
		$query = "SELECT kbolddocuments.kbolddocument_name, revision FROM kbolddocuments, kbolddocument_revisions where kbolddocuments.id = '$doc_id'";
		$query .= " AND kbolddocuments.id = kbolddocument_revisions.kbolddocument_id";
		$result = $this->db->query($query,true,"Error fetching document details...:");
		$row = $this->db->fetchByAssoc($result);
		if ($row != null) {
			$this->name = $row['kbolddocument_name'];
			$this->latest_revision = $row['revision'];	
		}	
	}
	...
	function get_document_revisions($doc_id){
		...
		$db = DBManagerFactory::getInstance();				
		$query="select id, revision from kbolddocument_revisions where kbolddocument_id='$doc_id' and deleted=0";
		$result=$db->query($query);
		if (!empty($result)) {
	...
	function get_documents($kbdocrev_id){
		...
		$db = DBManagerFactory::getInstance();				
		$query="select id from kbolddocuments where kbolddocument_revision_id='$kbdocrev_id' and deleted=0";
		$result=$db->query($query);
		if (!empty($result)) {
			while (($row=$db->fetchByAssoc($result)) != null) {
	...
	function get_docrevs($kbdocrev_id){
		...
		$query = 'select document_revisions.id from document_revisions, kbolddocument_revisions'; 
		$query .= ' where document_revisions.id=kbolddocument_revisions.document_revision_id AND document_revisions.id = ' . $db->quoted($kbdocrev_id);
		$query .= ' and kbolddocument_revisions.deleted = 0 and document_revisions.deleted = 0';
		$result=$db->query($query);
