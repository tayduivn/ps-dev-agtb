...
foreach($selectedArticles as $articleId){
	if(!empty($articleId)){		
        ...
        //retrieve article  
        $kbarticle = BeanFactory::getBean('KBOLDDocuments', $articleId);        	        		       
        $deleted=1; 
        //also retrieve children and check if there is any article linked                                                 
        $kbarticle->deleted = 1;
        $kbarticle->save();
        //also delete related 
        $kbdocrevs = KBOLDDocument::get_kbolddocument_revisions($articleId);
		 ...
		 foreach($kbdocrevs as $key=>$thiskbid) {	
	        ...
	        $docrev_ids = KBOLDDocumentRevision::get_docrevs($thiskbid);
		...
		$q = 'UPDATE kbolddocuments_kboldtags SET deleted = '.$deleted.' WHERE kbolddocument_id = \''.$articleId.'\'';    
		$GLOBALS['db']->query($q);
	}
}	
