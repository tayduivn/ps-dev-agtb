...
if (isset($_REQUEST['offset']) or isset($_REQUEST['record'])) {
	$result = $detailView->processSugarBean("KBOLDDOCUMENT", $focus, $offset);
	if($result == null) {
...
$params = array();
$params[] = $focus->kbolddocument_name;
...
$xtpl=new XTemplate ('modules/KBOLDDocuments/DetailView.html');
...
$xtpl->assign("DOCUMENT_NAME", $focus->kbolddocument_name);
...
$xtpl->assign("KBDOC_TAGS",KBOLDDocument::get_kbdoc_tags_heirarchy($focus->id,"Detail"));
//get the document body
$article_body = KBOLDDocument::get_kbdoc_body_without_incrementing_count($focus->id);
$xtpl->assign("KBDOC_BODY",from_html($article_body));
$xtpl->assign("KBDOC_ATTS",KBOLDDocument::get_kbdoc_attachments($focus->id,"Detail"));

if(isset($focus->status_id) && isset($app_list_strings['kbolddocument_status_dom']) && isset($app_list_strings['kbolddocument_status_dom'][$focus->status_id])){
	$xtpl->assign("STATUS", $app_list_strings['kbolddocument_status_dom'][$focus->status_id]);
 }
...
$buttons = array(
    <<<EOD
            <input title="{$app_strings['LBL_EDIT_BUTTON_TITLE']}" id="edit_button" accessKey="{$app_strings['LBL_EDIT_BUTTON_KEY']}" class="button primary" onclick="this.form.return_module.value='KBOLDDocuments'; this.form.return_action.value='DetailView'; this.form.return_id.value='{$focus->id}'; this.form.action.value='EditView'" type="submit" name="Edit" value="{$app_strings['LBL_EDIT_BUTTON_LABEL']}">
EOD
,
    <<<EOD
            <input title="{$app_strings['LBL_DUPLICATE_BUTTON_TITLE']}" id="duplicate_button" accessKey="{$app_strings['LBL_DUPLICATE_BUTTON_KEY']}" class="button" onclick="this.form.return_module.value='KBOLDDocuments'; this.form.return_action.value='index'; this.form.isDuplicate.value=true; this.form.action.value='EditView'" type="submit" name="Duplicate" value="{$app_strings['LBL_DUPLICATE_BUTTON_LABEL']}">
EOD
,
    <<<EOD
            <input title="{$app_strings['LBL_DELETE_BUTTON_TITLE']}" id="delete_button" accessKey="{$app_strings['LBL_DELETE_BUTTON_KEY']}" class="button" onclick="this.form.return_module.value='KBOLDDocuments'; this.form.return_action.value='SearchHome'; this.form.action.value='Delete'; return confirm('{$app_strings['NTC_DELETE_CONFIRMATION']}')" type="submit" name="Delete" value="{$app_strings['LBL_DELETE_BUTTON_LABEL']}">
EOD
,
    <<<EOD
...
if (!empty($focus->template_type)) {
	$xtpl->assign("TEMPLATE_TYPE", $app_list_strings['kbolddocument_template_type_dom'][$focus->template_type]);
}
