...
$xtpl=new XTemplate ('modules/KBOLDDocuments/EditView.html');
...
if(empty($focus->id)){
	...
	$params[] = "<a href='index.php?module=KBOLDDocuments&action=DetailView&record={$focus->id}'>{$focus->kbolddocument_name}</a>";
	$params[] = $GLOBALS['app_strings']['LBL_EDIT_BUTTON_LABEL'];
}
echo getClassicModuleTitle("KBOLDDocuments", $params, true);

$GLOBALS['log']->info("KBOLDDocument detail view");
...
/* get document body
if(from_html(KBOLDDocument::get_kbdoc_body_without_incrementing_count($focus->id)) != null) {
	$xtpl->assign("KBDOC_BODY", from_html(KBOLDDocument::get_kbdoc_body_without_incrementing_count($focus->id)));
   }
else {
	$xtpl->assign("KBDOC_BODY", "");
  }
*/
//for attachments and kboldtags
$kbId = '';
if(!empty($focus->id)) {
	    $kbId = $focus->id;
 }
...
if(isset($_REQUEST['isDuplicate']) && $_REQUEST['isDuplicate'] == 'true')
{
	...
	//get document tags
	if(KBOLDDocument::get_kbdoc_tags_heirarchy($kbId,"Edit") != null){
	  $xtpl->assign("KBOLDTAG_TITLE",$mod_strings['LBL_KBDOC_TAGS']);
	  $xtpl->assign("KBDOC_TAGS",KBOLDDocument::get_kbdoc_tags_heirarchy($kbId,"Edit"));
	}
	else{
		$xtpl->assign("KBOLDTAG_TITLE","");
		$xtpl->assign("KBDOC_TAGS","");
	}
	//get document attachments
	if(KBOLDDocument::get_kbdoc_attachments($kbId,'Edit') != null){
		$xtpl->assign("KBDOC_ATTS_TITLE",$mod_strings['LBL_KBDOC_ATTS_TITLE']);
		$xtpl->assign("KBDOC_ATTS",KBOLDDocument::get_kbdoc_attachments($kbId,'Edit'));
	}
	else{
...
$xtpl->assign("KBOLDDOCUMENT_NAME",$focus->kbolddocument_name);
$xtpl->assign("DESCRIPTION",$focus->description);
$xtpl->assign("FILENAME_TEXT",$focus->filename);
//$xtpl->assign("REVISION",$focus->latest_revision);

$xtpl->assign("REVISION",$focus->kbolddocument_revision_number);
//$xtpl->assign("OLD_ID",$old_id);
...
if (isset($focus->status_id)){
	$xtpl->assign("STATUS_OPTIONS", get_select_options_with_id($app_list_strings['kbolddocument_status_dom'], $focus->status_id));

}
else if (isset($_REQUEST['status_id'])) {
    $xtpl->assign("STATUS_OPTIONS", get_select_options_with_id($app_list_strings['kbolddocument_status_dom'], $_REQUEST['status_id']));
}
else{
	$xtpl->assign("STATUS_OPTIONS", get_select_options_with_id($app_list_strings['kbolddocument_status_dom'], ''));
}
...
if(isset($from_email) && !empty($from_email)){
    $xtpl->assign("KBOLDDOCUMENT_NAME",$from_email->name);
...
if (empty($focus->id) && !empty($from_case)) {
	$xtpl->assign("KBOLDDOCUMENT_NAME",$from_case->name);
	$xtpl->assign("REVISION",1);
	$revision = 1;
}
...
$popup_request_data = array(
	'call_back_function' => 'kbolddocument_set_return',
	'form_name' => 'EditView',
	'field_to_name_array' => array(
		'id' => 'related_doc_id',
...
$json = getJSONobj();
$xtpl->assign('encoded_kbolddocument_popup_request_data', $json->encode($popup_request_data));
...
$edValue = '';
$kbDocBody = from_html(KBOLDDocument::get_kbdoc_body_without_incrementing_count($kbId));
if (!empty($kbDocBody)) {
    $edValue = $kbDocBody;
...
/////////////////////////
 ...
 echo <<<EOQ
	  ...
	  function insert_variable_html_link(text) {
	  	var thelink="<a href='" + text + "''" + ">'" + SUGAR.language.get('KBOLDDocuments', 'LBL_DEFAULT_LINK_TEXT') + "'</a>";
...
$javascript->addFieldGeneric('kbolddocument_name', '','LBL_ARTICLE_TITLE','true');
...
$javascript->addFieldGeneric('kbolddocument_revision_number', '', 'LBL_DOC_VERSION' ,'true');
...
$json = getJSONobj();
$savedSearchSelects = $json->encode(array($GLOBALS['app_strings']['LBL_SAVED_SEARCH_SHORTCUT'] . '<br>' . $savedSearch->getSelect('KBOLDDocuments')));
$str = "<script>
