...
/**
 * This class is here to add in the file information to the KBOLDDocuments so that it can be easily displayed by remote consumers of the API. Otherwise you have to traverse a number of links to pull up this information.
 */
class KBOLDDocumentsApiHelper extends SugarBeanApiHelper
{
    ...
    public function formatForApi(SugarBean $bean, array $fieldList = array(), array $options = array() )
    {
        ...
        if ( empty($fieldList) || in_array('attachment_list',$fieldList) ) {
            ...
            $query = "SELECT rev.id rev_id, rev.filename filename, kbrev.id docrev_id FROM kbolddocument_revisions kbrev LEFT JOIN document_revisions rev ON (kbrev.document_revision_id = rev.id) WHERE kbrev.kbolddocument_id = '".$bean->id."' AND kbrev.deleted = 0 AND rev.deleted = 0 AND kbrev.kboldcontent_id is NULL";
            ...
            while ( $row = $db->fetchByAssoc($ret) ) {
                ...
                $thisFile['kbolddocument_revision_id'] = $row['docrev_id'];
