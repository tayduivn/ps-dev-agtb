...
require_once('modules/KBOLDDocuments/TreeData.php');
...
class Popup_Picker
{
    ...
    function _get_where_clause()
    {
        ...
        {
            $where_clauses = array();
            append_where_clause($where_clauses, "kbolddocument_name", "kbolddocuments.kbolddocument_name");
    ...
    function process_page()
    {
        ...
        if (isset($_REQUEST['caller']) && $_REQUEST['caller']=='KBOLDDocuments') {
            $b_from_documents=true;
        }
            
        //initalize template
        $form = new XTemplate('modules/KBOLDDocuments/Popup_picker.html');
        ...
        $doctree->set_param('module','KBOLDDocuments');
        if ($b_from_documents) {
            $doctree->set_param('caller','KBOLDDocuments');
        ...
        $document_name = empty($_REQUEST['kbolddocument_name']) ? '' : $_REQUEST['kbolddocument_name'];
        ...
        $form->assign('KBOLDDOCUMENT_NAME', $document_name);
        ...
        $ListView->setQuery($where, '', 'kbolddocument_name', 'KBOLDDOCUMENT');
        $ListView->setModStrings($mod_strings);

        ob_start();
        $ListView->processListView($seed_bean, 'main', 'KBOLDDOCUMENT');
        $output_html .= ob_get_contents();
        ob_end_clean();
