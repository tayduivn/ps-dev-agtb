...
{*  --export and pagination-- *}
	{include file='modules/KBOLDDocuments/tpls/ListViewPagination.tpl'}
{*  --Header Row -- *}
	<tr height='20'>
		{if $prerow}
				...
				<td scope='row' align='{$params.align|default:'left'}' valign='top'><span sugar="sugar{$colCounter}b">
					{if $params.link && !$params.customCode}
						...
						{/if}
							...
							onmouseover="return getDocumentDetails('KBOLDDocuments', '{$rowData[$params.id]|default:$rowData.ID}', 'adspan_{$rowData[$params.id]|default:$rowData.ID}', '{$rowData[$params.id]|default:$rowData.KBOLDDOCUMENT_NAME_js}', 'panel_{$rowData[$params.id]|default:$rowData.ID}','{$pageData.offsets.current+$smarty.foreach.rowIteration.iteration}')" onclick="updateKBViewsCount('{$rowData[$params.id]|default:$rowData.ID}')">
