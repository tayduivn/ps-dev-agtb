 ...
 */
    ...
    $current_module_strings = return_module_language($current_language, 'KBOLDDocuments');
    ...
    $view = new SugarView();
    $view->type = 'list';
    $view->module = 'KBOLDDocuments';
    $metadataFile = $view->getMetaDataFile();
    require_once($metadataFile);
    require_once('modules/KBOLDDocuments/KBListViewData.php');
    ...
    $json = getJSONobj();
    $seedDocument = BeanFactory::getBean('KBOLDDocuments'); // seed bean
    ...
    if(!empty($_REQUEST['displayColumns'])) {
        foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
            if(!empty($listViewDefs['KBOLDDocuments'][$col]))
                $displayColumns[$col] = $listViewDefs['KBOLDDocuments'][$col];
        }
    }
    elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences
        ...
        foreach($listViewDefs['KBOLDDocuments'] as $col => $params) {
    ...
    $lv->setup($seedDocument, 'modules/KBOLDDocuments/SearchListView.tpl', $where, $params);
    ...
    foreach ($lv->data['data'] as $arrRec){
        ...
        if(isset($arrRec['KBOLDDOCUMENT_NAME']) && !empty($arrRec['KBOLDDOCUMENT_NAME'])){
            $GLOBALS['log']->info("name of record getting quotes stripped is: ".$arrRec['KBOLDDOCUMENT_NAME']);
            if(!empty($arrRec['KBOLDDOCUMENT_NAME']))
                $temp_name = $arrRec['KBOLDDOCUMENT_NAME'];
                ...
                $arrRec['KBOLDDOCUMENT_NAME_js'] = $temp_name;
                $temp_Data[] = $arrRec;
        }
    }
	}
...
function get_admin_fts_list($where,$isMultiSelect=false){
    ...
    require_once('include/ListView/ListViewSmarty.php');
	require_once SugarAutoLoader::loadWithMetafiles('KBOLDDocuments', 'listviewdefs');
    require_once('modules/KBOLDDocuments/KBListViewData.php');
	...
	$seedCase = BeanFactory::getBean('KBOLDDocuments'); // seed bean
	// setup listview smarty
	$lv = new ListViewSmarty();
    $lv->lvd= new KBListViewData();
    $lv->export = false;
	...
	if(isset($_REQUEST['query']))
	{
	    ...
	    $where_clauses = $searchForm->generateSearchWhere(true, "kbolddocuments"); // builds the where clause from search field inputs
    ...
    $lv->delete = false;
    $lv->select = false;
    $lv->setup($seedCase, 'modules/KBOLDDocuments/AdminSearchListView.tpl', $where, $params);
	$lv->show_mass_update_form=false;
	$savedSearchName = empty($_REQUEST['saved_search_select_name']) ? '' : (' - ' . $_REQUEST['saved_search_select_name']);
   ...
   function get_faq_list($faq_id, $bean) {
       ...
       $query = "select distinct(k.id)as id, k.created_by as created_by from kbolddocuments k
                INNER join kbolddocuments_kboldtags kt on kt.kbolddocument_id = k.id ";
       //BEGIN SUGARCRM flav=ent ONLY
       $query .= " AND k.is_external_article = 1";
       //END SUGARCRM flav=ent ONLY
	$query .= "	AND kt.kboldtag_id in ($queryIds)";

       $result = $bean->db->query($query);

       while($row = $bean->db->fetchByAssoc($result)) {
             $id = $row['id'];
             $record = BeanFactory::getBean('KBOLDDocuments', $id, array("disable_row_level_security" => true));
       ...
       // With the list of KBOLDDocuments on hand, now add the hierarachy information
       $query = "select kb.id as id, kb.kbolddocument_id as doc_id, kb.kboldtag_id as tag_id, t.parent_tag_id as parent_id, t.tag_name as tag_name
                from kbolddocuments_kboldtags kb INNER JOIN kboldtags t on kb.kboldtag_id = t.id AND kb.kboldtag_id in ($queryIds)";
       $result = $bean->db->query($query);
       while($row = $bean->db->fetchByAssoc($result)) {
             $rec = $list[$row['doc_id']];
             $rec->tags[] = $row;
   ...
   function get_child_tags($tag, $bean) {
        $list = array();
        $query = "select id, parent_tag_id, tag_name from kboldtags where parent_tag_id = '$tag'";
        $result = $bean->db->query($query);
        if(!$result) {
           return $list;
        }
   ...
   function get_tag_docs($tag, $bean) {
        ...
        $date_filter = str_replace("kbolddocuments", "k", $date_filter);
        $date_filter = "($date_filter OR k.exp_date IS NULL)";
        $query = "select distinct(k.id) as doc_id, k.kbolddocument_name as doc_name from kbolddocuments k
                 INNER join kbolddocuments_kboldtags kt on kt.kbolddocument_id = k.id AND kt.deleted = 0";
       //BEGIN SUGARCRM flav=ent ONLY
       $query .= " AND k.is_external_article = 1";
       //END SUGARCRM flav=ent ONLY
       $query .= " AND k.status_id = 'Published' AND k.deleted = 0 AND " . $date_filter . " AND kt.kboldtag_id = '$tag'";
   ...
   /**
    * get_kbolddocument_body
    *
    */
   function get_kbolddocument_body($kbdoc_id, $bean) {

        $query="select kbolddocument_body from kboldcontents where id in
               (select kboldcontent_id from kbolddocument_revisions where kbolddocument_id = '$kbdoc_id')";
    ...
    * @param string $root_id The root id value to query for (i.e. the id column in kboldtags table)
   ...
   function get_child_ids($root_id, $bean) {
        $list = array();
        $query = "select id, parent_tag_id, tag_name from kboldtags";
        $result = $bean->db->query($query);
        if(!$result) {
           return $list;
        }
   ...
   class TagNode {
	...
	function create_portal_list_query($bean, $order_by, $where, $keywords, $row_offset, $limit) {
		...
		$date_filter = "($date_filter OR kbolddocuments.exp_date IS NULL) ";
		...
		if(!empty($keywords)) {
		   if(isset($keywords['keywords']) && $keywords['keywords'] == '[most_recent_articles]') {
			   ...
			   if(!empty($limit)) {
				   ...
				   if(preg_match('/LIMIT[\s]+[\d]+,[\d]+/', $sql)) {
				  	  $sql = substr($sql, 0, strpos($sql, "LIMIT"));
		   	   	   ...
		   	   	   $result = $bean->db->limitQuery($sql, $row_offset, $limit, true, "Error retrieving KBOLDDocument list: $sql");
                   return $result;
			   } //if
			   ...
			   if(!empty($limit)) {
				   $result = $bean->db->limitQuery($sql, $row_offset, $limit, true, "Error retrieving KBOLDDocument list: ");
			       return $result;
			   }
    ...
    function create_portal_most_recent_query($db, $n = '10', $where='')
	{
		...
		return $db->limitQuerySQL("SELECT kbolddocuments.* FROM kbolddocuments WHERE deleted=0
   ...
   /**getQSAuthor()
    ...
    /**
    $searchVars should be an array of bean variables to search.  Key name should be name of variables and should map directly to the kbolddocument table itself.
    example of some acceptable searchVars are:
        $searchVars['kbolddocument_name']
    ...
    ** $spec_SearchVars  array is meant to handle special search enhancements.  These enhancements and acceptable params are as follow:
            ...
            ** Active_Date Filters
                ...
                $spec_SearchVars['active_date_filter']  //define filter according to 'kbolddocument_date_filter_options' dom object
            ...
            ** Exp_Date Date Filters
                ...
                $spec_SearchVars['exp_date_filter']     //define filter according to 'kbolddocument_date_filter_options' dom object
            ...
            ** Viewed Frequency Rate Filters
                ...
                This parameter will be used to add on 'viewed frequency' filters, according to the 'kbolddocument_viewing_frequency_dom' dom object


            ** Canned Search Filters
                $spec_SearchVars['canned_search']

                This parameter will be used to add on 'canned query' filters, according to the 'kbolddocument_canned_search' dom object
    ...
    //create portion of query that holds the fts search
        ...
        $qry_arr['custom_from'] = "INNER JOIN(
              SELECT kbolddocument_id as id FROM kbolddocument_revisions WHERE deleted = 0 and latest = 1 ";
        ...
        if ((!empty($query_include) && $query_include[0] != '*') || !empty($query_must)) {
            ...
            $qry_arr['custom_from']= $qry_arr['custom_from']. "and kboldcontent_id in (
                 select id from kboldcontents where deleted = 0 and ".$db->getFulltextQuery('kbolddocument_body', $query_include, $query_must, $query_exclude).")";
        }

        $qry_arr['custom_from'] .=  ") derived_table ON kbolddocuments.id = derived_table.id ";
        ...
        if(isset($spec_SearchVars['tag_name'] )  && !empty($spec_SearchVars['tag_name'] )){
            ...
            if(isset($spec_SearchVars['tag_id'] )  && !empty($spec_SearchVars['tag_id'] )){
              ...
              $search_str .= "
                  and kbolddocuments.id in (
                        select kbolddocument_id from kbolddocuments_kboldtags where kboldtag_id in
                        (
                            $tag_name_string
                        )
                    )";
                  ...
                  $search_str .= "
                      and kbolddocuments.id in (
                            select kbolddocument_id from kbolddocuments_kboldtags where kboldtag_id in
                            (
                                select id from kboldtags where tag_name in($tag_name_string)
                            )
                        )";
                  }
                }
            ...
            foreach($searchVars as $key=>$val){
                ...
                if(is_array($val)){
                    ...
                    if(isset($val['operator']) && !empty($val['operator'])){
                        ...
                        $search_str .= " kbolddocuments.$key $op '".$constraint."' ";
                    }else{
        ...
        if($fullQuery){
            $search_str_full  = 'Select kbolddocuments.*, kbolddocuments_views_ratings.views_number';
            $search_str_full .= ' FROM kbolddocuments left join kbolddocuments_views_ratings ON kbolddocuments.id = kbolddocuments_views_ratings.kbolddocument_id  ';
...
function return_date_filter($db, $field, $filter, $filter_date='', $filter_date2=''){
    ...
    function return_view_frequency_filter($db, $freq_search_opt){
        ...
        if(substr($freq_search_opt, 0, 4) == 'Top_' && is_numeric(substr($freq_search_opt, 4))) {
            $filter_query = $db->limitQuerySQL("SELECT kbolddocument_id FROM kbolddocuments_views_ratings WHERE deleted = 0
            	AND kbolddocument_id IS NOT NULL ORDER BY kbolddocuments_views_ratings.views_number DESC",0, intval(substr($freq_search_opt, 4)));
        }

        //process 'bot X' filter
        if(substr($freq_search_opt, 0, 4) == 'Bot_' && is_numeric(substr($freq_search_opt, 4))) {
            $filter_query = $db->limitQuerySQL("SELECT kbolddocument_id FROM kbolddocuments_views_ratings WHERE deleted = 0
            	AND kbolddocument_id IS NOT NULL ORDER BY kbolddocuments_views_ratings.views_number ASC, kbolddocuments_views_ratings.date_modified ASC",0, intval(substr($freq_search_opt, 4)));
            $isTop = false;
        }
        ...
        if(!$db->supports("limit_subquery")){
            ...
            if(!empty($result)) {
                while($row = $db->fetchByAssoc($result)) {
                 	   $ids[] = $db->quoted($row['kbolddocument_id']);
                }
            }
        ...
        if(!empty($filter_query)) {
           return " AND kbolddocuments.id IN ( $filter_query )";
        } else {
            return "";
        }
    }
            ...
            $return_can = '';
             ...
             if (isset($canned_search_opt)){
                ...
                if($canned_search_opt == 'added'){
                    ...
                    return " AND (kbolddocuments.date_entered >=" .$db->convert($db->quoted($range[0]->asDb()), 'datetime') .
            			" AND kbolddocuments.date_entered <= " . $db->convert($db->quoted($range[1]->asDb()), 'datetime') .") ";
                }

                //process 'articles pending my approval' filter
                if($canned_search_opt == 'pending'){
                    global $current_user;
                    $return_can = "and kbolddocuments.status_id='In Review' and kbolddocuments.kbdoc_approver_id = '". $current_user->id . "'";
                }

                //process 'articles updated last 30 days' filter
                if($canned_search_opt == 'updated'){
                    $range = TimeDate::getInstance()->parseDateRange('last_30_days');
                    return " AND (kbolddocuments.date_modified >=" .$db->convert($db->quoted($range[0]->asDb()), 'datetime') .
            			" AND kbolddocuments.date_modified <= " . $db->convert($db->quoted($range[1]->asDb()), 'datetime') .") ";
                }
                ...
                if($canned_search_opt == 'faqs'){
                  $return_can .= "
                      and kbolddocuments.id in (
                            select kbolddocument_id from kbolddocuments_kboldtags where kboldtag_id in
                            (
                                select id from kboldtags where tag_name ='faqs'
                            )
                        )";
                }
            }
  ...
  function return_attachment_filter($db, $spec_SearchVars){
            ...
            //process if filter exists
             if (isset($attachment_search_opt)){
                ...
                if($attachment_search_opt == 'none' || $attachment_search_opt == 'some'){
                    $return_att = "and kbolddocuments.id not in (
                            select kbolddocument_id
                            from kbolddocument_revisions
                            where coalesce({$db->convert('kbolddocument_id', 'length')},0) > 0 or kboldcontent_id IS NULL
                        )";
                }
                ...
                if($attachment_search_opt == 'name'){
                    if(isset($spec_SearchVars['filename'])  && !empty($spec_SearchVars['filename'])){
                        $return_att =
                                    " AND kbolddocuments.id in
                                    (
                                        select kbolddocument_id from kbolddocument_revisions where document_revision_id  in
                                        (
    ...
    function updateKBView($kbid){
        ...
        $query = "select * from kbolddocuments_views_ratings where kbolddocument_id='$kbid'";
        $result = $GLOBALS['db']->query($query);
        $rowq  =  $GLOBALS['db']->fetchByAssoc($result);

        if(!isset($kbid) || empty($kbid)){
         $GLOBALS['log']->fatal("KBOLDdocument id is null in updateKBView, this is an error.");
        }

        //update if exists else create a new entry
        if($rowq != null){
           $query = "update kbolddocuments_views_ratings set views_number = views_number+1 where kbolddocument_id='$kbid'";
           $result = $GLOBALS['db']->query($query);
        }
        else{
          $query = "insert into kbolddocuments_views_ratings (id,kbolddocument_id,views_number) values('$guid','$kbid',1)";
          $result = $GLOBALS['db']->query($query);
        }
    ...
    function create_most_recent_articles_query($bean, $order_by, $where, $keywords, $row_offset, $limit, $date_filter) {
		$where = " AND kbolddocuments.status_id = 'Published' AND " . $date_filter;
