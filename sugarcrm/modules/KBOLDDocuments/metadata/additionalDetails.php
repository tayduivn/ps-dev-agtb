...
function additionalDetailsKBOLDDocument($fields,$full=false) {
    static $mod_strings;
...
//    if($full){
        ...
        if(!empty($fields['ID'])){
            ...
            //query for content
            $content_query = "Select kbolddocument_body from kboldcontents where id in";
            $content_query .= "(select kboldcontent_id from kbolddocument_revisions where ";
            $content_query .= "kbolddocument_id = '" . $fields['ID'] . "' and latest = '1')";
            ...
            if(!isset($row) || empty($row)){
                ...
                if(!empty($row['kbolddocument_body'])){
                    $contentBody = from_html($row['kbolddocument_body']);
   ...
   /* }else{
        ...
        $overlib_string = '';
        if(!empty($fields['KBOLDDOCUMENT_NAME'])) $overlib_string .= '<b>' . $mod_strings['LBL_NAME'] . '</b> ' . $fields['KBOLDDOCUMENT_NAME'] . '<br>';
        if(!empty($fields['DESCRIPTION'])) $overlib_string .= '<tr><td nowrap><b>'. $mod_strings['LBL_DESCRIPTION'] . '</b> ' . $fields['DESCRIPTION'] . '<br>';
    ...
    return array('fieldToAddTo' => 'NAME',
                 'string' => $overlib_string,
                 'editLink' => "index.php?action=EditView&module=KBOLDDocuments&return_module=KBOLDDocuments&record={$fields['ID']}",
                 'viewLink' => "index.php?action=DetailView&module=KBOLDDocuments&return_module=KBOLDDocuments&record={$fields['ID']}");
