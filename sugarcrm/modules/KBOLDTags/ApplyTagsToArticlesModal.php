...
for ($i = 0; $i < $tagsCount; $i++) {
    ...
    $qdocs = 'SELECT kbolddocument_id FROM kbolddocuments_kboldtags WHERE kboldtag_id=\'' . $tagId .
    ...
    for ($j = 0; $j < $articlesCount; $j++) {
        ...
        while (($rows = $GLOBALS['db']->fetchByAssoc($results)) != null) {
            if (!empty($rows['kbolddocument_id'])) {
                if ($rows['kbolddocument_id'] == $tagArticleIds[$j + 2 + $tagsCount]) {
                    $articleExists = true;
                    break;
                }
            }
        ...
        if (!$articleExists) {
            $KBOLDDocument = BeanFactory::getBean('KBOLDDocuments', $tagArticleIds[$j + 2 + $tagsCount]);
            $doc_team_id = $KBOLDDocument->team_id;
            $guid = create_guid();
            $qi = 'INSERT INTO kbolddocuments_kboldtags (id,kboldtag_id,kbolddocument_id,team_id) values(\'' .
