 ...
 */
 function get_tags_nodes($create_root_node=false,$admin=false,$parent_id=null){
    ...
    $mod_strings = get_kboldtag_strings();
    
    //$query="select id,tag_name from kboldtags where parent_tag_id is null and deleted=0 order by tag_name";
...
function get_tags_nodes_cached($admin=false){
 ...
 function childNodes(&$parent_tag_node,$parent_tag_id,$nodes,$admin=false){ 	
    $nodes=array();    
 	$query_child="select id,tag_name from kboldtags where parent_tag_id = '$parent_tag_id' and deleted=0 order by id";
...
function getParentNode($tag_id){ 	 
 	...
 	$query_parent="select pr.id , pr.tag_name from kboldtags pr where pr.id in (select parent_tag_id from kboldtags where id = '$tag_id' and deleted=0)";
 ...
 function get_admin_browse_articles($params){
    $_REQUEST['module'] = 'KBOLDDocuments';
    ...
    $colorclass = '';$bgColor=$even_bg;
        ...
        $search_str = "kbolddocuments.id
                            IN (
                                SELECT kbd.id
                                FROM kbolddocuments kbd, kbolddocuments_kboldtags kbd_kt
                                WHERE kbd.id = kbd_kt.kbolddocument_id
                                AND kbd_kt.kboldtag_id = '$tag_id'
 ...
 function tagged_documents_count($tag_id) {
    ...
    $docs = array();
    $count = 0;	 
    $query = " select count(kbt.id) doc_count from kbolddocuments_kboldtags kbt";
    ...
    $query.= " where kbt.kboldtag_id='$tag_id' and kbt.deleted=0";
 	$result=$GLOBALS['db']->query($query);    
...
function tag_count($tag_id) {
    ...
    $docs = array();
    $count = 0;  
    $query = " select count(kt.id) file_count from kboldtags kt";
    ...
    $colorclass = '';$bgColor=$even_bg;
     ...
     //get id of node selected
        ...
        if($tag_id == 'UNTAGGED_NODE_ID'){
            ...
            $search_str= "kbolddocuments.id NOT IN
                            (select kbolddocument_id from kbolddocuments_kboldtags where deleted=0) and kbolddocuments.deleted = 0";
 ...
 */
 function untagged_documents_count() {
    ...
    $query = "select count(kb.id) doc_count  from kbolddocuments kb";
    //BEGIN SUGARCRM flav=pro ONLY
    if (!is_admin($current_user)){ 
        $query.= " Inner Join team_memberships tm on kb.team_id = tm.team_id  AND tm.user_id =  '".$current_user->id. "' and tm.deleted = 0 ";
    }
        //END SUGARCRM flav=pro ONLY
    $query.= " where kb.id not in (select kbolddocument_id from kbolddocuments_kboldtags where deleted=0) and kb.deleted = 0";
    $result=$GLOBALS['db']->query($query);    
    $docs = $GLOBALS['db']->fetchByAssoc($result);   
    if($docs['doc_count'] != null) $count= $docs['doc_count'];
    return $count;
...
function check_tag_child_tags_for_articles($parent_tag_id){
    ...
    global $app_list_strings;
    $mod_strings = get_kboldtag_strings(); 
    $query="select id,tag_name from kboldtags where parent_tag_id = '$parent_tag_id' and deleted=0 order by id";
    $result=$GLOBALS['db']->query($query);
   $hasArticle = false;
    while (($row=$GLOBALS['db']->fetchByAssoc($result))!= null) {
        if (!empty($row['id'])) {                   
 ...
 function childTagsHaveArticles($child_tag_id){ 	
 	...
 	$query_child="select id,tag_name from kboldtags where parent_tag_id = '$child_tag_id' and deleted=0 order by id";
    $result_child=$GLOBALS['db']->query($query_child);
    $hasArticle = false;
    while (($rowChild=$GLOBALS['db']->fetchByAssoc($result_child))!= null) {
       $tag_id=$rowChild['id'];
...
function get_kboldtag_strings(){
    global $mod_strings, $sugar_config;
        ...
        return return_module_language($current_language,"KBOLDTags");
