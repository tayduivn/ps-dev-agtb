...
if(isset($_POST['handle']) && $_POST['handle'] == 'Save'){
	...
	if(isset($_POST['selectedContact']) && !empty($_POST['selectedContact'])){
		...
		$_POST['Contactsteam_id'] = $lead->team_id;
		$_POST['Contactsteam_set_id'] = $lead->team_set_id;
	...
	if((isset($_POST['selectedAccount'])&& !empty($_POST['selectedAccount'])) || (isset($_POST['newaccount']) && $_POST['newaccount']=='on' )){
		if(isset($_POST['selectedAccount']) && !empty($_POST['selectedAccount'])){
			...
			$_POST['Accountsteam_id'] = $lead->team_id;
			$_POST['Accountsteam_set_id'] = $lead->team_set_id;
	...
	if(isset($_POST['newopportunity']) && $_POST['newopportunity']=='on' ){
		...
		if( isset($_POST['selectedOpportunity']) && !empty($_POST['selectedOpportunity'])){
			...
			$_POST['Opportunitiesteam_id'] = $lead->team_id;
			$_POST['Opportunitiesteam_set_id'] = $lead->team_set_id;
			$_POST['Opportunitieslead_source'] = $lead->lead_source;
			$_POST['Opportunitiescampaign_id'] = $lead->campaign_id;
            // Bug 30655 - Copy over Lead custom fields that have the same name as Opportunity custom fields
            foreach ( $lead->getFieldDefinitions() as $name => $def )
                if ( isset($def['source']) && $def['source'] == 'custom_fields' && isset($lead->$name) )
                    $_POST['Opportunities'.$name] = $lead->$name;
	...
	if(isset($account) && (isset($_POST['newaccountnote'])&& !empty($_POST['newaccountnote']))){
		$_POST['AccountNotesteam_id'] = $lead->team_id;
		$_POST['AccountNotesteam_set_id'] = $lead->team_set_id;
	...
	if(isset($contact) && (isset($_POST['newcontactnote'])&& !empty($_POST['newcontactnote']))){
		$_POST['ContactNotesteam_id'] = $lead->team_id;
		$_POST['ContactNotesteam_set_id'] = $lead->team_set_id;
	...
	if(isset($opportunity) && (isset($_POST['newoppnote'])&& !empty($_POST['newoppnote']))){
		$_POST['OpportunityNotesteam_id'] = $lead->team_id;
		$_POST['OpportunityNotesteam_set_id'] = $lead->team_set_id;
		$opportunitynote= $noteForm->handleSave('OpportunityNotes',false, false);
		}

	$_POST['Appointmentsteam_id'] = $lead->team_id;
	$_POST['Appointmentsteam_set_id'] = $lead->team_set_id;
	...
	if(isset($contact)){
		...
		if(isset($_POST['selectedContact']) && $_POST['selectedContact'] == $contact->id){
			array_push($ROWVALUES, "<LI>".$mod_strings['LBL_EXISTING_CONTACT']." - <a href='index.php?action=DetailView&module=Contacts&record=".$contact->id."'>".$locale->getLocaleFormattedName($contact->first_name,$contact->last_name)."</a>" );
		}else{
			array_push($ROWVALUES, "<LI>".$mod_strings['LBL_CREATED_CONTACT']." - <a href='index.php?action=DetailView&module=Contacts&record=".$contact->id."'>".$locale->getLocaleFormattedName($contact->first_name,$contact->last_name)."</a>" );
		}
	}
	if(isset($account)){
  ...
  echo $sugar_smarty->fetch('modules/Leads/ConvertLead.tpl');
    ...
    $_REQUEST['birthdate'] = $lead->birthdate;
