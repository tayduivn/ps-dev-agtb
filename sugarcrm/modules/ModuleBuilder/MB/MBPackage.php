...
function buildInstall($path){
    ...
    /**
     * Create .zip package
     * @param string $zipDir directory where .zip should be
     * @param string $zipPath directory to pack
     */
    protected function createZip($zipDir = null, $zipPath = null)
    {
        ...
        $fname = $zipDir. '/'. $this->name. $date. '.zip';
        touch($fname); // so that realpath would work
        $real_fname = realpath($fname);
        $cwd = getcwd();
        if(empty($zipPath)) {
            $zipPath = realpath($this->getBuildDir());
        } else {
    ...
    function exportProject($package, $export=true, $clean = true){
        ...
        $fname = $this->createZip("custom/modulebuilder/packages/ExportProjectZips", $tmppath);
