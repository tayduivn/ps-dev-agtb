...
'LBL_POPHELP_FTS_FIELD_CONFIG' => 'Configure the field to be full text searchable <br />When selecting <i>store only</i> the field value will be stored in the full text search engine being able to return its value in the results. Stored fields are not searchable. The value of a searchable field will always be returned in the results.',
'LBL_POPHELP_FTS_FIELD_BOOST' => 'Enter the boost level for this field.<br />Fields with a higher boost level will be given greater weight when the search is performed. When a search is performed, matching records containing fields with a greater weight will be appear higher in the search results. Besides the boost value other factors come in to play influencing the relevance order like the term to document distribution, the influence of other field boosts and the different analyzers which are working behind the curtains.<br />The default value is 1.0 which stands for a neutral boost. To apply a positive boost any float value higher than 1 is accepted. For a negative boost use values lower than 1. For example a value of 1.35 will positively boost a field by 135%. Using a value of 0.60 will apply a negative boost.<br />Note that in previous versions it was required to perform a full text search reindex. This is no longer required.',
