...
class History implements HistoryInterface
{
    ...
    public function __construct ( $previewFilename )
    {
        ...
        $this->_dirname = dirname ( $this->_previewFilename ) . DIRECTORY_SEPARATOR . md5( $this->_basename );
...
/*
    ...
    public function append ($path)
    {
        ...
        $time = $now->__get('ts');
        while ( (file_exists ( $new_file = $this->getFileByTimestamp( $time ) ) && $retries < 5) )
        {
            $now->modify("+1 second");
            $time = $now->__get('ts');
            $retries ++ ;
        ...
        if ($this->_historyLimit != 0 && $to_delete = ($this->getCount() - $this->_historyLimit))
        {
    ...
    /**
     * Returns full path to history file by timestamp
     * @param  $timestamp
     * @return string
     */
    private function getFileByTimestamp($timestamp)
    {
        return $this->_dirname . DIRECTORY_SEPARATOR . $timestamp ;
    }
