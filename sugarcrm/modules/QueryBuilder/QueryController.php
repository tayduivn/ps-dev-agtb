 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: QueryController.php 45763 2009-04-01 19:16:18Z majed $
 * Description:
 ********************************************************************************/
...
class QueryController extends SugarBean {
	...
	function change_component_order($magnitude, $direction, $parent_id=""){
		...
		if(!empty($this->type) && $this->type=="Save"){
			...
			if($wall_test==false){
				...
				//Figure out how to do this right here:
		...
		//this is a new component, set the x or y value to the max + 1
				...
				$query = "SELECT MAX(".$this->focus->controller_def['start_var'].") as max_start FROM ".$this->focus->table_name."
...
//this is only single axis check_wall mechanism, will need to upgrade this to double axis	
...
//If up or left, then simply check the 0 value	
	...
	//Should be ok to change order if you got this far
	return true;
...
$current_position = $this->focus->$variable_name;
	...
	$query .= "WHERE ".$this->focus->controller_def['start_var']." > ".$current_position." AND deleted=0 ";
		...
		$result = $this->db->query($query,true," Error updating the list order info: ");
...
//end controller component functions	
