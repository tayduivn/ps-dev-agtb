...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
		<td nowrap>
			<h3><?php echo SugarThemeRegistry::current()->getImage/*ALTFIXED*/("h3Arrow", 'border="0"', 11, 11, ".gif", $current_module_strings['LBL_HOME_TITLE']) ?>
...
if(!empty($items)) {
	foreach($items as $k => $bean) {
		...
		if($bean->object_name == 'Email') {
			...
			$queueIcon = $ahref.SugarThemeRegistry::current()->getImage/*ALTFIXED*/("Emails", "", null, null, ".gif", $mod_strings['LBL_EMAILS'])."</a>";
			...
			if($bean->type == 'inbound') {
				...
				if(!empty($bean->case_id) && !empty($bean->case_name)) {
					...
					$taskIcon = $ahref.SugarThemeRegistry::current()->getImage/*ALTFIXED*/("Cases", "", null, null, ".gif", $mod_strings['LBL_CASES'])."</a>";
