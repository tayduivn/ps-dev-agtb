...
function checkACLForEachColInArr ($arr, $full_table_list, $is_owner = 1){
	...
	foreach ($arr as $column) {
		...
		if(!SugarACL::checkField($col_module, $column['name'], $is_owner?array("owner_override" => true):array())) {
			return false;
		}
	}
	//END SUGARCRM flav!=sales ONLY
...
function checkACLForEachColForFilter($filters, $full_table_list, $is_owner, $hasAccess) {
	...
	while (isset($filters[$i])) {
		...
		if (isset($current_filter['operator'])) {
			...
			//BEGIN SUGARCRM flav!=sales ONLY
			if(!SugarACL::checkField($col_module, $current_filter['name'], $is_owner?array("owner_override" => true):array())) {
				return false;
			} // if
			//END SUGARCRM flav!=sales ONLY
		}
