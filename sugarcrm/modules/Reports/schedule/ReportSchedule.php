...
function save_schedule($id, $user_id, $report_id, $date_start, $interval, $active, $schedule_type){
	global $timedate;
	$time = time();
    $adjustmentForUserTimeZone = $timedate->adjustmentForUserTimeZone();
	$origDateStart = $date_start;
	if(empty($id)){
		$id = create_guid();

		if(empty($date_start)){
      		$date_start = date($timedate->get_db_date_time_format(), $time-$adjustmentForUserTimeZone*60); //in user's timezone
		} else if(strtotime($date_start)+$adjustmentForUserTimeZone*60>$time){
      		$date_start .= ' 00:00:00';  	//in user's timezone
    	} else {
    		$date_start .= ' '.date('H:i:00', $time-$adjustmentForUserTimeZone*60); //in user's timezone
    	}
    	
        $date_start = strtotime($date_start);   
        $date_start += $adjustmentForUserTimeZone*60;  //server time  
           
        while($date_start <= $time){
            $date_start += $interval; 
        }  
        $date_start = gmdate($timedate->get_db_date_time_format(), $date_start);
		...
		$query = "INSERT INTO $this->table_name (id, user_id, report_id, date_start,next_run, time_interval, active, schedule_type) VALUES ('$id', '$user_id', '$report_id', $date_start_str , '$date_start', '$interval', '$active', '$schedule_type')";	
	}else{
	    $date_start_str = "";
	    if(strlen(trim($origDateStart))==0) {
		...
		if(!empty($date_start) && $active){
	       	...
	       	if(strtotime($date_start)+$adjustmentForUserTimeZone*60>$time){
				$date_start .= ' 00:00:00'; 
	        } else {
	        	$date_start .= ' '.date('H:i:00', $time-$adjustmentForUserTimeZone*60);	            
	        }
			...
			$date_start = db_convert($date_start, 'datetime');
			$query .= ", next_run=$date_start";		
		} // if
		$query .= " WHERE id='$id'";
	}
