...
{/php}
	...
	{/if}
	{if ($isSummaryComboHeader)}
	<th><span id="img_{$divId}"><a href="javascript:expandCollapseComboSummaryDiv('{$divId}')"><<!--ALTFIXED-->img width="8" height="8" border="0" absmiddle="" alt="{$mod_strings.LBL_SHOW}" src="{$image_path}advanced_search.gif"/></a></span></th>
	{/if}
	{php}
		$count = 0;
		$this->assign('count', $count);
