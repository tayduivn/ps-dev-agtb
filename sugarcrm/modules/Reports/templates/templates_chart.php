...
function get_row_remap(& $row, & $reporter) {
    ...
    $row_remap['group_text'] = $group_text = (!empty($reporter->chart_group_position)) ? chop($row['cells'][$reporter->chart_group_position]['val']) : '';
    $row_remap['group_key'] = (!empty($reporter->chart_group_position) ? $row['cells'][$reporter->chart_group_position]['key'] : '');
    $row_remap['count'] = $row['count'];
    $row_remap['group_label'] = (!empty($reporter->chart_group_position) ? $reporter->chart_header_row[$reporter->chart_group_position]['label'] : '');
