...
function template_pagination(&$args) {
	...
	if(!ACLController::checkAccess('Reports', 'export', $is_owner) || $sugar_config['disable_export'] || (!empty($sugar_config['admin_export_only']) && !(is_admin($current_user) || (ACLController::moduleSupportsACL($reporter->module) && ACLAction::getUserAccessLevel($current_user->id,$reporter->module, 'access') == ACL_ALLOW_ENABLED && ACLAction::getUserAccessLevel($current_user->id, $reporter->module, 'admin') == ACL_ALLOW_ADMIN)))){
		...
		$smarty->assign('exportImagePath', SugarThemeRegistry::current()->getImage/*ALTFIXED*/('export',"  border='0' align='absmiddle'",null,null,'.gif',translate('LBL_EXPORT')));
		$isExportAccess = true;
	} // else
	$smarty->assign('isExportAccess', $isExportAccess);
	$smarty->assign('start_link_ImagePath', SugarThemeRegistry::current()->getImage/*ALTFIXED*/("start_off","  border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_START']));
	$smarty->assign('prev_link_ImagePath', SugarThemeRegistry::current()->getImage/*ALTFIXED*/("previous_off","border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_PREVIOUS']));
	$smarty->assign('end_link_ImagePath', SugarThemeRegistry::current()->getImage/*ALTFIXED*/("end_off","border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_END']));
	$smarty->assign('next_link_ImagePath', SugarThemeRegistry::current()->getImage/*ALTFIXED*/("next_off","border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_NEXT']));
	...
	$next = $reporter->row_end + $reporter->report_offset;
 	if($reporter->report_offset > 0){
	 	...
	 	$smarty->assign('start_link_ImagePath', SugarThemeRegistry::current()->getImage/*ALTFIXED*/("start","  border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_START']));
	 	$smarty->assign('start_link_onclick' , "onClick=javascript:set_offset(0);");
	 	$smarty->assign('start_link_disabled', false);
		$smarty->assign('prev_link_ImagePath', SugarThemeRegistry::current()->getImage/*ALTFIXED*/("previous","border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_PREVIOUS']));
	...
	if($next < $reporter->total_count){
		...
		$smarty->assign('end_link_ImagePath', SugarThemeRegistry::current()->getImage/*ALTFIXED*/("end","  border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_END']));
 		$smarty->assign('end_link_disabled', false);
	 	$smarty->assign('end_link_onclick' , "onClick=javascript:set_offset($end);");
		
		$smarty->assign('next_link_ImagePath', SugarThemeRegistry::current()->getImage/*ALTFIXED*/("next"," border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_NEXT']));
...
<tr height="20" class="<?php echo $row_class; ?>">
    ...
    $count = 0;
		if ($isSummaryComboHeader) {
			echo '<td><span id="img_'.$divId.'"><a href="javascript:expandCollapseComboSummaryDiv(\''.$divId.'\')">'.SugarThemeRegistry::current()->getImage/*ALTFIXED*/("advanced_search", 'border="0" absmiddle=""', 8, 8, ".gif", $mod_strings['LBL_SHOW']).'"</a></span></td>';
		}      
