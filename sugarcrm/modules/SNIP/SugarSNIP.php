...
class SugarSNIP
{
    ...
    */
    ...
    {
    /// should nener be called in Lite!
        $user = new User();
        ...
        //$user->portal_only = 1;
        //$user->is_group = 1;
        $user->user_hash = strtolower(md5(time()));
    ...
    public function importEmail($email)
    {
        ...
        $e->id = create_guid(); //Will want to generate id from email message contents and ensure no dups.
        $e->new_with_id = true;
        ...
        $e->from_addr = $e->from_addr_name = $e->from_name;
        $e->name = $e->subject;
        ...
        if(isset($email['user'])) {
            $user = new User();
            $e->assigned_user_id = $user->retrieve_user_id($email['user']);
        }
        if(empty($e->assigned_user_id)) {
