 ...
 *Portions created by SugarCRM are Copyright (C) 2008 SugarCRM, Inc.; All Rights Reserved.
 ********************************************************************************/
...
class SugarFeedFlush {
    function flushStaleEntries($bean, $event, $arguments) {
        ...
        if ( $admin->settings['sugarfeed_flushdate'] != $currDate ) {
            ...
            $tmpTime = time();
            $tmpSF = new SugarFeed();
            $flushBefore = $timedate->asDbDate($timedate->getNow()->get("-14 days")->setTime(0,0));
