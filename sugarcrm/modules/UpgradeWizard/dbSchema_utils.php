...
function checkSchema($execute=false,$return=false,$checkThisRel){
	...
	*/
	...
	// also add check for duplicate relationships
     if($checkThisRel != null) $currcheckThisRel = $checkThisRel;
     traceDuplicateRelations($currcheckThisRel);
...
function traceDuplicateRelations($checkThisRel){
    ...
    foreach ( $rel_dictionary as $rel_name => $rel_data )
    {
        ...
        traceDuplicateRelationshipMeta( $rel_name, $table, $rel_dictionary, '',$processed_relationships,$duplicate_relationships,$currcheckThisRel);
    ...
    if($duplicate_relationships != null){
		...
		foreach($duplicate_relationships as $rel_name=>$rel){
			...
			if($checkThisRel != null){
				if(is_array($checkThisRel)){
					foreach($checkThisRel as $checkrel_name=>$rel){
						if(trim(strtolower($rel_name))==trim(strtolower($checkrel_name))){
		...
		if (isset($dictionary[$key]['relationships']))
		{
			...
			foreach ($RelationshipDefs as $rel_name=>$rel_def)
			{
				...
				else
				{
					...
					//if($checkThisRel != null && trim(strtolower($rel_name))==trim(strtolower($checkThisRel))){
						//break;
					//}
				}
...
function checkIndexExists($table_name,$column_name){
    ...
    $row=mysql_fetch_assoc($ct);
    if($row == null){
    	$row = checkIndexExistsInVardef($table_name,$column_name);
    }
	return $row;
}

function checkIndexExistsInVardef($table_name,$column_name){
	include ('include/modules.php') ;
	...
	foreach ( $beanFiles as $bean => $file )
	{
	    if (strlen ( $file ) > 0 && file_exists ( $file ))
	    {
	        ...
	        if(trim(strtolower($focus->table_name)) == trim(strtolower($table_name))) {
	       		$table_found = true;
				if($dictionary[$focus->object_name]['indices'] != null){
			        foreach($dictionary[$focus->object_name]['indices'] as $indexArray){
			    		if(isset($indexArray['name']) && $indexArray['name']!= null && isset($indexArray['type']) && $indexArray['type']!= null){
			    			if(isset($indexArray['fields']) && $indexArray['fields'] != null){
			    				if(is_array($indexArray['fields'])){
			    					foreach($indexArray['fields'] as $colName){
			    						if(trim(strtolower($column_name)) == trim(strtolower($colName))){
	...
	if(!$table_found){
	    ...
	    foreach ( $rel_dictionary as $rel_name => $rel_data )
	    {
	        if(strtolower($table_name) == $rel_data [ 'table' ]){
				if($rel_data['indices'] != null){
			        foreach($rel_data ['indices'] as $indexArray){
			    		if(isset($indexArray['name']) && $indexArray['name']!= null && isset($indexArray['type']) && $indexArray['type']!= null){
			    			if(isset($indexArray['fields']) && $indexArray['fields'] != null){
			    				if(is_array($indexArray['fields'])){
			    					foreach($indexArray['fields'] as $colName){
...
function checkDupIndeicesInVardef($table_name,$column_name){
	include ('include/modules.php') ;
...
function checkTableComments($table_name){
	include ('include/modules.php') ;
	...
	global $dictionary;
	$commentHeader = "****************************************************************************************** "."\n";
	$commentHeader .= "************** MISSING COMMENTS FOR TABLE ".strtoupper($table_name)." ********************* "."\n";
	$commentHeader .="\n";
	//clear cache before proceeding..
	VardefManager::clearVardef () ;
    $relsDrop = '';
	...
	foreach ( $beanFiles as $bean => $file )
	{
	    if (strlen ( $file ) > 0 && file_exists ( $file ))
	    {
	        ...
	        if(trim(strtolower($focus->table_name)) == trim(strtolower($table_name))) {
	       		$table_found = true;
	       			if(!isset($dictionary[$focus->object_name]['comment']) || $dictionary[$focus->object_name]['comment']==null){
						$commentScan .=" Table  ".strtoupper($table_name)." is a missing comment "."\n";
					}
    				if($dictionary[$focus->object_name]['fields'] != null){
				        foreach($dictionary[$focus->object_name]['fields'] as $key=>$vals){
				    		if(!isset($vals['comment']) || $vals['comment'] == null){
				    			...
				    			$commentScan .=" Column  ".strtoupper($key)." is a missing comment "."\n";
				    		}
				        }
    				}
	       		break;
	...
	if(!$table_found){
	    ...
	    {
	        if(strtolower($table_name) == $rel_data [ 'table' ]){
				if(!isset($rel_data ['comment']) || $rel_data ['comment']==null){
						$commentScan .=" Table  ".strtoupper($table_name)." is a missing comment "."\n";
					}
    				if($rel_data ['fields'] != null){
				        foreach($rel_data ['fields'] as $vals){
				    		if(!isset($vals['comment']) || $vals['comment'] == null){
				    			//add to comment string
				    			$commentScan .=" Column  ".strtoupper($vals['name'])." is a missing comment "."\n";
				    		}
				        }
    				}
				break;
    ...
    if($commentScan != null) {
    	$commentScan .="\n";
		$commentScan .="****************************************************************************************** "."\n";
		return $commentHeader.$commentScan;
    }
	return null;
}
