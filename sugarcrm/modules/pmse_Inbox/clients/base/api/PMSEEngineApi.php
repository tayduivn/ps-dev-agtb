...
class PMSEEngineApi extends SugarApi
{
    ...
    public function getNotes($api, $args)
    {
        ...
        $records = $this->wrapper->getSelectRows($notesBean,'date_entered ASC', $where, 0, -1, -1, $selects, $joinTables);
    ...
    public function engineRoute($api, $args)
    {
        ...
        {
            $message=sprintf(translate('PMSE_LBL_ACTIVITY_STREAM_APPROVE', $params['module_name']), $taskName);
        }elseif($args['frm_action']=='Reject'){
            $message=sprintf(translate('PMSE_LBL_ACTIVITY_STREAM_REJECT', $params['module_name']), $taskName);
        }elseif($args['frm_action']=='Route'){
            $message=sprintf(translate('PMSE_LBL_ACTIVITY_STREAM_ROUTE', $params['module_name']), $taskName);
        }
    ...
    public function engineClaim($api, $args)
    {
        ...
        if ($cas_started != 1) {
            ...
            $params['module_name'] = 'pmse_Inbox';
            $log->activity(sprintf(translate('PMSE_LBL_ACTIVITY_STREAM_CLAIM', $params['module_name']), $taskName), $params);
    ...
    public function adhocReassign($api, $args)
    {
        ...
        $case['not_type'] = isset($case['adhoc_type']) ? $case['adhoc_type'] : '';
        $case['frm_comment'] = $case['adhoc_comment'];
        $case['not_user_recipient_id'] = $case['adhoc_user'];
        $case['idFlow'] = $case['flow_id'];
        $case['idInbox'] = $case['inbox_id'];
        $case['Type'] = 'Adhoc';

        $case['cas_adhoc_type'] = isset($case['adhoc_type']) ? $case['adhoc_type'] : '';
        $flowBeanObject = BeanFactory::getBean('pmse_BpmFlow',$case['flow_id']);
        $case['moduleName'] = $flowBeanObject->cas_sugar_module;
        $case['beanId'] = $flowBeanObject->cas_sugar_object_id;

        $this->caseFlowHandler->saveFormAction($case);
//        $this->engine->saveFormAction($case);
        $userId = $case['adhoc_user'];
        $result = $this->userAssignmentHandler->adhocReassign($case, $userId);
//        $result = $this->engine->adhocReassign($case, $userId);
        ...
        $params['module_name'] = 'pmse_Inbox';
        if($result===true){
            $log->activity(sprintf(translate('PMSE_LBL_ACTIVITY_STREAM_REASSIGN', $params['module_name']), $case['taskName']), $params);
        }
        return $result;
    }
...
//        $userList = $this->engine->getReassignableUserList($args['cas_id'], $args['cas_index']);
    ...
    {
        $resultArray = array();$this->userAssignmentHandler->getAdhocAssignableUserList($args['cas_id'], $args['cas_index']);
...
//        $tmpArray[] = array ('value'=>'is_admin', 'text'=>translate('LBL_PMSE_FORM_OPTION_ADMINISTRATOR'));
        while ($row = $db->fetchByAssoc($result)) {
            ...
            $tmpArray[] = array(
                ...
                'cas_delegate_date' => $time_data->to_display_date_time($row['cas_delegate_date'], true, true, $current_user),
...
//                $resultUpdate = $db->Query($update_activity);
                ...
                $flows = $this->wrapper->getSelectRows($flowBean,'', "cas_id = '$cas_id' and cas_index = '$cas_index'", 0, -1, -1);
                //
    ...
    public function getReassignFlows($api, $args)
    {
        ...
        $q->distinct(false);
        $fields = array('cas_id', 'cas_index', 'cas_delegate_date', 'cas_flow_status', 'cas_user_id', 'cas_due_date', 'bpmn_id');
    ...
    private function getUsersForReassign($options)
    {
        ...
        if (isset($options['act_assignment_method']) &&
            ...
            if ($teamsBean->fetched_row) {
                ...
                $result = $this->getUsersForReassign(array('user_id' => $options['user_id'], 'act_assignment_method' => 'static'));
            }
        } else {
            $teamsBean =  new Team();
