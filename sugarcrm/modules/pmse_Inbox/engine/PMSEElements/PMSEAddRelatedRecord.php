                    ...
                    foreach ($arr_fields as $value) {
                        if (!empty($value->field) && !empty($value->value)) {
                            ...
                            if ($value->type == 'Datetime') {
                                ...
                                switch ($value->value) {
                                    case 'currentuser':
                                        $newValue = $this->beanHandler->mergeBeanInTemplate($bean, $this->userAssignmentHandler->getCurrentUserId());
                                        break;
                                    case 'supervisor':
                                        $newValue = $this->beanHandler->mergeBeanInTemplate($bean, $this->userAssignmentHandler->getSupervisorId($this->getCurrentUserId()));
                                        break;
                                    case 'owner':
                                        $newValue = $this->beanHandler->mergeBeanInTemplate($bean, $this->userAssignmentHandler->getRecordOwnerId($bean->id, $sugarModule));
                                        break;
                                    default:
