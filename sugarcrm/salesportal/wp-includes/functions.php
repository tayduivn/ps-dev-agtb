...
function date_i18n( $dateformatstring, $unixtimestamp = false, $gmt = false ) {
	...
	if ( ( !empty( $wp_locale_object->month ) ) && ( !empty( $wp_locale_object->weekday ) ) ) {
		$datemonth = $wp_locale_object->get_month( $datefunc( 'm', $i ) );
		$datemonth_abbrev = $wp_locale_object->get_month_abbrev( $datemonth );
		$dateweekday = $wp_locale_object->get_weekday( $datefunc( 'w', $i ) );
		$dateweekday_abbrev = $wp_locale_object->get_weekday_abbrev( $dateweekday );
		$datemeridiem = $wp_locale_object->get_meridiem( $datefunc( 'a', $i ) );
		$datemeridiem_capital = $wp_locale_object->get_meridiem( $datefunc( 'A', $i ) );
...
function number_format_i18n( $number, $decimals = null ) {
	...
	$decimals = ( is_null( $decimals ) ) ? $wp_locale_object->number_format['decimals'] : intval( $decimals );

	$num = number_format( $number, $decimals, $wp_locale_object->number_format['decimal_point'], $wp_locale_object->number_format['thousands_sep'] );
...
function wp_die( $message, $title = '', $args = array() ) {
	...
	if ( ( $wp_locale ) && ( 'rtl' == $wp_locale_object->text_direction ) ) $text_direction = 'rtl';
?>
...
function _mce_set_direction( $input ) {
	...
	if ( 'rtl' == $wp_locale_object->text_direction ) {
...
function wp_timezone_choice( $selected_zone ) {
	...
	if ( !$mo_loaded ) {
		...
		$mofile = WP_LANG_DIR . '/continents-cities-' . $wp_locale . '.mo';
	...
	foreach ( timezone_identifiers_list() as $zone ) {
		...
		$zonen[] = array(
			...
			't_continent' => ( $exists[3] ? wp_translate( str_replace( '_', ' ', $zone[0] ), 'continents-cities' ) : '' ),
			't_city'      => ( $exists[4] ? wp_translate( str_replace( '_', ' ', $zone[1] ), 'continents-cities' ) : '' ),
			't_subcity'   => ( $exists[5] ? wp_translate( str_replace( '_', ' ', $zone[2] ), 'continents-cities' ) : '' )
		);
	}
