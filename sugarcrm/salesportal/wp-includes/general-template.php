...
function get_bloginfo($show = '', $filter = 'raw') {
	...
	switch($show) {
		...
		case 'text_direction':
			global $wp_locale;
			$output = $wp_locale_object->text_direction;
			break;
		case 'name':
		default:
...
function wp_title($sep = '&raquo;', $display = true, $seplocation = '') {
	...
	if ( !empty($m) ) {
		...
		$my_month = $wp_locale_object->get_month(substr($m, 4, 2));
	...
	if ( !empty($year) ) {
		$title = $year;
		if ( !empty($monthnum) )
			$title .= "$t_sep" . $wp_locale_object->get_month($monthnum);
		if ( !empty($day) )
...
function single_month_title($prefix = '', $display = true ) {
	...
	if ( !empty($monthnum) && !empty($year) ) {
		$my_year = $year;
		$my_month = $wp_locale_object->get_month($monthnum);
	} elseif ( !empty($m) ) {
		$my_year = substr($m, 0, 4);
		$my_month = $wp_locale_object->get_month(substr($m, 4, 2));
	}
	...
	if ( 'monthly' == $type ) {
		...
		if ( $arcresults ) {
			...
			foreach ( (array) $arcresults as $arcresult ) {
				...
				$text = sprintf(__('%1$s %2$d'), $wp_locale_object->get_month($arcresult->month), $arcresult->year);
				if ( $show_post_count )
...
function get_calendar($initial = true) {
	...
	<caption>' . sprintf($calendar_caption, $wp_locale_object->get_month($thismonth), date('Y', $unixmonth)) . '</caption>
	<thead>
	<tr>';
	...
	for ( $wdcount=0; $wdcount<=6; $wdcount++ ) {
		$myweek[] = $wp_locale_object->get_weekday(($wdcount+$week_begins)%7);
	}

	foreach ( $myweek as $wd ) {
		$day_name = (true == $initial) ? $wp_locale_object->get_weekday_initial($wd) : $wp_locale_object->get_weekday_abbrev($wd);
	...
	if ( $previous ) {
		echo "\n\t\t".'<td abbr="' . $wp_locale_object->get_month($previous->month) . '" colspan="3" id="prev"><a href="' .
		get_month_link($previous->year, $previous->month) . '" title="' . sprintf(__('View posts for %1$s %2$s'), $wp_locale_object->get_month($previous->month),
			date('Y', mktime(0, 0 , 0, $previous->month, 1, $previous->year))) . '">&laquo; ' . $wp_locale_object->get_month_abbrev($wp_locale_object->get_month($previous->month)) . '</a></td>';
	} else {
	...
	if ( $next ) {
		echo "\n\t\t".'<td abbr="' . $wp_locale_object->get_month($next->month) . '" colspan="3" id="next"><a href="' .
		get_month_link($next->year, $next->month) . '" title="' . sprintf(__('View posts for %1$s %2$s'), $wp_locale_object->get_month($next->month),
			date('Y', mktime(0, 0 , 0, $next->month, 1, $next->year))) . '">' . $wp_locale_object->get_month_abbrev($wp_locale_object->get_month($next->month)) . ' &raquo;</a></td>';
	} else {
...
function the_weekday() {
	...
	$the_weekday = $wp_locale_object->get_weekday(mysql2date('w', $post->post_date, false));
...
function the_weekday_date($before='',$after='') {
	...
	if ( $day != $previousweekday ) {
		...
		$the_weekday_date .= $wp_locale_object->get_weekday(mysql2date('w', $post->post_date, false));
