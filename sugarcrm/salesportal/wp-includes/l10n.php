 ...
 * @uses $wp_locale Gets the locale stored in the global.
 *
...
function get_locale() {
	...
	if (isset($wp_locale))
		return apply_filters( 'locale', $wp_locale );
 ...
 *
 * @see __() Don't use wp_translate() directly, use __()
 * @since 2.2.0
 ...
 */
function wp_translate( $text, $domain = 'default' ) {
...
/**
 * Translates $text like wp_translate(), but assumes that the text
...
function translate_with_context( $text, $domain = 'default' ) {
	return before_last_bar( wp_translate( $text, $domain ) );
 ...
 * the domain isn't loaded the original text is returned.
 *
 * @see wp_translate() An alias of wp_translate()
 * @since 2.1.0
 *
 * @param string $text Text to translate
...
function esc_attr__( $text, $domain = 'default' ) {
	return esc_attr( wp_translate( $text, $domain ) );
}
...
function esc_html__( $text, $domain = 'default' ) {
	return esc_html( wp_translate( $text, $domain ) );
}

/**
 * Displays the returned translated text from wp_translate().
 *
 * @see wp_translate() Echoes returned wp_translate() string
 * @since 1.2.0
 *
 * @param string $text Text to translate
 * @param string $domain Optional. Domain to retrieve the translated text
...
function esc_attr_e( $text, $domain = 'default' ) {
	echo esc_attr( wp_translate( $text, $domain ) );
}
...
function esc_html_e( $text, $domain = 'default' ) {
	echo esc_html( wp_translate( $text, $domain ) );
}
...
function load_default_textdomain() {
	...
	$mofile = WP_LANG_DIR . "/$wp_locale.mo";
...
function load_plugin_textdomain($domain, $abs_rel_path = false, $plugin_rel_path = false) {
	...
	$mofile = $path . '/'. $domain . '-' . $wp_locale . '.mo';
