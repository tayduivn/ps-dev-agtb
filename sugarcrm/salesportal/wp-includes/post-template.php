...
/* DEE CUSTOMIZATION 080623 - SALES PORTAL NAVIGATION */

function get_pagelist($page_id)
{
  global $wpdb;
  $parent_id = array();
  $parent_pages =  $wpdb->get_results("SELECT * from wp_posts where post_parent=0 and post_type='page' and ID!=3 and post_status = 'publish' order by menu_order");


  /* for home page */
  if(empty($page_id))
  {
    echo "<li><h3><a href='index.php'>".get_the_title(0)."</a></h3></li>";

    foreach($parent_pages as $ppage)
    {
      echo "<li><a href='index.php?page_id=".$ppage->ID."'>".$ppage->post_title."</a></li>";
    }
  }
  /* end - for home page */
  ...
  if(!empty($page_id))
  {
      ...
      /* if selected item is a parent item */
      if(in_array($page_id, $parent_id))
      {
        echo "<li class='pagenav'><h3><a href='index.php?page_id=".$page_id."'>".get_the_title($page_id)."</a></h3></li>";
        $childs =  $wpdb->get_results("SELECT * from wp_posts where post_parent='$page_id' and post_type='page' and ID != 3 and post_status = 'publish' order by menu_order");
        foreach($childs as $child)
        {
            if($_REQUEST['page_id'] == $child->ID)
            { $style = "id=subcurrent"; }
            else
            { $style = ""; }
            echo "<li><a href='index.php?page_id=".$child->ID."' $style>".$child->post_title."</a></li>";

            $sub_childs =  $wpdb->get_results("SELECT * from wp_posts where post_parent='$child->ID' and post_type='page' and ID!=3 and post_status = 'publish' order by  menu_order");
            if($sub_childs) {
              foreach($sub_childs as $sub_child)
                 {
                if($_REQUEST['page_id'] == $sub_child->ID)
                { $style = "id=subcurrent"; }
             else
                { $style = ""; }
                echo "<li id='subchild'><a href='index.php?page_id=".$sub_child->ID."' $style>".$sub_child->post_title."</a></li>";
              }
            }
        }
      }

      /* if selected item is a child item */
      else if(!in_array($page_id, $parent_id))
      {
         $child_parent =  $wpdb->get_results("SELECT post_parent from wp_posts where ID='$page_id' AND post_type='page' and post_status = 'publish'");
         foreach($child_parent as $cp)
         {
            get_pagelist($cp->post_parent);
         }
      }
  }
}
