...
$server->register(
    'create_DCE_action',
    array('session'=>'xsd:string', 'record'=>'xsd:string',  'actionType'=>'xsd:string', 'startDate' => 'xsd:string', 'priority' => 'xsd:int', 'upgradeVars' => 'xsd:string', 'dbCloned' => 'xsd:int'),
    array('return'=>'xsd:string'),
    $NAMESPACE);
/**
 * This function will create an action depending of the parameter
 * check the create_action function in DCEInstance class.
 *
 * @param string $session : the session id
 * @param string $record : id of the instance
 * @param string $actionType : "create", ...
 * @param string $startDate : date
 * @param string $priority
 * @param string $upgradeVars
 * @param bool $dbCloned
 * @return Status of the instance after the action has been created
 */
function create_DCE_action($session, $record, $actionType, $startDate='',$priority='',$upgradeVars='',$dbCloned=false){
    if(!validate_authenticated($session)){
...
$server->register(
    'return_when_found',
    array('session'=>'xsd:string', 'module_name'=>'xsd:string',  'id'=>'xsd:string', 'field' => 'xsd:string', 'values' => 'tns:select_fields', 'wait' => 'xsd:int', 'sleep' => 'xsd:int'),
    array('return'=>'xsd:string'),
    $NAMESPACE);

/**
 * This function check the "$field" field of the "$id" entry from "$module_name" module
 * every "$sleep" secondes during a maximum of "$wait" secondes. If the value of the field
 * equal one the value of the $values array it will return this value.
 *
 * @param string $session : the session id
 * @param string $module_name : name of the module where the entry is
 * @param string $id : id of the entry to check
 * @param string $field : name of the field to check
 * @param array $values : array of value which will return if match
 * @param int $wait : max wait time in seconds
 * @param int $sleep : interval between two check in seconds
 * @return string : the value which match
 */

function return_when_found($session, $module_name, $id, $field, $values, $wait, $sleep){
	$error = new SoapError();
    if(!validate_authenticated($session)){
        $error->set_error('invalid_login');
        return array('id'=>-1, 'error'=>$error->get_soap_array());
    }
	$select_fields = array($field);
	$time = time();
    ini_set("max_execution_time", $wait+10);
	do{
		$return = get_entry($session, $module_name, $id,$select_fields );
		if($return['entry_list'][0]['error']['number']!=0){
	        $error->set_error($return['entry_list'][0]['error']['name']);
	        return '-1';
		}
		if(in_array($return['entry_list'][0]['name_value_list'][$field]['value'], $values)){
			return $return['entry_list'][0]['name_value_list'][$field]['value'];
		}
		sleep($sleep);
...
$server->register(
    'dce_update_license',
    array('session'=>'xsd:string', 'instance_id'=>'xsd:string', 'contact_id'=>'xsd:string', 'deploy' => 'xsd:string', 'enable' => 'xsd:int', 'disable' => 'xsd:int'),
    array('return'=>'tns:set_entry_result'),
    $NAMESPACE);

/**
 * This function retrieve the Instance and the Contact records and create an input_array
 * from the values of these records. action_dce_get_key is call with the array and a license key
 * is created or updated. An "create" or "key" action is created for the instance.
 *
 * @param string $session : the session id REQUIRED
 * @param string $instance_id : id of the Instance REQUIRED
 * @param string $contact_id : id of the contact REQUIRED
 * @param int $deploy : 0 if we don't want to deploy the instance if it's a "new" instance (default : 1)
 * @param int $enable : 1 if we want to enable the key (default : 0)
 * @param int $disable : 1 if we want to disable the key (default : 0)
 * @return array : "Success" and a message on success and the error.
 */

function dce_update_license($session, $instance_id, $contact_id, $deploy, $enable, $disable){
    if(!isset($deploy) || (isset($deploy) && ($deploy == 'true' || $deploy == 1 || $deploy == '1'))){
        $deploy=1;
    }else{
        $deploy=0;
    }
    if(!isset($enable) || (isset($enable) && ($enable == 'false' || $enable == 0 || $enable == '0'))){
        $enable=0;
    }else{
        $enable=1;
    }
    if(!isset($disable) || (isset($disable) && ($disable == 'false' || $disable == 0 || $disable == '0'))){
        $disable=0;
    }else{
        $disable=1;
    }
    ...
    if(empty($instance_id) || empty($contact_id)){
        $error->set_error('DCE_missing_parameter');
        return array('id'=>-1, 'error'=>$error->get_soap_array());
    }
    if($enable && $disable){
        $error->set_error('DCE_enable_and_disable');
        return array('id'=>-1, 'error'=>$error->get_soap_array());
    }
    global $current_user;
    ...
    if($inst->retrieve($instance_id) && $ct->retrieve($contact_id)){
        ...
        $ct->set_relationship('dceinstances_contacts', array('contact_id'=>$ct->id ,'instance_id'=> $instance_id), false);
        if($inst->status == 'new'){
            $actionType = 'create';
            if($deploy == 0){
                $createAction=false;
        ...
        if(empty($inst->license_key)){
            if($enable || $disable){
                $error->set_error('DCE_empty_license_key_with_disable');
                return array('id'=>-1, 'error'=>$error->get_soap_array());
            }
        ...
        $license_details = array('disable_license'=>false, 'enable_license'=>false, 'license_key'=>'', 'license_start_date'=>'', 'license_duration'=>'', 'licensed_users'=>'', 'type'=>'', 'name'=>'');
        foreach($license_details as $k=>$v){
            if(isset($inst->$k)){
                $license_details[$k]=$inst->$k;
            }
        }

        $tpl = new DCETemplate();
        $tpl->retrieve($inst->dcetemplate_id);
        $license_details['sugar_version'] = $tpl->sugar_version;
        $license_details['sugar_edition'] = $tpl->sugar_edition;
        ...
        require_once('modules/DCEInstances/controller.php');
        $ctrl = new DCEInstancesController();
        $result=$ctrl->action_dce_get_key($input_array);
        if(isset($result['error']['number']) && $result['error']['number'] == 0){
            $inst->license_key = $result['get_key_result']['license_key'];
            if(isset($result['get_key_result']['license_key_status'])){
                $inst->license_key_status = $result['get_key_result']['license_key_status'];
            }
            $inst->save();
            if($createAction){
                $inst->create_action($instance_id, $actionType, '','','',false);
            }
            $return=$instance_id;
            return array('id'=>$return, 'error'=>$error->get_soap_array());
        }else if(isset($result['error']['number'])){
            return array('id'=>-1, 'error'=>$result['error']);
        }else{
            $error->set_error('DCE_action_dce_get_key_failure');
            return array('id'=>-1, 'error'=>$error->get_soap_array());
        }
    }else{
        $error->set_error('DCE_record_not_found');
        return array('id'=>-1, 'error'=>$error->get_soap_array());
    }
