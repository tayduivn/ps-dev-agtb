...
nv.models.tree = function() {
  ...
  */
  ...
  var r = 5.5
    ...
    , nodeSize = { 'width': 115, 'height': 42 }       // the size of a node
    , nodeImgPath = '../img/'
  ;
  ...
  function chart(selection)
  {
    ...
    function(data) {
      ...
      var zoom = 1          // chart zoom scale; set by zoomer
        , trans = [ 0, 0 ]  // chart translation; set by zoomer
        , scale = 1         // initial chart zoom scale to fit chart in container; set by chart.update
        , shift = [ 0, 0 ]  // initial chart translation to position chart in the center of container; set by chart.update
        , bbox = false      // the actual size of a node
        , offset = { 'top': padding.top + nodeSize.height, 'left': padding.left }  // this is needed because the origin of a node is at the bottom
        , svgSize = { 'width': 0, 'height': 0 }       // the size of the svg container
        , availableSize = { 'width': 0, 'height': 0 } // the size of the svg container minus padding
        , chartSize = { 'width': 0, 'height': 0 }     // the size of the chart itself
      ;
      ...
      var wrapEnter = wrap.enter().append('g')
            ...
            .on('zoom', function() {
              ...
              treeChart.attr('transform',
                'translate('+
                  //[ trans[0] + (offset.left + shift[0]) * scale , trans[1] + (offset.top + shift[1]) * scale ]
                  trans
                +')scale('+
                  scale * zoom
                +')'
      ...
      var treeChart = wrapEnter.append('g')
            .attr('class', 'nv-tree')
            .attr('id', 'vis');
      ...
      chart.update = function(source) {
        ...
        scale = d3.min([ availableSize.width/chartSize.width, availableSize.height/chartSize.height ])

        if (availableSize.width/chartSize.width < availableSize.height/chartSize.height) {
          //width controls, set center height
          shift = [ 0, ((availableSize.height/scale*zoom)-chartSize.height)/2 ];
        } else {
          shift = [ ((availableSize.width/scale*zoom)-chartSize.width)/2, 0 ];
        }

        // console.log('availableSize.height',availableSize.height);
        // console.log('chartHeigth',chartSize.height);
        // console.log('shift',shift);
        // console.log('trans[1]',trans[1]);
        // console.log('offset.top',offset.top);
        // cconsole.log('scale',scale);
        // console.log('============');
