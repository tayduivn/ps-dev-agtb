...
const { JSDOM } = jsdom;
// let serverUrl = process.env.SERVER_CUCUMBER_URL;
let serverUrl = 'https://pkoushik810ent.odd.sugarcrm.io/';
const { window } = new JSDOM(`<!DOCTYPE html>`,{ url: `${serverUrl}/` });
 ...
 * @see  http://zeptojs.com/#jQuery.ajax
 *
 * @class HttpError
 */
...
_.extend(HttpRequest.prototype, {
    ...
    execute: function(token, mdHash, upHash) {
        ...
        if (this.debug) {
            ...
            var p = jQuery.extend({}, this.params);
            delete p.data;
         ...
         * @instance
         */
        this.xhr = jQuery.ajax(this.params);
    }
});
...
function SugarApi(args) {
    ...
    if (_keyValueStore) {
        if (!jQuery.isFunction(_keyValueStore.set) ||
            !jQuery.isFunction(_keyValueStore.get) ||
            !jQuery.isFunction(_keyValueStore.cut))
        {
        ...
        if (jQuery.isFunction(_keyValueStore.initAsync)) {
            _keyValueStore.initAsync(init);
        } else {
            init();
        }

        if (jQuery.isFunction(_keyValueStore.on)) {
        ...
        return function(xhr, textStatus, errorThrown) {
            ...
            var onError = function() {
                ...
                if (!_rqueue.length || self.refreshingToken(request.params.url)) {
                    ...
                    else if (jQuery.isFunction(self.defaultErrorHandler)) {
    ...
    return {
        ...
        buildURL: function(module, action, attributes, params) {
            ...
            if (action && jQuery.inArray(action, _baseActions) === -1) {
                parts.push(action);
            }
         ...
         * @param {Object} [jQueryfiles] jQuery/Zepto DOM elements that carry the files to upload.
         * @param {Object} [callbacks] callback object.
         ...
         * @instance
         */
        file: function(method, data, jQueryfiles, callbacks, options) {
            var ajaxParams = {
                files: jQueryfiles,
                processData: false
            };
         ...
         * @param {jQuery} jQueryel JQuery selector to element.
         ...
         * @instance
         */
        exportRecords: function(params, jQueryel, callbacks, options) {
            var self = this;
        ...
        handleExternalLogin: function(request, error, onError) {
            ...
            jQuery(window).on('message', function(event) {
                ...
                jQuery(window).on('message', null);
                var authData = jQuery.parseJSON(event.originalEvent.data);
