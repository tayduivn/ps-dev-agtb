...
Feature: History dashlet verification
  ...
  Scenario: History dashlet verification
    ...
    Given 3 Meetings records exist:
      | *          | name              | status | date_start      |
      | T{{index}} | Meeting {{index}} | Held   | now -{{index}}d |

    # Create 1 meeting record that held in last 30 days
    Given 1 Meetings records exist:
      | *  | name      | status | date_start |
      | T4 | Meeting 4 | Held   | now -10d   |

    # Create 1 meeting record that held in last quarter
    Given 1 Meetings records exist:
      | *  | name      | status | date_start |
      | T5 | Meeting 5 | Held   | now -60d   |

    # Create 3 call records that held in last 7 days
    Given 3 Calls records exist:
      | *          | name           | status | date_start      | duration_hours | duration_minutes |
      | C{{index}} | Call {{index}} | Held   | now -{{index}}d | 0              | 30               |

    # Create 1 call record that held in last 30 days
    Given 1 Calls records exist:
      | *  | name   | status | date_start | duration_hours | duration_minutes |
      | C4 | Call 4 | Held   | now -20d   | 0              | 30               |

    # Create 1 call record that held in last quarter
    Given 1 Calls records exist:
      | *  | name   | status | date_start | duration_hours | duration_minutes |
      | C5 | Call 5 | Held   | now -80d   | 0              | 30               |
