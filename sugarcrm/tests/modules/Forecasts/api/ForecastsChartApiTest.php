...
class ForecastsChartApiTest extends RestTestBase
{
    ...
    public static function setUpBeforeClass()
    {
		...
		$product->retrieve_by_string_fields(array("opportunity_id"=>$opp1->id));
		$product->name = $opp1->name;
        $product->best_case = $opp1->best_case;
        $product->likely_case = $opp1->amount;
        $product->worst_case = $opp1->worst_case;
        $product->cost_price = $opp1->amount;
        $product->quantity = 1;
        $product->currency_id = $opp1->currency_id;
        $product->base_rate = $opp1->base_rate;
        $product->probability = $opp1->probability;
        $product->date_closed = $opp1->date_closed;
        $product->date_closed_timestamp = $opp1->date_closed_timestamp;
        $product->assigned_user_id = $opp1->assigned_user_id;
        $product->opportunity_id = $opp1->id;
        $product->commit_stage = $opp1->commit_stage;
        $product->save();
