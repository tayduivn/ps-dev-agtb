...
class ForecastsCommittedApiTest extends RestTestBase
{
     ...
     * @group forecasts
     */
    public function testForecastsMgrNeedsCommitted()
    {
    	$manager = SugarTestForecastUtilities::createForecastUser(array("createForecast"=>false));
    	$timeperiod = SugarTestForecastUtilities::getCreatedTimePeriod();
    	$tempUser = $GLOBALS["current_user"] = $this->_user;
    	...
    	//initial call w/o any forecasts
    	$response = $this->_restCall("Forecasts/committed/mgrNeedsCommitted?user_id=" . $manager["user"]->id. "&timeperiod_id=" . $timeperiod->id);

    	//needsCommitted should be false in this case
        $this->assertEquals(false, $response["reply"]["needsCommitted"], "needsCommitted should be false");
        ...
        //Now make a mgrNeedsCommitted call with an empty mgr sheet, and a new rep view sheet
        $response = $this->_restCall("Forecasts/committed/mgrNeedsCommitted?user_id=" . $manager["user"]->id. "&timeperiod_id=" . $timeperiod->id);
         
         //needsCommitted should be true in this case
        $this->assertEquals(true, $response["reply"]["needsCommitted"], "needsCommitted should be true");
        ...
        //Now make a mgrNeedsCommitted call with an old mgr sheet, and a new rep view sheet
        $response = $this->_restCall("Forecasts/committed/mgrNeedsCommitted?user_id=" . $manager["user"]->id. "&timeperiod_id=" . $timeperiod->id);
