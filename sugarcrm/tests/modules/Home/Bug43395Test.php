...
class Bug43395Test extends Sugar_PHPUnit_Framework_OutputTestCase
{
    ...
    public function testPersonLocaleNameFormattting()
    {
    	...
    	$this->assertEquals('Mr. Bug43395 Test', self::$contact->name, 'Assert that _create_proper_name_field with createLocaleFormattedName set to true returns salutation');

    	self::$contact->createLocaleFormattedName = false;
    	self::$contact->_create_proper_name_field();
    	$this->assertEquals('Bug43395 Test', self::$contact->name, 'Assert that _create_proper_name_field with createLocaleFormattedName set to false does not return salutation');
    }
