...
require_once 'modules/KBSContents/clients/base/api/KBSContentsUsefulnessApi.php';

/**
 * Tests for KBSContentsUsefulnessApi
 */
class KBSContentsUsefulnessApiTest extends Sugar_PHPUnit_Framework_TestCase
{
    /**
     * @var RestService
     */
    ...
    public function setUp()
    {
        ...
        $this->api = new KBSContentsUsefulnessApi();

        $this->kbscontent = BeanFactory::newBean('KBSContents');
        $this->kbscontent->name = 'SugarKBSContent' . time();
        $this->kbscontent->save();
    ...
    public function tearDown()
    {
        DBManagerFactory::getInstance()
            ->query('DELETE FROM kbscontents WHERE id \'' . $this->kbscontent->id . '\'');
    ...
    {
        $this->assertEquals(0, $this->kbscontent->useful);
        for ($i = 1; $i <= 3; $i++) {
            $result = $this->api->voteUseful(
                $this->service,
                array(
                    'module' => 'KBSContents',
                    'record' => $this->kbscontent->id
                )
            );
            ...
            $this->assertEquals($this->kbscontent->id, $result['id']);
    ...
    {
        $beanMock = $this->getMock('KBSContents', array('ACLAccess'));
        $beanMock->expects($this->once())
            ->method('ACLAccess')
            ->will($this->returnValue(false));

        $apiMock = $this->getMock('KBSContentsUsefulnessApi', array('loadBean'));
