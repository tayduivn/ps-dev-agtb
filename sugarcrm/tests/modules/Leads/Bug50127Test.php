...
class Bug50127Test extends Sugar_PHPUnit_Framework_OutputTestCase
{
    ...
    public function testConvertLinkingExistingContact() {
        ...
        $this->assertEquals($contact->id, $lead->contact_id, 'Contact linked with Lead successfully.');
        // Check if account is linked with lead properly
        $this->assertEquals($account->id, $lead->account_id, 'Account linked with Lead successfully.');
        // Check if account is linked with contact properly        
        $this->assertEquals($account->id, $contact->account_id, 'Account linked with Contact successfully.');
