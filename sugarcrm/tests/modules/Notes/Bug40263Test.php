...
class Bug40263Test extends Sugar_PHPUnit_Framework_TestCase
{
	...
	public function testGetListViewQueryCreatedBy()
    {
    	...
    	$regex = "/select.*jt\d.created_by.*LEFT JOIN\s*users jt\d ON\s*jt\d\.id\s*=\s*notes.created_by.*/si";
    	return $this->assertRegExp($regex, $query, "Unable to find the created user in the notes list view query");
    }
