...
class Bug43680Test extends Sugar_PHPUnit_Framework_TestCase
{

var $dbType;	
var $accountsCustomTableExists = false;
var $hasCustomIdColumn = false;
...
/*
	...
	//Save the original dbType
	$this->dbType = $GLOBALS['db']->dbType;
	
	$contact = SugarTestContactUtilities::createContact();
	$account = SugarTestAccountUtilities::createAccount();
	
	$this->accountsCustomTableExists = $GLOBALS['db']->tableExists('accounts_cstm');
	
	if(!$this->accountsCustomTableExists)
	{
	    $GLOBALS['db']->createTableParams('accounts_cstm',
						           		array(
						                'id_c' => array (
						                    'name' => 'id_c',
						                    'type' => 'id',
	    ...
	    $GLOBALS['db']->query("INSERT INTO accounts_cstm (id_c,contact_id_c) VALUES ('{$account->id}','{$contact->id}')");
	} else {
		$columns = $GLOBALS['db']->getHelper()->get_columns('accounts_cstm');
	    $this->hasCustomIdColumn = isset($columns['contact_id_c']);
        if(!$this->hasCustomIdColumn)
        {		
		    $GLOBALS['db']->addColumn('accounts_cstm',
		            	array(
		                	'contact_id_c' => array (
			                    'name' => 'contact_id_c',
			                    'type' => 'id',
    ...
    global $current_language, $app_strings, $app_list_strings, $mod_strings;
*/
...
<?php
\$dictionary['Account']['fields']['related_c']['dependency']='';
EOQ;
        create_custom_directory("Extension/modules/Accounts/Ext/Vardefs/sugar_field_related_c.php");
        $fileLoc = "custom/Extension/modules/Accounts/Ext/Vardefs/sugar_field_related_c.php";
        $this->createdFiles[] = "custom/Extension/modules/Accounts/Ext/Vardefs/sugar_field_related_c.php";
        file_put_contents($fileLoc, $extensionContent);
        $_REQUEST['repair_silent']=1;
        $rc = new RepairAndClear();
        $rc->repairAndClearAll(array("rebuildExtensions", "clearVardefs"), array("Accounts"),  false, false);	
	
        */
	
        $extensionContent = <<<EOQ
<?php
// created: 2011-05-09 18:09:57
...
EOQ;
    ...
    if(!file_exists('cache/modules/Accounts'))
    {
       mkdir_recursive('cache/modules/Accounts');	
    }
    
    $fileLoc = 'cache/modules/Accounts/Accountvardefs.php';
    file_put_contents($fileLoc, $extensionContent);        
...
public function tearDown()
{
    ...
    //Drop table if it didn't exist
    if(!$this->accountsCustomTableExists)
    {
    	$GLOBALS['db']->dropTableName('accounts_cstm');
    } else if(!$this->hasCustomIdColumn) {

    	$GLOBALS['db']->deleteColumn(new Account(), 	            	
					    				array(
						                	'contact_id_c' => array (
							                    'name' => 'contact_id_c',
							                    'type' => 'id',
	...
	unlink('cache/modules/Accounts/Accountvardefs.php');

}
	
	 
public function testMssqlMatrixQuery()
{
	$_REQUEST = array();
	$_REQUEST['action'] = 'ReportCriteriaResult';
	$_REQUEST['module'] = 'Reports';
	$_REQUEST['page'] = 'report';
	$_REQUEST['globalLinksOpen'] = true;
	
	$id =  'bud43680' . mt_rand();
	$_REQUEST['id'] = $id;
	
	$query = "INSERT INTO saved_reports (team_id, team_set_id, id, name, module, report_type, content, deleted, assigned_user_id, chart_type, schedule_type, favorite) values 
	                              ('1', '1', '{$id}', 'Bug 43680', 'Accounts', 'Matrix', '{\"display_columns\":[],\"module\":\"Accounts\",\"group_defs\":[{\"name\":\"account_type\",\"label\":\"Type\",\"table_key\":\"self\",\"type\":\"enum\"},{\"name\":\"related_c\",\"label\":\"Related\",\"table_key\":\"self\",\"type\":\"relate\"}],\"summary_columns\":[{\"name\":\"account_type\",\"label\":\"Type\",\"table_key\":\"self\"},{\"name\":\"related_c\",\"label\":\"Related\",\"table_key\":\"self\"},{\"name\":\"count\",\"label\":\"Count\",\"field_type\":\"\",\"group_function\":\"count\",\"table_key\":\"self\"}],\"report_name\":\"Bug 43680 Report\",\"chart_type\":\"hBarF\",\"do_round\":1,\"chart_description\":\"\",\"numerical_chart_column\":\"self:count\",\"numerical_chart_column_type\":\"\",\"assigned_user_id\":\"1\",\"report_type\":\"summary\",\"layout_options\":\"2x2\",\"full_table_list\":{\"self\":{\"value\":\"Accounts\",\"module\":\"Accounts\",\"label\":\"Accounts\"}},\"filters_def\":{\"Filter_1\":{\"operator\":\"AND\",\"0\":{\"name\":\"name\",\"table_key\":\"self\",\"qualifier_name\":\"not_empty\",\"input_name0\":\"not_empty\",\"input_name1\":\"on\"}}}}',
	                               0, '1', 'hBarF', 'pro', 0)";
	...
	$reportObj = new Bug43680MockReport($saved_report_seed->content);
	...
	$args['reporter'] = new Bug43680MockReport($saved_report_seed->content);
...
class Bug43680MockReport extends Report
{
    ...
    function run_query($do_group_by=false)
    {
        ...
        return $this->execute_query('query', 'result', 'row_count', 'row_start', 'row_end', $limit);
    }
