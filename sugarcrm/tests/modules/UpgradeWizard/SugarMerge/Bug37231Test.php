...
function test_accounts_editview_merge() {		
   ...
   foreach($viewdefs['Accounts']['EditView']['panels']['lbl_account_information'] as $row) {
      	foreach($row as $col_key=>$col) {
   	  	 	$id = is_array($col) && isset($col['name']) ? $col['name'] : $col;
            if($id == 'team_name') {
   ...
   $this->assertTrue($found_team_name, "Assert that team_name is present in lbl_account_information panel");
