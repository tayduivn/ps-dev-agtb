...
class RestTestMetadataSugarViews extends RestTestBase {
    ...
    public function testMetadataSugarViews() {
        $restReply = $this->_restCall('metadata?typeFilter=sugarViews');

        $this->assertTrue(isset($restReply['reply']['sugarViews']['_hash']),'SugarView hash is missing.');
    }
    ...
    public function testMetadataSugarViewsController() {
        ...
        $restReply = $this->_restCall('metadata/?typeFilter=sugarViews&platform=mobile');
        $this->assertEquals('MOBILE CODE',$restReply['reply']['sugarViews']['address']['controller'],"Didn't get mobile code when that was the direct option");
        ...
        $restReply = $this->_restCall('metadata/?typeFilter=sugarViews&platform=mobile');
        $this->assertEquals('MOBILE CODE',$restReply['reply']['sugarViews']['address']['controller'],"Didn't get mobile code when base code was there.");


        // Make sure we get the base code when we ask for it.
        $restReply = $this->_restCall('metadata/?typeFilter=sugarViews&platform=base');
        $this->assertEquals('BASE CODE',$restReply['reply']['sugarViews']['address']['controller'],"Didn't get base code when it was the direct option");


        // Delete the mobile address and make sure it falls back to base
        unlink('clients/mobile/views/address/address.js');
        $restReply = $this->_restCall('metadata/?typeFilter=sugarViews&platform=mobile');
        $this->assertEquals('BASE CODE',$restReply['reply']['sugarViews']['address']['controller'],"Didn't fall back to base code when mobile code wasn't there.");
        ...
        $restReply = $this->_restCall('metadata/?typeFilter=sugarViews&platform=mobile');
        $this->assertEquals('CUSTOM MOBILE CODE',$restReply['reply']['sugarViews']['address']['controller'],"Didn't use the custom mobile code.");

        // Make sure custom portal code works
        file_put_contents('custom/clients/portal/views/address/address.js','CUSTOM PORTAL CODE');
        $restReply = $this->_restCall('metadata/?typeFilter=sugarViews&platform=portal');
        $this->assertEquals('CUSTOM PORTAL CODE',$restReply['reply']['sugarViews']['address']['controller'],"Didn't use the custom portal code.");
    ...
    public function testMetadataSugarViewsTemplates() {
        ...
        $restReply = $this->_restCall('metadata/?typeFilter=sugarViews&platform=mobile');
        $this->assertEquals('MOBILE EDITVIEW',$restReply['reply']['sugarViews']['address']['templates']['editView'],"Didn't get mobile code when that was the direct option");


        // Make sure we get it when we ask for mobile, even though there is base code there
        file_put_contents('clients/base/views/address/editView.hbt','BASE EDITVIEW');
        $restReply = $this->_restCall('metadata/?typeFilter=sugarViews&platform=mobile');
        $this->assertEquals('MOBILE EDITVIEW',$restReply['reply']['sugarViews']['address']['templates']['editView'],"Didn't get mobile code when base code was there.");


        // Make sure we get the base code when we ask for it.
        $restReply = $this->_restCall('metadata/?typeFilter=sugarViews&platform=base');
        $this->assertEquals('BASE EDITVIEW',$restReply['reply']['sugarViews']['address']['templates']['editView'],"Didn't get base code when it was the direct option");


        // Delete the mobile address and make sure it falls back to base
        unlink('clients/mobile/views/address/editView.hbt');
        $restReply = $this->_restCall('metadata/?typeFilter=sugarViews&platform=mobile');
        $this->assertEquals('BASE EDITVIEW',$restReply['reply']['sugarViews']['address']['templates']['editView'],"Didn't fall back to base code when mobile code wasn't there.");


        // Make sure the mobile code is loaded before the non-custom base code
        file_put_contents('custom/clients/mobile/views/address/editView.hbt','CUSTOM MOBILE EDITVIEW');
        $restReply = $this->_restCall('metadata/?typeFilter=sugarViews&platform=mobile');
        $this->assertEquals('CUSTOM MOBILE EDITVIEW',$restReply['reply']['sugarViews']['address']['templates']['editView'],"Didn't use the custom mobile code.");

        // Make sure custom base code works
        file_put_contents('custom/clients/base/views/address/editView.hbt','CUSTOM BASE EDITVIEW');
        $restReply = $this->_restCall('metadata/?typeFilter=sugarViews&platform=base');
        $this->assertEquals('CUSTOM BASE EDITVIEW',$restReply['reply']['sugarViews']['address']['templates']['editView'],"Didn't use the custom base code.");
    }
