...
describe('Base.View.Dashablerecord', function() {
    ...
    describe('getContainerWidth', function() {
        var btnBar;
        var titleBar;
        var cancelButton;
        var saveButton;
        ...
        beforeEach(function() {
            var dashletHeaderDom = '<div data-dashlet="toolbar"><div class="dashlet-header"><div ' +
                'class="btn-toolbar pull-right"><span class="dashlet-toolbar"><a class="btn btn-invisible' +
                ' btn-link hide" role="button" name="dashlet_cancel_button" style="display: none;">Cancel' +
                '</a></span><span class="dashlet-toolbar"><a class="rowaction btn btn-primary hide" role="button"' +
                ' name="dashlet_save_button" style="display: none;">Save</a></span><span class="fieldset' +
                ' actions dashlet-toolbar"><span class="dashlet-toolbar"><a class="rowaction btn btn-primary"' +
                ' role="button" name="edit_button">Edit</a></span></span><span class="dashlet-toolbar">' +
                '<a rel="tooltip" aria-label="Open/Close" data-dashletaction="toggleMinify" role="button" name=""' +
                ' class="btn btn-invisible dashlet-toggle minify"><i class="fa fa-chevron-up"></i></a></span><div' +
                ' class="btn-group"><ul class="dropdown-menu left" role="menu"><li role="menuitem"><span ' +
                'class="dashlet-toolbar"><a data-dashletaction="editClicked" role="button" name="edit_button">Edit' +
                '</a></span></li><li role="menuitem"><span class="dashlet-toolbar"><a ' +
                'data-dashletaction="refreshClicked" role="button" name="refresh_button">Refresh</a></span></li>' +
                '<li role="menuitem"><span></span></li></ul></div></div><h4 data-toggle="dashlet" ' +
                'class="dashlet-title record-toolbar"><span class="record-cell" data-type="avatar" data-name=' +
                '"picture"><span class="table-cell-wrapper"><span class="index" data-fieldname="picture" ' +
                'data-index=""><span class="detail"><span class="label label-module label-module-btn label-Cases"' +
                ' rel="tooltip">Cs</span></span></span></span></span><span class="record-cell" data-type="name" ' +
                'data-name="name"><span class="table-cell-wrapper"><span class="index" data-fieldname="name" ' +
                'data-index=""><span class="detail"><div class="ellipsis_inline">System not responding</div>' +
                '</span></span></span></span></h4></div></div>';

            view.layout.$el.append(dashletHeaderDom);
            btnBar = view.layout.$el.find('.btn-toolbar');
            titleBar = view.layout.$el.find('.dashlet-title');

            titleBar.width(400);
            $(titleBar.children()[0]).css('marginLeft', 25);
            $(titleBar.children()[1]).css('marginRight', 40);

            cancelButton = $(btnBar.children()[0]);
            saveButton = $(btnBar.children()[1]);
            editButton = $(btnBar.children()[2]);
            openCloseButton = $(btnBar.children()[3]);
            refreshButton = $(btnBar.children()[4]);

            cancelButton.width(0);
            saveButton.width(0);
            editButton.width(25);
            openCloseButton.width(15);
            refreshButton.width(15);
        });

        it('should return default minimum width if there is no title present', function() {
            titleBar.remove();
            expect(view.getContainerWidth()).toEqual(230);
        });

        it('should subtract the margin of childrens from the total width', function() {
            btnBar.remove();
            expect(view.getContainerWidth()).toEqual(325);
        });

        it('should take into consideration the buttons', function() {
            titleBar.css('paddingRight', 65);
            expect(view.getContainerWidth()).toEqual(325);
        });

        it('should NOT take into consideration the buttons', function() {
            titleBar.css('paddingRight', 30);
            expect(view.getContainerWidth()).toEqual(300);
        });
    });
});
