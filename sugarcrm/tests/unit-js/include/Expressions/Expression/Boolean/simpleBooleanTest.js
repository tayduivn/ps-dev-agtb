...
describe("Simple Boolean Functions Test", function () {
    ...
    describe("Is Valid Phone Expression", function () {
        var phone_str1 = new SUGAR.expressions.StringLiteralExpression(["!!test_phone_fake!!"]);
        var phone_str2 = new SUGAR.expressions.StringLiteralExpression(["(408) 123-4567"]);

        it("should return whether a string is a valid Phone Number", function () {
            var res =  new SUGAR.expressions.IsValidPhoneExpression([phone_str1],  getSLContext(model));
            expect(res.evaluate()).toBe("false");
            res =  new SUGAR.expressions.IsValidPhoneExpression([phone_str2],  getSLContext(model));
            expect(res.evaluate()).toBe("true");
        });
    });

    describe("Is Valid Time Expression", function () {
        //needs to be in HH:MM format
        var phone_str1 = new SUGAR.expressions.StringLiteralExpression(["12:00:00"]);
        var phone_str2 = new SUGAR.expressions.StringLiteralExpression(["12:00"]);

        it("should return whether a string is a valid time string", function () {
            var res =  new SUGAR.expressions.IsValidTimeExpression([phone_str1],  getSLContext(model));
            expect(res.evaluate()).toBe("false");
            res =  new SUGAR.expressions.IsValidTimeExpression([phone_str2],  getSLContext(model));
            expect(res.evaluate()).toBe("true");
        });
    });
    ...
    describe("Binary Dependency Expression Function", function () {
        ...
        it("should return if both values exist", function () {
            expect(new SUGAR.expressions.BinaryDependencyExpression([a, a], getSLContext(model)).evaluate()).toBe("false");
            expect(new SUGAR.expressions.BinaryDependencyExpression([a, b], getSLContext(model)).evaluate()).toBe("false");
            expect(new SUGAR.expressions.BinaryDependencyExpression([b, a], getSLContext(model)).evaluate()).toBe("false");
            expect(new SUGAR.expressions.BinaryDependencyExpression([b, b], getSLContext(model)).evaluate()).toBe("true");
        });
    });
});
/*
