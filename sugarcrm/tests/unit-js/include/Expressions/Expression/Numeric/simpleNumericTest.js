...
describe("Simple Numeric Functions Test", function () {
    ...
    describe("String Length Expression Function", function () {
        it("should return length of a string", function () {
            ...
            var test_str_expression = new SUGAR.expressions.StringLiteralExpression([test_string]);
            var strlen_expr = new SUGAR.expressions.StringLengthExpression([test_str_expression], getSLContext(model))
            expect(parseFloat(strlen_expr.evaluate())).toBe(test_string.length);
        });
    });
