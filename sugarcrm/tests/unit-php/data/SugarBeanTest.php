...
class SugarBeanTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * @covers ::loadAutoIncrementValues()
     * @dataProvider noAutoincrementProvider
     */
    public function testLoadAutoIncrementValuesWithoutAutoincrement($field, array $defs)
    {
        // The mock bean object we will be working on
        $bean = TestMockHelper::getObjectMock(
            $this,
            'SugarBean',
            ['getDBValuesForFields']
        );
        ...
        $bean->expects($this->never())
             ->method('getDBValuesForFields');

        // Call the method to test now
        TestReflection::callProtectedMethod($bean, 'loadAutoIncrementValues');

        // Verify what was done
        $this->assertNull($bean->{$field});
    }

    public function noAutoincrementProvider()
    {
        $t1 = 'test_no_ai';
        $t2 = 'test_false';

        return [
            // Tests auto_increment not set
            [
                'field' => $t1,
                'defs' => [
                    $t1 => [
            ...
            // Tests auto_increment set to not true
            [
                'field' => $t2,
                'defs' => [
                    $t2 => [
