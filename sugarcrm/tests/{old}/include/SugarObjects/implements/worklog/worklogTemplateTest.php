...
class worklogTemplateTest extends TestCase
{
    /**
     * Checks if all modules uses basics or default has worklog as a field
     * @param string $module The module we would like to check for whether worklog_field exists
     * @dataProvider moduleListProvider
     */
    public function testCheckAllAvailableModuleHasWorklogField($module)
    {
        $bean = BeanFactory::newBean($module);
        $this->assertArrayHasKey('worklog', $bean->field_defs);
        $this->assertArrayHasKey('worklog_link', $bean->field_defs);
    }

    /**
     * Checks if default enabled module has worklog field in record view
     * @param string $folder The folder name of the $module
     * @param bool $default_enabled Whether worlog field should be default enabled or not
     * @dataProvider moduleListProvider
     */
    public function testCheckDefaultEnabled(string $folder, bool $default_enabled)
    {
        ...
        foreach ($viewdefs[$folder]['base']['view']['record']['panels'][1]['fields'] as $field) {
            if (is_array($field) && $field['name'] === 'worklog') {
                $enabled = true;
                break;
            }
        }
