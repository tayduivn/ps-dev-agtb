...
// END SUGARCRM flav=ent ONLY
    ...
    public function testCreateUniqueIndex()
    {
        ...
        $this->createTableParams(
            ...
            [
                [
                    'name'   => 'idx_baz_'. $tablename,
                    'type'   => 'unique',
                    'fields' => ['baz'],
                ],
            ]
        );
        $bazIndex = $this->db->get_index($tablename, 'idx_baz_' . $tablename);
        $this->assertEquals('unique', $bazIndex['type']);
        ...
        $this->db->addIndexes(
            ...
            [
                [
                    'name'   => 'idx_foo_'. $tablename,
                    'type'   => 'unique',
                    'fields' => ['foo'],
                ],
            ]
        );
        $fooIndex = $this->db->get_index($tablename, 'idx_foo_' . $tablename);
        $this->assertEquals('unique', $fooIndex['type']);
        $this->db->insertParams(
            $tablename,
            $def,
            [
        ...
        $this->db->addIndexes(
            ...
            [
                [
                    'name'   => 'idx_bar_'. $tablename,
                    'type'   => 'unique',
                    'fields' => ['bar'],
                ],
            ]
        );
        $barIndex = $this->db->get_index($tablename, 'idx_bar_' . $tablename);
        $this->assertEquals('unique', $barIndex['type']);
        $this->db->addIndexes(
            $tablename,
            [
                [
                    'name'   => 'idx_foobar_'. $tablename,
                    'type'   => 'unique',
                    'fields' => ['foo', 'bar'],
                ],
            ]
        );
        $foobarIndex = $this->db->get_index($tablename, 'idx_foobar_' . $tablename);
        $this->assertEquals('unique', $foobarIndex['type']);
        $this->db->addIndexes(
            $tablename,
            [
                [
                    'name'   => 'idx_foobarbaz_'. $tablename,
                    'type'   => 'unique',
                    'fields' => ['foo', 'bar', 'baz'],
                ],
            ]
        );
        $foobarbazIndex = $this->db->get_index($tablename, 'idx_foobarbaz_' . $tablename);
        $this->assertEquals('unique', $foobarbazIndex['type']);
    }

    public function testUniqueIndexException()
    {
