...
// Adding node-deprecated to support YUI3.3.0

    YUI({combine: true, timeout: 10000, base:"include/javascript/yui3/build/", comboBase:"index.php?entryPoint=getYUIComboFile&"}).use("node-deprecated", "anim", function(Y) {
...
YAHOO.util.Event.onContentReady("moduleList", function() 
{
    ...
    function onSubmenuBeforeShow(p_sType, p_sArgs) 
    {
		...
		if (this.parent) {
			...
			var handleSuccess = function(o){
				...
				oShadowBodyCenter.style.height = (oShadow.offsetHeight)+"px";
	...
	if (this.parent) {
		...
		if(oElement.id.substr(0,4) != "More" && oElement.id.substring(0,8) != "TabGroup") {
			...
			oVR.style.top = (oBd.offsetTop+15) +"px";
