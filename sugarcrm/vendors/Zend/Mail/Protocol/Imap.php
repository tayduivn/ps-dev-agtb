...
class Zend_Mail_Protocol_Imap
{
    ...
    public function connect($host, $port = null, $ssl = false)
    {
        ...
        if (!$this->_socket) {
             ...
             */
            require_once 'vendors/Zend/Mail/Protocol/Exception.php';
     ...
     * @throws Zend_Mail_Protocol_Exception
     */
    public function readLine(&$tokens = array(), $wantedTag = '*', $dontParse = false, $onPlus = null)
    {
        $line = $this->_nextTaggedLine($tag);
        while(!is_null($onPlus) && $tag == '+') {
            $this->sendRequest($onPlus);
            $line = $this->_nextTaggedLine($tag);
        }
        if (!$dontParse) {
    ...
    public function readResponse($tag, $dontParse = false)
    {
        ...
        if ($dontParse) {
            // first two chars are still needed for response code
