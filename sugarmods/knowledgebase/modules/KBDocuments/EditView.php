...
echo "\n<p>\n";
echo get_module_title("KBDocuments", $mod_strings['LBL_MODULE_TITLE'].": ".$focus->kbdocument_name, true);
echo "\n</p>\n";
...
if (isset($focus->status_id)) $xtpl->assign("STATUS_OPTIONS", get_select_options_with_id($app_list_strings['kbdocument_status_dom'], $focus->status_id));
else $xtpl->assign("STATUS_OPTIONS", get_select_options_with_id($app_list_strings['kbdocument_status_dom'], ''));
...
if( file_exists("include/FCKeditor/fckeditor.php"))
{
  ...
  $oFCKeditor->Config["CustomConfigurationsPath"] = $sugar_config['site_url']."/modules/KBDocuments/kbfckconfig.js?";
  $oFCKeditor->ToolbarStartExpanded = true;
  $oFCKeditor->ToolbarSet = 'KB';
  
  
  //$oFCKeditor->Value = "<h3 style='color: rgb(153, 51, 0);'>This is FCKEditor. Create/Edit/Insert/Embed the Content for Article</h3>";
  if(from_html(KBDocument::get_kbdoc_body_without_incrementing_count($kbId)) != null){
   //str replace the cache/upload
   $article_body = str_replace('/cache/images/', $sugar_config['site_url'].'/cache/images/', KBDocument::get_kbdoc_body_without_incrementing_count($kbId));
   $oFCKeditor->Value = $article_body;
	...
	//set the kb fields from case i.e. kb name
	 if($from_case->case_number != null){
	  $oFCKeditor->Value = "Case Number: $from_case->case_number</br>";
	 }
	 if($from_case->name != null){
	  $oFCKeditor->Value .= "Subject: $from_case->name</br>";
	 }
	 if($from_case->description != null){
	  $oFCKeditor->Value .= "Description: $from_case->description</br>";
	 }
	 if($from_case->resolution != null){
	  $oFCKeditor->Value .= "Resolution: $from_case->resolution</br>";
	 } 
	...
	//set the kb fields from case i.e. kb name
	 if($from_email->description != null){
	  $oFCKeditor->Value = "$from_email->description</br>";
	 }
	 if($from_email->description_html != null){
	  $oFCKeditor->Value = "$from_email->description_html</br>";
	 } 
	 //if bugs are associated to cases then extract the work_log and notes etc...
	 //also extract notes associated to the cases
  }
 ...
 echo <<<EOQ
	  ...
	  function insert_variable_html_link(text) {
	  	var oEditor = FCKeditorAPI.GetInstance('{$instancename}') ;
	  	thelink="<a href='" + text + "''" + ">'Default link text.'</a>";
...
$javascript->addFieldGeneric('fck_vals', '', 'LBL_ARTICLE_BODY' ,'true');
