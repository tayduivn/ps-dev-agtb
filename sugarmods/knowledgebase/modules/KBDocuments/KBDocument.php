...
//todo remove leads relationship.
	...
	function fill_in_additional_list_fields() {
		...
		if(!empty($row)) {
		   ...
		   $this->kbarticle_author_name = $row['first_name'] . ' ' . $row['last_name'];
    ...
    function create_new_list_query($order_by, $where,$filter=array(),$params=array(), $show_deleted = 0,$join_type='', $return_array = false,$parentbean){
        ...
        $ret_array['select'] .= ", kbdocuments.id, kbdocuments.kbdocument_name, kbdocuments.active_date, kbdocuments.exp_date, kbdocuments.status_id, kbdocuments.date_entered date_entered, kbdocuments.date_modified, kbdocuments.deleted, kbdocuments.is_external_article, kbdocuments.modified_user_id, kbdocuments.created_by";        
        ...
        //BEGIN SUGARCRM flav=pro ONLY 
        if (!is_admin($current_user)){ 
            $ret_array['from'] .=  " Inner Join team_memberships jt3 on kbdocuments.team_id = jt3.team_id  AND jt3.user_id = '".$current_user->id. "' and jt3.deleted = 0 ";
        }
        //END SUGARCRM flav=pro ONLY 
	...
	function create_list_query($order_by, $where, $show_deleted = 0) {
		...
		$query .= "  ,kbdocument_revisions.revision as latest_revision";
		$query .= "  ,kbdocument_revisions.date_entered as last_rev_create_date";
		$query .= " ,kbdocument_revisions.created_by as last_rev_created_by";

		if ($custom_join) {
			$query .= $custom_join['select'];
		}
		$query .= " FROM kbdocument_revisions,$this->table_name  ";
		...
		$query .= " AND kbdocuments.document_revision_id = document_revisions.id";
	...
	function get_kbdoc_attachments($kbdoc_id,$screen){	    
		...
		if($screen =='Detail'){
			for($i=0;$i<count($docrevs);$i++){      	 	     	      	
				 ...
				 $kbdoc_atts .="<div id=tag$i> <a href='download.php?id=$file_url_noimage&type=Documents' class='tabDetailViewDFLink'>$filename</a>&nbsp;</div>";									
			}		
		}
		if($screen =='Edit'){			
		 ...
		 $doc='Doc';
			for($i=0;$i<count($docrevs);$i++){      	 	     	      	
			     ...
			     $kbdoc_atts .='<img src="themes/Sugar/images/delete.gif" alt="Remove" onclick="SUGAR.kb.strikeOutFromImage('.$cDoc.','.$doc_rev.','.$att.');SUGAR.kb.setCheckBox('.$doc_rev.')">';//.'&nbsp;&nbsp;';
                 //$kbdoc_atts .= '<input type="checkbox" name="remove_attachment[]" value="'.$doc_rev_id.'"> '.$app_strings['LNK_REMOVE'].'&nbsp;</div>';
				 $kbdoc_atts .="<a href='download.php?id=$file_url_noimage&type=Documents' class='tabDetailViewDFLink'>$filename</a>&nbsp;";
		  ...
		  if($kbtags != null){
			  ...
			  $parent_tag_id = $kbtags['parent_tag_id'];     
			    while ($parent_tag_id != null) {		      		
		         ...
		         if($screen == 'Edit'){
			       $tags .='<img src="themes/Sugar/images/delete.gif" alt="Remove" onclick="SUGAR.kb.strikeOutFromImage('.$cTag.','.$cBox.','.$att.')">';//.'&nbsp;&nbsp;';
			       ...
			       //$tags .='<img src="themes/Sugar/images/delete.gif" alt="Remove" onclick="strikeOutFromImage('.$cTag.','.$cBox.')">'.'&nbsp;&nbsp;';;
