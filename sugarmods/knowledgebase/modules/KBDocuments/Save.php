...
require_once('modules/KBDocumentKBTags/KBDocumentKBTag.php');
...
if (isset($KBDocument->id)) {	
       ...
       //check if status was draft or in review
	    ...
	    if($KBDocument->status_id == 'Published'){
	    	...
	    	if(empty($KBDocument->active_date) || $KBDocument->active_date==null){	    		
	            $KBDocument->active_date = $timedate->to_display_date(gmdate("Y-m-d"), false);
	    	}
  ...
  $article_body = str_replace($sugar_config['site_url'].'/cache/images/', '/cache/images/', $_REQUEST['body_html']);
	...
	$KBDocumentKBTag->db->query($q); 
	  ...
	  if($KBDocument != null){	
		...
		if($KBDocument->status_id == 'Published'){
	    	...
	    	if(empty($KBDocument->active_date) || $KBDocument->active_date==null){	    		
	            $KBDocument->active_date = $timedate->to_display_date(date("Y-m-d"), false);
