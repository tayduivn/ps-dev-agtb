...
function get_admin_fts_list($where,$isMultiSelect=false){
   ...
   function get_faq_list($faq_id, $bean) {
       ...
       $query = "select distinct(k.id)as id, k.created_by as created_by from kbdocuments k
                INNER join kbdocuments_kbtags kt on kt.kbdocument_id = k.id AND k.is_external_article = 1
                AND kt.kbtag_id in ($queryIds)";
   ...
   function get_tag_docs($tag, $bean) {
        ...
        $spec_SearchVars = array(); 
     	$spec_SearchVars['exp_date'] = date('Y-m-d');
        ...
        $query = "select distinct(k.id) as doc_id, k.kbdocument_name as doc_name from kbdocuments k
                 INNER join kbdocuments_kbtags kt on kt.kbdocument_id = k.id AND kt.deleted = 0 AND k.is_external_article = 1
                 AND k.status_id = 'Published' AND k.deleted = 0 AND " . $date_filter . " AND kt.kbtag_id = '$tag'";         
   ...
   class TagNode {
	...
	function create_portal_list_query($bean, $order_by, $where, $keywords, $row_offset, $limit, $test = false) {
		...
		$date_filter = return_date_filter($bean->db->dbType, 'exp_date', 'after', date('Y-m-d'), null);
		   	   ...
		   	   if(!empty($limit)) {
                   ...
                   if($bean->db->dbType == 'mssql') {
                   	  $sql = "SELECT TOP $limit *
                              ...
                              WHERE deleted = 0 AND latest = 1) AND kbdocuments.is_external_article = '1' AND kbdocuments.status_id = 'Published' AND $date_filter AND kbdocuments.id IN
    ...
    function create_portal_most_recent_query($dbtype ='mysql', $n = '10', $where='') {
        ...
        if($dbtype == 'mysql'){
            ...
            $portal_most_recent_query .= ' kbdocuments.deleted = 0 AND kbdocuments.is_external_article = 1 ';
    ...
    if (!empty($filter)){
        ...
        if ($filter == 'on'){
            if(!empty($filter_date)){
                if ($dbType == 'oci8') {
                    //BEGIN SUGARCRM flav=ent ONLY 
                    return $field . "=TO_DATE('".PearDatabase :: quote($filter_date)."','YYYY-MM-DD') ";
                    //END SUGARCRM flav=ent ONLY 
                } else {
                    return $field . "='".PearDatabase :: quote($filter_date)."' ";
                }
            }            
            ...
            if(!empty($filter_date) && !empty($filter_date2)){
                if ($dbType == 'oci8') {
                    //BEGIN SUGARCRM flav=ent ONLY 
                    return $field . " >= TO_DATE('".PearDatabase :: quote($filter_date)."','yyyy-mm-dd') AND   " . $field . "<=TO_DATE('".PearDatabase :: quote($filter_date2)."','yyyy-mm-dd')";
                    //END SUGARCRM flav=ent ONLY 
                } else {
                    return $field . ">='" . PearDatabase :: quote($filter_date) . "' AND  " . $field . "<='" . PearDatabase :: quote($filter_date2)."'";
                }
            }          
