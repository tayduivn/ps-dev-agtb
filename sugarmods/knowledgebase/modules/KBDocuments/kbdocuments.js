	...
	return{
	   ...
	   hideApply:function (){
		...
		var myButtons = [ //{ text:"Ok", handler:handleSubmit },
		 ...
		 tagsTreeModalMoveDocs = new YAHOO.widget.SimpleDialog('moveDlg', 
			...
			fillInTags = function(data) {                                                
						...
						var listeners = new YAHOO.util.KeyListener(document, { keys : 27 }, {fn: function() {this.hide();}, scope: tagstreeMoveDocsModal, correctScope:true} );
		...
		var myButtons = [ //{ text:"Ok", handler:handleSubmit },
		 ...
		 applyTagsToDocs = new YAHOO.widget.SimpleDialog('applyDlg', 
			...
			fillInTags = function(data) {                                                
						...
						applyTagsToDocs.setHeader(' Tags');											 
						var listeners = new YAHOO.util.KeyListener(document, { keys : 27 }, {fn: function() {this.hide();}, scope: tagstreeApplyTags, correctScope:true} );
		...
		moveDocsAndModalClose:function(){
			...
			//+'articles='+YAHOO.lang.JSON.stringify(selectedArticleIds)                                 	    
		     ...
		     if(newTag != null && newTag.length > 0){
		       ...
		       if(!nodeChildExists){       
			       //Ajax call for adding the new tag
					...
					postData = 'newTagName=' + YAHOO.lang.JSON.stringify(idName) + '&module=KBTags&action=SaveTagsModal&to_pdf=1';    
					YAHOO.util.Connect.asyncRequest('POST', 'index.php', callback, postData);
				    if(admin){
	...
	searchTagsAll:function(){		 
	 ...
	 modalInit:function(){
		  ...
		  document.getElementById('dialog1').style.display='';
			...
			var myButtons = [ //{ text:"Ok", handler:handleSubmit },
			    ...
			    myDialog = new YAHOO.widget.SimpleDialog('dialog1', 
						{ visible:false, width: "410",height:"400",
			...
			postData = 'tagsMode=' + YAHOO.lang.JSON.stringify(selectCreateTags)+ '&module=KBTags&action=SelectCreateApplyAndMoveTags&to_pdf=1';    
			YAHOO.util.Connect.asyncRequest('POST', 'index.php', {success: fillInTags, failure: fillInTags}, postData);															 	
		},
				  ...
				  if(node.data.id=='All_Tags'){
				   ...
				   if(!tagExists){ 		
					   ...
					   //create a new div element
						...
						new_row_button_remove_tag.setAttribute("src", "themes/Sugar/images/delete_inline.gif");	
		...
		selectTagToApplyArticles:function(){    
		     ...
		     */ 
		     if(checked){
			     document.getElementById('CreateNodeApply').innerHTML = 'Click Apply Tag';    	  	   	   	 
		...
		multiAttachments:function(list_target,elmentsName,site_url,theme_name,isAtt){
			...
			this.addListRow = function( element,div_id ){    
		    	...
		    	new_row_button_remove.setAttribute("src", "themes/"+theme_name+"/images/delete_inline.gif");	
		...
		//		new_row_file_name.type = 'text';
		       ...
		       new_row_attach_file.value ='/themes/'+theme_name+'/images/company_logo.png';	
		       new_row_attach_file.disabled ='true';	
		
			    var imgElement = document.createElement("img");
				imgElement.setAttribute("src", "themes/"+theme_name+"/images/Accounts.gif");	
				...
				new_row_button_embed.setAttribute("src", "themes/"+theme_name+"/images/plus_inline.gif");	
				...
				new_row_button_remove.onclick= function(){	
                   ...
                   if(isAtt){
			    		...
			    		if(!isAtt){
			                ...
			                imglocation=site_url+'/cache/images/';
		...
		copyFCKVal:function(){
			var oEditor = FCKeditorAPI.GetInstance('body_html');				
			var currValFCK = oEditor.GetXHTML('');
			document.getElementById('fck_vals').value=currValFCK;
		},
		...
		strikeOutFromImage:function(currTag,currBox,tagOrDoc){
		...
		strikeOutFromBox:function(currTag,currBox){
		  var boxState = document.getElementById(currBox).checked;
		  if(boxState){    
		     currTag.innerHTML = "<strike><strong><font COLOR=red>" +currTag.innerHTML +"</font></>";		
		  }
