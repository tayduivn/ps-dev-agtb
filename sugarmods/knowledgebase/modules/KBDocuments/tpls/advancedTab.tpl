		...
		<tr><td>
			<div>				
				<table width="100%" border="0" cellspacing="0" cellpadding="0" >
					<tr>
				        <td width="15%" class="dataLabel" >{$MOD.LBL_CONTAINING_THESE_WORDS}</td>
				        <td width="35%" class="dataField" ><input type='text' id='searchText_include' class='text' name='searchText_include' size='50'"  value='{$searchText_include}' maxlength='150'></td>
				        <td width="15%" class="dataLabel" >{$MOD.LBL_SEARCH_WITHIN}</td>
				        <td width="35%" class="dataField" ><select name='canned_search' id='canned_search'>{$CANNED_SEARCH_OPTIONS}</select></td>
					</tr>
					<tr>
				        <td width="15%" class="dataLabel" >{$MOD.LBL_EXCLUDING_THESE_WORDS}</td>
				        <td width="35%" class="dataField" ><input type='text' id='searchText_exclude' class='text' name='searchText_exclude' size='50'"  value='{$searchText_exclude}' maxlength='150'><br></td>	  
				        <td class="dataLabel" >{$MOD.LBL_UNDER_THIS_TAG}</td>
				        <td class="dataField" >
			...
			<div id='more_options_div' name='more_options_div' style='display: none'>
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td width="15%" class="dataLabel"><span sugar='slot2'>{$MOD.LBL_ARTICLE_TITLE}&nbsp;<span class="required"></span></span sugar='slot'></td>
						<td width = "35%" class="dataField"><span sugar='slot2b'><input name='kbdocument_name'  type='text' size='50' value="{$kbdocument_name}"></span sugar='slot'></td>		        
				        <td width="15%" class="dataLabel"><span sugar='slot3'>{$MOD.LBL_TIMES_VIEWED}</td>
						<td width="35%" class="dataField"><span sugar='slot3b'><select  name='frequency'>{$FRQ_VIEW_OPTIONS}</select></span sugar='slot'></td>		                

					</tr>
					<tr>
						<td valign="top" class="dataLabel"><span sugar='slot4'>{$MOD.LBL_DOC_STATUS}</span sugar='slot'></td>
						<td class="dataField"><span sugar='slot4b'><select  name='status_id'>{$STATUS_OPTIONS}</select></span sugar='slot'></td>
						<td valign="top" class="dataLabel"><span sugar='slot5'>{$APP.LBL_TEAM} </span sugar='slot'></td>
						<td class="dataField"><span sugar='slot5b'><input class="sqsEnabled"  autocomplete="off" id="team_name" name='team_name' type="text" value="{$team_name}"><input id='team_id' name='team_id' type="hidden" value="{$team_id}" />
					...
					<tr>
						<td class="dataLabel"><span sugar='slot6'>{$MOD.LBL_KBDOC_APPROVED_BY}</span sugar='slot'></td>
						...
						<td class="dataLabel"><span sugar='slot7'>{$MOD.LBL_ARTICLE_AUTHOR}</span sugar='slot'></td>
				   	...
				   	<tr>
						<td class="dataLabel"><span sugar='slot8'>{$MOD.LBL_PUBLISHED}&nbsp;</span sugar='slot'></td>
						<td colspan = '3' class="dataField" nowrap><span sugar='slot8b'>
					...
					<tr>
						<td class="dataLabel"><span sugar='slot9'>{$MOD.LBL_EXPIRES}</span sugar='slot'></td>
						<td  colspan = '3' class="dataField"  nowrap><span sugar='slot9b'>
					...
					<tr>
						<td class="dataLabel"><span sugar='slot9'>{$MOD.LBL_ATTACHMENTS}</span sugar='slot'></td>
						<td   class="dataField"><span sugar='slot9b' nowrap>
						...
						<td class="dataLabel"><span sugar='slot10'>{$MOD.LBL_IS_EXTERNAL_ARTICLE}</span sugar='slot'></td>
					...
					<tr>
						<td class='dataLabel' nowrap width='1%'>
							{$MOD.LBL_SAVE_SEARCH_AS} <img border='0' src='themes/{$THEME}/images/help.gif' onmouseover="return overlib('{$MOD.LBL_SAVE_SEARCH_AS_HELP}', FGCLASS, 'olFgClass', CGCLASS, 'olCgClass', BGCLASS, 'olBgClass', TEXTFONTCLASS, 'olFontClass', CAPTIONFONTCLASS, 'olCapFontClass', CLOSEFONTCLASS, 'olCloseFontClass' );" onmouseout="return nd();">
						</td>
						<td class='dataField'>
						...
						<td nowrap class='dataLabel'>
							{$MOD.LBL_PREVIOUS_SAVED_SEARCH} <img border='0' src='themes/{$THEME}/images/help.gif' onmouseover="return overlib('{$MOD.LBL_PREVIOUS_SAVED_SEARCH_HELP}', FGCLASS, 'olFgClass', CGCLASS, 'olCgClass', BGCLASS, 'olBgClass', TEXTFONTCLASS, 'olFontClass', CAPTIONFONTCLASS, 'olCapFontClass', CLOSEFONTCLASS, 'olCloseFontClass' );" onmouseout="return nd();">
						</td>
						<td class='dataField'>
				...
				<div>
			        <span onclick="hideShowMoreOptions('more_options_img');" id='more_options_img' name='more_options_img' ><img src='themes/{$THEME}/images/advanced_search.gif'>&nbsp;<a href='#'  class='listViewTdLinkS1'>{$MOD.LBL_SHOW_OPTIONS}</a></span>
			        <span onclick="hideShowMoreOptions('less_options_img');" id='less_options_img' name='less_options_img'  style='display: none'><img src='themes/{$THEME}/images/basic_search.gif'>&nbsp;<a href='#' class='listViewTdLinkS1'>{$MOD.LBL_HIDE_OPTIONS}</a></span>
				</div>
			</td></tr></table>
	        	<div style='padding: 2'>
...
{literal}				
	Calendar.setup ({
		inputField : "active_date_field", ifFormat : format, showsTime : false, button : "active_date_trigger", singleClick : true, step : 1
	});
	
	Calendar.setup ({
		inputField : "exp_date_field", ifFormat : format, showsTime : false, button : "exp_date_trigger", singleClick : true, step : 1
	});

	Calendar.setup ({
		inputField : "active_date_field2", ifFormat : format, showsTime : false, button : "active_date_trigger2", singleClick : true, step : 1
	});
	
	Calendar.setup ({
		inputField : "exp_date_field2", ifFormat : format, showsTime : false, button : "exp_date_trigger2", singleClick : true, step : 1
	});
	...
	function hideShowMoreOptions(span) {
		if(span == 'more_options_img'){
			document.getElementById('more_options_div').style.display = '';
			document.getElementById('more_options_img').style.display = 'none';
			document.getElementById('less_options_img').style.display = '';			

		}else{
			document.getElementById('more_options_div').style.display = 'none';
			document.getElementById('more_options_img').style.display = '';
			document.getElementById('less_options_img').style.display = 'none';							
		}
	}
...
{/literal}				
	    ...
	    <div id='dialog1' style="display: none">                    
				 <div id="tagsCreate" style="display:none" >	                                     					       
					          <table>  					             						            			                     
					                <tr>					                  					                     								         
			                             <td class="dataLabel"><span sugar='slot19' id='new_tag_name'>Tag Name:</span sugar='slot'></td>
									     <td class="dataField"><input id='tag_new' name='tag_new' type='hidden' size='20'></td>  						  
	           ...
	           <div id='tagstree' class="tabForm" >
				 ...
				 <div id='CreateNodeMessage' style="display:none">
				      <table>
					      <tr><td class="dataLabel" colspan='2'><span sugar='slot25' id ='CreateNodeM' ><b><font COLOR=red>Select the parent tag, from tree</b></span sugar='slot'></td>
