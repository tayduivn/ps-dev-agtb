...
{if $PROJECTS}
	<table width="100%" cellspacing="1" cellpadding="5" border="0" class="tabDetailView2">	
		<tr height="25" >
		...
		<tr valign="top">
			<td class="tabDetailViewDL2" valign="top"><a href="index.php?module=Project&action=DetailView&record={$PROJECT->id}&return_module=Project&return_action=DetailView">{$PROJECT->name}</a><br/>{$PROJECT->estimated_end_date}</td>
			<td class="tabDetailViewDF2" valign="top">
				<table width="100%" cellpadding="2" cellspacing="0" border="0" class="tabDetailView2">
					{foreach from=$OVERDUE_TASKS item="OVERDUE_TASKS_FOR_PROJECT" key="PROJECT_KEY"}
						{if $PROJECT_KEY == $PROJECT->id}
							{foreach from=$OVERDUE_TASKS_FOR_PROJECT item="OVERDUE_TASK"}
								...
								<tr valign="top">
									<td width="55%" valign="top"><a class="tabDetailViewDL2Link" href="index.php?module=Project&action=EditGridView&record={$PROJECT->id}&return_module=Project&return_action=DetailView">{$OVERDUE_TASK->name}</a>{if $OVERDUE_TASK->milestone_flag == 1}&nbsp;*{/if}</td>
					...
					{foreach from=$UPCOMING_TASKS item="UPCOMING_TASKS_FOR_PROJECT" key="PROJECT_KEY"}
						{if $PROJECT_KEY == $PROJECT->id}
							{foreach from=$UPCOMING_TASKS_FOR_PROJECT item="UPCOMING_TASK"}
								...
								<tr valign="top">
									<td width="55%" valign="top"><a class="tabDetailViewDL2Link"  href="index.php?module=Project&action=EditGridView&record={$PROJECT->id}&return_module=Project&return_action=DetailView">{$UPCOMING_TASK->name}</a>{if $UPCOMING_TASK->milestone_flag == 1}&nbsp;*{/if}</td>
					...
					{foreach from=$OPEN_CASES item="OPEN_CASES_FOR_PROJECT" key="PROJECT_KEY"}
						{if $PROJECT_KEY == $PROJECT->id}
							{foreach from=$OPEN_CASES_FOR_PROJECT item="OPEN_CASE"}
								<tr valign="top">
									<td width="50%" valign="top"><a class="tabDetailViewDL2Link"  href="index.php?module=Cases&action=DetailView&record={$OPEN_CASE->id}&return_module=Project&return_action=DetailView">{$OPEN_CASE->case_number}</a></td>
								</tr>
							{/foreach}
						{/if}
					{/foreach}
...
<h2>{$MOD.LBL_MY_PROJECT_TASKS}</h1>
	<table width="70%" cellspacing="1" cellpadding="0" border="0" class="tabDetailView2">	
		<tr height="25">
		...
		<tr valign="top">
			...
			<td class="tabDetailViewDF2">
				<table width="100%" cellpadding="0" cellspacing="0" border="0" class="tabDetailView2">
