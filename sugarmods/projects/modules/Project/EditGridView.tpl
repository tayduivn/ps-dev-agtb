...
<!--<link rel="stylesheet" type="text/css" href="include/javascript/yui/fonts/fonts.css">-->
<link rel="stylesheet" type="text/css" href="include/javascript/yui/assets/menu.css">
<script src="include/javascript/yui/container_core.js"></script>
<script src="include/javascript/yui/menu.js"></script>
...
<script type="text/javascript" src="modules/Project/gantt.js"></script>
<script type="text/javascript" src="modules/Project/grid.js"></script>
...
function initUI(){
	...
	/*
	else if (view == "grid_gantt") {
		SUGAR.gantt.gridGanttView();
	}
	*/
	else {	
...
<div id="projectButtonsDiv">
	...
	{if $SELECTED_VIEW <= 1 && $OWNER == $CURRENT_USER}
			<a valign="bottom" title="{$MOD.LBL_INSERT_BUTTON}"><img src='{$IMAGE_PATH}ProjectInsertRows.gif' onClick="javascript:SUGAR.grid.insertRow()"></img></a>
			<a valign="bottom" title="{$MOD.LBL_INDENT_BUTTON}"><img src='{$IMAGE_PATH}ProjectIndent.gif' onClick="javascript:SUGAR.grid.indentSelectedRows()"></img></a>
			<a valign="bottom" title="{$MOD.LBL_OUTDENT_BUTTON}"><img src='{$IMAGE_PATH}ProjectOutdent.gif' onClick="javascript:SUGAR.grid.addToOutdent()"></img></a>
			<a valign="bottom" title="{$MOD.LBL_COPY_BUTTON}"><img src='{$IMAGE_PATH}ProjectCopy.gif' onClick="javascript:SUGAR.grid.copyRow()"></img></a>
			<a valign="bottom" title="{$MOD.LBL_CUT_BUTTON}"><img src='{$IMAGE_PATH}ProjectCut.gif' onClick="javascript:SUGAR.grid.cutRow()"></img></a>
			<a valign="bottom" title="{$MOD.LBL_PASTE_BUTTON}"><img src='{$IMAGE_PATH}ProjectPaste.gif' onClick="javascript:SUGAR.grid.pasteRow()"></img></a>
			<a valign="bottom" title="{$MOD.LBL_DELETE_BUTTON}"><img src='{$IMAGE_PATH}ProjectDelete.gif' onClick="javascript:SUGAR.grid.deleteRows()"></img></a>
	...
	<!--<input type="button" name="button" value="{$MOD.LBL_SAVE_BUTTON}" onclick="javascript:SUGAR.grid.save()"/>-->
	...
	<input class="button" type="button" name="button" value="{$MOD.LNK_PROJECT_RESOURCE_REPORT}" onclick="window.open('index.php?module=Project&sugar_body_only=1&action=ResourceReport&record={$ID}','','width=700,height=700,resizable=1,scrollbars=1')" />
<!--	<input class="button" type="button" name="button" value="{$MOD.LNK_PROJECT_RESOURCE_REPORT}" onclick="open_popup('Project', 600, 400, '&act=ResourceReport&id={$ID}', true, true, '')" />-->
	...
	<div id="grid_space">
		<div>
			<table id="projectTable" border="1" cellpadding="0" cellspacing="0">
				<tr align=center bgcolor="{$BG_COLOR}" height="50">
					<th width="8%">{$MOD.LBL_TASK_ID}</th>
					<th width="5%">{$MOD.LBL_PERCENT_COMPLETE}</th>
					<th width="30%" nowrap>{$MOD.LBL_TASK_NAME}</th>
					<!--
					<th width="30%" id="name_header" style="cursor:E-resize;" onmouseup="SUGAR.grid.setMouseUp();SUGAR.grid.columnMouseUp(event);" onmousemove="SUGAR.grid.columnMouseMove(event)"
						onmousedown="SUGAR.grid.setMouseDown();SUGAR.grid.columnMouseDown(event, this);//SUGAR.grid.colMouseDown(event, this.id);">{$MOD.LBL_TASK_NAME}</th>
					-->
					<!--<th id="task_name_header">{$MOD.LBL_TASK_NAME}<div id="task_name_header_div"></div></th>-->
				...
				<tr id="project_task_row_{$TASK->project_task_id}" height="23">
					<td style="cursor:default" bgcolor="{$BG_COLOR}" align="center" onClick="SUGAR.grid.clickedRow({$TASK->project_task_id}, event);">
					...
					<td>
						<input  {if $CURRENT_USER != $TASK->resource_id && $CURRENT_USER != $OWNER}readonly="readonly"{/if} type=text size=5 style="border:0" name=percent_complete_{$TASK->project_task_id} id=percent_complete_{$TASK->project_task_id} value="{$TASK->percent_complete}"
					...
					<td nowrap ondblclick="SUGAR.grid.toggle('description_{$TASK->project_task_id}_div', 'description_{$TASK->project_task_id}', 'description_{$TASK->project_task_id}_divlink');">
						<div id='description_{$TASK->project_task_id}_div' style="display:none;">
							<input {if $CURRENT_USER != $OWNER}readonly="readonly"{/if} name=description_{$TASK->project_task_id} type=text maxlength=50 style="border:0"  size=40 id=description_{$TASK->project_task_id} value="{$TASK->name}" 
					...
					<td>
						<input  {if $CURRENT_USER != $OWNER}readonly="readonly"{/if} type=text  style="border:0;" size=3 name=duration_{$TASK->project_task_id} id=duration_{$TASK->project_task_id} value="{$TASK->duration}" 
					...
					<td>
						...
						<select  {if $CURRENT_USER != $OWNER}disabled{/if} style='border:0' name=duration_unit_{$TASK->project_task_id} id=duration_unit_{$TASK->project_task_id} onChange="SUGAR.grid.changeDurationUnits('{$TASK->project_task_id}')">
					...
					<td>					
						<input {if $CURRENT_USER != $OWNER}readonly="readonly"{/if} name=date_start_{$TASK->project_task_id} id=date_start_{$TASK->project_task_id} style="border:0" onchange="parseDate(this, '{$CALENDAR_DATEFORMAT}');"
					...
					<td>					
						<input {if $CURRENT_USER != $OWNER}readonly="readonly"{/if} name=date_finish_{$TASK->project_task_id} id=date_finish_{$TASK->project_task_id} style="border:0" onchange="parseDate(this, '{$CALENDAR_DATEFORMAT}');"
					...
					<td>
						<input  {if $CURRENT_USER != $OWNER}readonly="readonly"{/if} type=text size=10  style='border:0' name=predecessors_{$TASK->project_task_id} id=predecessors_{$TASK->project_task_id} value="{$TASK->predecessors}"
					...
					<td>
						...
						<select  {if $CURRENT_USER != $OWNER}disabled{/if} style='border:0' name=resource_{$TASK->project_task_id} id=resource_{$TASK->project_task_id}
					...
					<td id=optional_{$TASK->project_task_id}><input {if $CURRENT_USER != $TASK->resource_id && $CURRENT_USER != $OWNER}readonly="readonly"{/if} type=text size=10 style="border:0" name=actual_duration_{$TASK->project_task_id} id=actual_duration_{$TASK->project_task_id} value="{$TASK->actual_duration}" onBlur="SUGAR.grid.validateDuration(this);">
					</td>
				</tr>
				<script type="text/javascript">
