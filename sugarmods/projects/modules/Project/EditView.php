...
// Bug 12072: running clean_string on _id request variables for editview to avoid XSS problems
foreach(array_keys($_REQUEST) as $requestKey){
	if (strpos($requestKey, '_id') !== false && isset($_REQUEST[$requestKey])){
		$_REQUEST[$requestKey] = clean_string($_REQUEST[$requestKey]);
	}
}
...
if($current_user->id == $focus->assigned_user_id || $current_user->is_admin){
    $xtpl->parse("main.owner1");
    $xtpl->parse("main.owner2");
}
