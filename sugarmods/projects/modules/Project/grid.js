...
SUGAR.grid = function() {
	...
	return {
			...
			if (document.getElementById(divlink).style.display != 'none') {
				//if (document.getElementById(input).value == "") {
					...
					if (divlinkVal.search("Minus.gif") != -1) {
						...
						"<img src=" + imagePath + "ProjectMinus.gif onClick='javascript:SUGAR.grid.expandCollapseRow("+ row +")'>&nbsp;" + 
						"<b>" + document.getElementById(input).value + "</b>";
					}
					else if (divlinkVal.search("Plus.gif") != -1) {
						document.getElementById(divlink).innerHTML = 
						"<img src=" + imagePath + "ProjectPlus.gif onClick='javascript:SUGAR.grid.expandCollapseRow("+ row +")'>&nbsp;" + 
						"<b>" + document.getElementById(input).value + "</b>";
					}
					else {
						document.getElementById(divlink).innerHTML = document.getElementById(input).value;
		...
		insertRow: function(atRow) 
		{
			...
			SUGAR.grid.setupCalendar(numRows, calendar_dateformat, bg_color, gridName, imagePath);
...
/*
		...
		outdentRow: function(row)
		{
			...
			if (allChildren.length > 0) {
				newElementVal += "<img src=" + imagePath + "ProjectMinus.gif onClick='javascript:SUGAR.grid.expandCollapseRow("+ row +")'>&nbsp;";
		...
		indentParentRow: function(row) {
			...
			if (parentElementVal.search(".gif") == -1)
			{
				...
				if (parentDataStart > 0) {
					...
					newParentElementVal = "";
				newParentElementVal += "<img src=" + imagePath + "ProjectMinus.gif onClick='javascript:SUGAR.grid.expandCollapseRow("+ indentedRows[row].parentRow +")'>&nbsp;";
...
//			extendedAncestors.sort(SUGAR.grid.sortNumber);
		...
		*/
		...
		isChildsPredecessor: function (row, allChildren) {
			for (var i = 0; i < allChildren.length; i++) {
				var predecessorsVal = document.getElementById('predecessors_' + allChildren[i]).value;
				var predecessorsArray = predecessorsVal.split(",");
				for (var j = 0; j < predecessorsArray.length; j++) {
					if (predecessorsArray[j] == row)
						return true;
				}
			}
			return false;
...
//			debugger
			...
			for (var i = 0; i < predecessorsArray.length; i++) {
				...
				if (predecessorsArray[i] == SUGAR.grid.getActualRow(dependencyCheckRow) || SUGAR.grid.isAncestor(SUGAR.grid.getMappedRow(predecessorsArray[i]), allAncestors) ||
					SUGAR.grid.isChildsPredecessor(row, allChildren)) {
		...
		viewTaskDetails: function () {
			...
			frames["task_detail_area_iframe"].location.href =
				"index.php?module=ProjectTask&fromGrid=1&sugar_body_only=1&action=EditView&record=" + record;
		},
		...
		//	parent.document.getElementById("task_detail_area_div").style.position = "absolute";
		//	parent.document.getElementById("task_detail_area_div").style.top = "350";

			parent.document.getElementById("task_detail_area_div").style.display = "";
			parent.document.getElementById("task_detail_area_div").innerHTML = parent.frames["task_detail_area_iframe"].document.body.innerHTML;
			SUGAR.grid.unSelectRow(selectedRows[0]);
		},
		 ...
		 */	
		setupCalendar: function(forRow, dateformat, bgcolor, name, imgPath) {
			...
			if (!forRow) {
				for (var i = 1; i < document.getElementById(gridName).rows.length; i++) {
					Calendar.setup ({
						inputField : "date_start_" + i, ifFormat : calendar_dateformat, showsTime : false, button : "date_start_" + i, singleClick : true, step : 1
					});
					Calendar.setup ({
						inputField : "date_finish_" + i, ifFormat : calendar_dateformat, showsTime : false, button : "date_finish" + i, singleClick : true, step : 1
					});
				...
				Calendar.setup ({
					inputField : "date_start_" + forRow, ifFormat : calendar_dateformat, showsTime : false, button : "date_start_" + forRow, singleClick : true, step : 1
				});
				Calendar.setup ({
					inputField : "date_finish_" + forRow, ifFormat : calendar_dateformat, showsTime : false, button : "date_finish" + forRow, singleClick : true, step : 1
				});		
			}
		},
...
//			debugger
		...
		exportToPDF: function() {
			...
			//document.getElementById('EditView').attributes['action'].value = "pdf.php"
			//document.getElementById('EditView').submit();			
			//document.getElementById('EditView').action.value='SaveGrid';
			//document.getElementById('EditView').attributes['action'].value = "index.php"
			var pdfWindow = window.open('about:blank');
			/*
			//pdfWindow.document.body.innerHTML = '<div id="hiddenPDFdiv" style="display:none;"></div>';
			var hiddenPDFdiv = pdfWindow.document.createElement('div');
			hiddenPDFdiv.setAttribute('id', 'hiddenPDFdiv');
			hiddenPDFdiv.setAttribute('style', "'display:none'");
			pdfWindow.document.body.appendChild(hiddenPDFdiv);
			pdfWindow.document.getElementById('hiddenPDFdiv').innerHTML = document.body.innerHTML;
			pdfWindow.document.getElementById('EditView').action.value='Layouts';
			pdfWindow.document.getElementById('EditView').attributes['action'].value = "pdf.php"
			// This submit doesn't work on Firefox. 
			pdfWindow.document.getElementById('EditView').submit();			
			*/
			
			// Submit only works on Firefox if we open, write and close the document.
			pdfWindow.document.open();
			pdfWindow.document.write('<div id="hiddenPDFdiv" style="display:none;"></div>');			
			pdfWindow.document.close();
			
			// Mozilla Support
			if (!document.selection) {
				var body = document.body; 
				var allNodes = body.cloneNode(true);
				pdfWindow.document.getElementById('hiddenPDFdiv').appendChild(allNodes); 
			}
			else { // IE Support
				pdfWindow.document.getElementById('hiddenPDFdiv').innerHTML = document.body.innerHTML;
			}
		...
		changeView: function() {
			...
			if (parseInt(document.getElementById("gridViewSelect").value) == 5) {
				...
				document.getElementById('view_filter_button').className = "button";
				Calendar.setup ({inputField : "view_filter_date_start", ifFormat : calendar_dateformat, showsTime : false, button : "view_filter_date_start", singleClick : true, step : 1});
				Calendar.setup ({inputField : "view_filter_date_finish", ifFormat : calendar_dateformat, showsTime : false, button : "view_filter_date_finish", singleClick : true, step : 1});
			}
			else
				document.getElementById("EditView").submit();
