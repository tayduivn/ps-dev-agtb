 ...
 ********************************************************************************/
/*********************************************************************************
 * $Id: ListView.php 16706 2006-09-13 00:01:01 +0000 (Wed, 13 Sep 2006) jenny $
...
if(file_exists('custom/modules/ProjectTask/metadata/listviewdefs.php')){
    require_once('custom/modules/ProjectTask/metadata/listviewdefs.php');  
}else{
    require_once('modules/ProjectTask/metadata/listviewdefs.php');
}
...
$seedProjectTask = new ProjectTask(); // seed bean
$searchForm = new SearchForm('ProjectTask', $seedProjectTask); // new searchform instance
...
if(!empty($_REQUEST['displayColumns'])) {
    foreach(explode('|', $_REQUEST['displayColumns']) as $num => $col) {
        if(!empty($listViewDefs['ProjectTask'][$col])) 
            $displayColumns[$col] = $listViewDefs['ProjectTask'][$col];
    }    
}
elseif(!empty($savedDisplayColumns)) { // use user defined display columns from preferences 
    ...
    foreach($listViewDefs['ProjectTask'] as $col => $params) {
...
if(isset($_REQUEST['query']))
{
    ...
    $where_clauses = $searchForm->generateSearchWhere(true, "ProjectTask"); // builds the where clause from search field inputs
...
// awu: Bug 11774 - removing export for non-admin users without a mass update form
...
if (!is_admin($current_user)){
	$lv->setup($seedProjectTask, 'include/ListView/ListViewNoMassUpdate.tpl', $where, $params);
}
else {
	$lv->setup($seedProjectTask, 'include/ListView/ListViewGeneric.tpl', $where, $params);
}
