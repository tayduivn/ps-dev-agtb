...
require_once ('include/utils/zip_utils.php');
 ...
 class PortalSync{
	...
	function performSync($session, $force_md5_sync = false){
    	...
    	$file_list = array();
    	if(!$is_first_sync){
    		$all_include_files = findAllTouchedFiles( "include", array(), $last_local_sync);    			
    		$all_src_files  = array_merge(findAllTouchedFiles( "modules", array(), $last_local_sync), $all_include_files);
    		foreach( $all_src_files as $src_file ){
        		$file_list[$src_file] = $src_file;
    		}
    	}else{
    		$all_include_files = findAllFiles( "include", array(), $last_local_sync);    			
    		$all_src_files  = array_merge(findAllFiles( "modules", array(), $last_local_sync), $all_include_files);
