...
class SugarFieldUsername extends SugarFieldBase {
    function getDetailViewSmarty($parentFieldArray, $vardef, $displayParams) {
        global $sugar_config;
        if($vardef['value'] == $sugar_config['portal_username']) return;  // do not display portal username
        $this->setup($parentFieldArray, $vardef, $displayParams);

        return $this->ss->fetch('include/SugarFields/Fields/SugarFieldUsername/SugarFieldUsernameDetailViewSmarty.tpl');
    }
