...
class SugarFieldVarchar extends SugarFieldBase {
    ...
    function getDetailViewSmarty($parentFieldArray, $vardef, $displayParams) {
        ...
        return $this->ss->fetch('include/SugarFields/Fields/SugarFieldVarchar/SugarFieldVarcharDetailViewSmarty.tpl');
    }

    function getEditViewSmarty($parentFieldArray, $vardef, $displayParams) {
        if(!isset($displayParams)) $displayParams = array();
        if(!isset($displayParams['size'])) $displayParams['size'] = 25;

        $this->setup($parentFieldArray, $vardef, $displayParams);

        return $this->ss->fetch('include/SugarFields/Fields/SugarFieldVarchar/SugarFieldVarcharEditViewSmarty.tpl');
    }
}
?>
